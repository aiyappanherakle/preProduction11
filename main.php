...
if (isset($ilance->GPC['cmd']) AND $ilance->GPC['cmd'] == 'cp')
{
        ...
        if (isset($_SESSION['ilancedata']['user']['userid']) AND $_SESSION['ilancedata']['user']['userid'] > 0)
        {
				...
				$cons_note_sql="SELECT c.consignid as consign_id ,cm.message as consign_message,date_format(cm.created_on,'%D %b %Y %h:%i %p')as created_on
                FROM " . DB_PREFIX . "consignment_messages cm
                join " . DB_PREFIX . "consignments c on cm.consignment_id=c.consignid
				WHERE c.user_id = '".$_SESSION['ilancedata']['user']['userid']."'
				ORDER BY cm.created_on desc";
				
				$cons_noteresult = $ilance->db->query($cons_note_sql, 0, null, __FILE__, __LINE__);
				if($ilance->db->num_rows($cons_noteresult)>0)
				{
				$i=0;
				while ($res_consign = $ilance->db->fetch_array($cons_noteresult))
					{
						$i++;
						

						
						if(!is_null($res_consign['consign_message']))
						{
						$show['message_column']=true;
			
						}else
						{
						$res_consign['consign_message']='';
						}
						if($i>10)
						{
						$show['view_list']=true;
						$details_link='<a href="'.HTTPS_SERVER.'Consignmentsreceived">View more</a>';
											
						}
						if($i < 11)
						{
						$res_consign['created_on'] = date('M d, Y h:i A',strtotime($res_consign['created_on']));
						$res_consign['consign_id'] = $res_consign['consign_id'];	
						$res_consign['consign_message'] = $res_consign['consign_message'];	
                ...
                $sqlbannerlist = $ilance->db->query("SELECT *
                FROM ".DB_PREFIX."banner_order AS bo
                LEFT JOIN ".DB_PREFIX."banner AS b ON bo.banner_id = b.id
                LEFT JOIN ".DB_PREFIX."projects AS p ON b.coin_id = p.project_id
                LEFT JOIN ".DB_PREFIX."catalog_toplevel AS ct ON ct.id = p.coin_series_denomination_no
                where p.user_id ='".$_SESSION['ilancedata']['user']['userid']."'");
                    $bannerlist = $ilance->db->num_rows($sqlbannerlist);
                    if ($bannerlist > 0)
                    {
                        $bann .='<tr>
                        <td style="vertical-align: top;">Active Banner Items      
                        </td>
                        <td style="vertical-align: top;"><div {curr_scroll}>
                        <table align="left" border="0" cellpadding="0" cellspacing="0"  width="95%">';
                        while($fetch_text=$ilance->db->fetch_array($sqlbannerlist))
                        { 

                                 $bann .=' <tr>
                                    <td style="padding-bottom: 16px;"><strong>
                                <a style="color: #e16e19y;" href="'.HTTP_SERVER.'merch.php?id='.$fetch_text['project_id'].'"> Congratulations! Your "'.$fetch_text['project_title'].'" is currently featured on our home page in a banner.</a></strong></td>     
                                  </tr>';
                        } 
                        $bann .='</table></div>
                         </td>
                        </tr>';
                        $banner['newfeature']= $bann;
                        $banner_loop[]=$banner;
                    }
                      
                    if ($bannerlist > 10)
                    {  
                        $curr_scroll='class="myBox"';
                    }
                    else
                    {
                        $curr_scroll='';
                ...
                $pprint_array = array('details_link','curr_scroll','counthotlist','enable_hotlist','curr_win','curr_out','access_bb','enable_batch_bid','is_auto_lower_min_bid','auto_min_bid_lower_prec','curr_sell','subscriptiondaysleft','subscriptionexpirydate','unpaidinvoices','unpaidamount','flashstats','sellingreminders','buyingreminders','datereset','bidsleft','ridlink','referalactivity','mycalendar','remote_addr','rid','login_include','headinclude','onload','area_title','page_title','site_name','https_server','http_server');
				...
				$ilance->template->parse_loop('main', array('consign_receive'));
        ...
        if (in_array($ilance->GPC['cmd'], $accepted))
        {
		   ...
		   //  end 1457
		       ...
		       // changes on Sep27
                ...
                if($ilance->GPC['cmd'] == 'FUN')
                {
                    $area_title = 'FUN (Florida United Numismatists)';
                } 	
...
else
{
	   ...
	   from 	   
		...
		$mydaily.= '<div class="panel">
				...
				//banner
	$sql_banner = $ilance->db->query("SELECT * FROM " . DB_PREFIX . "banner_order o left join  " . DB_PREFIX . "banner b on b.id=o.banner_id ORDER BY o.id ASC
									", 0, null, __FILE__, __LINE__);
	$v=1;
