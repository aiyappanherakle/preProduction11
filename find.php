...
$drop_sql=$ilance->db->query("SELECT coin_series_unique_no,coin_series_name FROM ilance_catalog_second_level order by coin_series_unique_no asc", 0, null, __FILE__, __LINE__);
...
//        $show['widescreen'] = true;
			...
			if ($show['mode_service'])
			{
				...
				else
				{
					$sqlquery['genrequery'] = '';
                        ...
                        $sqlquery['select'] = "SELECT $sqlquery[fields] $sqlquery[from] $sqlquery[leftjoin] $sqlquery[leftjoin_attachment] $sqlquery[leftjoin_proxy]  WHERE p.user_id = u.user_id AND u.status = 'active' " . (($ilconfig['globalauctionsettings_payperpost']) ? "AND p.visible = '1' AND (p.insertionfee = 0 OR (p.insertionfee > 0 AND p.ifinvoiceid > 0 AND p.isifpaid = '1'))" : "AND p.visible = '1'") . "";
						
						$sqlquery['select_count'] = "SELECT count(*) $sqlquery[from]  $sqlquery[leftjoin] WHERE p.user_id = u.user_id AND u.status = 'active' " . (($ilconfig['globalauctionsettings_payperpost']) ? "AND p.visible = '1' AND (p.insertionfee = 0 OR (p.insertionfee > 0 AND p.ifinvoiceid > 0 AND p.isifpaid = '1'))" : "AND p.visible = '1'") . "";
						
						
                        //sukumar
                        // #### build sql query ################################
                        echo $SQL  = "$sqlquery[select] $sqlquery[timestamp] $sqlquery[projectstatus] $sqlquery[keywords] $sqlquery[categories] $sqlquery[projectdetails] $sqlquery[projectstate] $sqlquery[options] $sqlquery[pricerange] $sqlquery[location] $sqlquery[radius] $sqlquery[userquery] $sqlquery[hidequery] $sqlquery[genrequery] $sqlquery[groupby] $sqlquery[orderby] $sqlquery[limit]";
                        $SQL2 = "$sqlquery[select_count] $sqlquery[timestamp] $sqlquery[projectstatus] $sqlquery[keywords] $sqlquery[categories] $sqlquery[projectdetails] $sqlquery[projectstate] $sqlquery[options] $sqlquery[pricerange] $sqlquery[location] $sqlquery[radius] $sqlquery[userquery] $sqlquery[hidequery] $sqlquery[genrequery] $sqlquery[orderby]";
			...
			else
			{
				...
				$text .= '&nbsp;&nbsp;<span class="smaller gray">[ <span class="blueonly"><a href="javascript:void(0)" rel="nofollow" onclick="javascript:jQuery(\'#saved_search_modal\').jqm({modal: false}).jqmShow();" onmouseover="Tip(\'<div><strong>' . $phrase['_save_this_search'] . '</strong></div><div>' . $phrase['_saved_searches_can_be_used_when_you_are_viewing_search_results_from_the_marketplace'] . '</div>\', BALLOON, true, ABOVE, true, OFFSETX, -17, FADEIN, 600, FADEOUT, 600, PADDING, 8)" onmouseout="UnTip()">' . $phrase['_save_as_favorite_search'] . '</a></span> ]</span>';
                      ...
                      //  $page_title = ((isset($keyword_text) AND !empty($keyword_text)) ? $keyword_text . ', ' : '') . '' . $phrase['_find'] . ' ' . (($project_state == 'service') ? $phrase['_services'] : $phrase['_items']) . ' ' . ((!empty($metadescription)) ? $phrase['_in'] . ' ' . $metatitle . ', ' . $metadescription : '') . ' | ' . SITE_NAME;
						...
						//changes on aug 23 for page_title
                        ...
                        if ($ilance->db->num_rows($result) > 0)
                        {
							...
							$picture_sql = $ilance->db->query("SELECT COUNT(attachid) AS picture_count 
															   FROM " . DB_PREFIX . "attachment
															   WHERE project_id='".$row['project_id']."'
																");
							$picture_res = $ilance->db->fetch_array($picture_sql);
							
							$pictures=$picture_res['picture_count'];
	...
	: '<div class="smaller blue"> Bid </div>';
					...
					$pbit = $row['maxamount'];
						...
						//sukumar
					 $featuredproductauctions = ($show['mode_product']) ?fetch_featured_auctions_new('product', 4, 1, $cid, $keyword_text, false, $excludelist,$series) : '';
...
$result=$ilance->db->query("SELECT b.user_id
                                        FROM ilance_project_bids AS b,
                                        ilance_proxybid AS p
