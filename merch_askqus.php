			...
			if(isset($users_list) and is_array($users_list) and count($users_list)>0)
			{
				...
				foreach($bid_results_rows as $bids_row)
				{
					...
					if(isset($_SESSION['ilancedata']['user']['userid']) and $_SESSION['ilancedata']['user']['userid']>0 and $bids_row['user_id']==$_SESSION['ilancedata']['user']['userid'])
						$bids_row['provider']=$_SESSION['ilancedata']['user']['username'];
					else
...
//-->
			...
			$slideq = $ilance->db->query("
			    ...
			    $profile_slidqft = ((PROTOCOL_REQUEST == 'https') ? HTTPS_SERVER:HTTP_SERVER) .  'image.php?cmd=thumb&subcmd=itemphoto&id=' . $rowt['filehash'] .'&w=268';
...
For a larger image, click on lower right hand corner.
	...
	<img  class="rounded" title="'.$res['project_title'].'Online Coin Auction at GreatCollections"  alt="'.$res['project_title'].'Online Coin Auction at GreatCollections"  src="' .((PROTOCOL_REQUEST == 'https') ? HTTPS_SERVER:HTTP_SERVER).  'image.php?cmd=thumb&subcmd=itemphoto&id=' . $rowt['filehash'] .'&w=170&h=140"/></a><div class="highslide-caption" align="center">
 For a larger image, click on lower right hand corner.
</div></td>';
...
//daily deal
				...
				if(!empty($_SESSION['ilancedata']['user']['userid']) AND $_SESSION['ilancedata']['user']['userid'] > 0)
				{
					$user_detls = $ilance->db->query(" SELECT first_name, last_name  FROM " . DB_PREFIX . "users WHERE user_id = '" . $_SESSION['ilancedata']['user']['userid'] . "' ");
					...
					$ask_username = $userdet['first_name']." ".$userdet['last_name'];
	                        ...
	                        $pprint_array = array('ask_username','tt_est','newthumb','daily','soldprice','hidden_val','pageurlpath','twitpath','facepath','mesa','mess','disab','population_guide','price_guide_info','watch_list_note','no_tracking_users','alt_no','totalvalue','dismsg','vel','trva','newhead','pro_tes1','list','sold_text','currency','shipperid','featured','featured_date','highlite','bold','autorelist','enhancements','transactionstatus','winningbid','winningbidder','jsend','min_bidamountformatted','min_bidamount','date_starts','additional_info','views','proxybit','merchantscore','highbidderscore','currentbid','startprice','feed12','qty_pulldown','cid','reserve_met','buynow_price','buynow_price_plain','timeleft','ends','bids','highbidder','additional_info','buyer_stars','project_title','description','project_id','login_include','headinclude','onload','area_title','page_title','site_name','https_server','http_server'	);



			($apihook = $ilance->api('merch_detailed_end')) ? eval($apihook) : false;

                        $ilance->template->fetch('main', 'item_page_askqus.html');
			$ilance->template->parse_hash('main', array('ilpage' => $ilpage));
