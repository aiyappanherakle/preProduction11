...
else if(isset($ilance->GPC['cmd']) AND $ilance->GPC['cmd'] == '_validate-bid_amount' AND isset($ilance->GPC['bidamount']) AND $ilance->GPC['bidamount'] != '')
{
	$result['bidamount_error'] = false;
	$result['bidamount'] = '';

	//echo "<pre>ajax";print_r($ilance->GPC);echo "</pre>";exit();

	if (preg_match_all("/[\+]/", $ilance->GPC['bidamount']))
	{
		if (!preg_match('/^\+?\d+(\.\d{1,5})?+$/D', $ilance->GPC['bidamount']))
		{
			//print $ilance->GPC['bidamount']." is valid";echo "<br>";
			//$result['bidamount'] = preg_replace('/[+]/', "", $ilance->GPC['bidamount']);
			$result['bidamount_error'] = true;
		}
	}
...
else if (isset($ilance->GPC['cmd']) AND $ilance->GPC['cmd'] == '_do-bid-preview' AND isset($ilance->GPC['minimum']) AND isset($ilance->GPC['subcmd']) AND $ilance->GPC['subcmd'] == 'product-bid')
{
	...
	if (preg_match_all("/[\+]/", $ilance->GPC['bidamount']))
	{
		...
		{
			print $ilance->GPC['bidamount']." is not valid";echo "<br>";
			$result['bidamount_error'] = true;
			$result['bidamount'] = '';
		}
		echo "<pre>ajax";print_r($ilance->GPC);echo "</pre>";exit();
	...
	if (preg_match_all("/[\+]/", $ilance->GPC['bidamount'])) {
	    if (preg_match('/^\+?\d+(\.\d{1,5})?+$/D', $ilance->GPC['bidamount'])) {
			print $ilance->GPC['bidamount']." is valid";echo "<br>";
			$ilance->GPC['bidamount'] = preg_replace('/[+]/', "", $ilance->GPC['bidamount']);echo "<br>";
		} else {
			print $ilance->GPC['bidamount']." is not valid";echo "<br>";
		}
		echo "<pre>sdfsdfsd";print_r($ilance->GPC);echo "</pre>";exit();
    }
