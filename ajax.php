...
// #### FOR BUG #4836, #5027 ############
if (isset($ilance->GPC['do']) AND $ilance->GPC['do'] == 'watchlist_itempage' AND isset($_SESSION['ilancedata']['user']['userid']) AND $_SESSION['ilancedata']['user']['userid'] > 0)
{
        $ilance->xml = construct_object('api.xml');
	...
	// #### SAVE WATCHLIST SUBSCRIPTION PREFERENCES ########################
        if (isset($ilance->GPC['userid']) AND !empty($ilance->GPC['userid']) AND $ilance->GPC['userid'] > 0)
        {
                ...
                if (isset($ilance->GPC['projectid']) AND $ilance->GPC['projectid'] > 0)
                {
					...
					if(isset($ilance->GPC['cmd']) && $ilance->GPC['cmd'] == 'save_to_watchlist')
					{
						$success = $ilance->watchlist->insert_item(intval($ilance->GPC['userid']), intval($ilance->GPC['projectid']), 'auction', $comment, 0, 0, 0, 0);
						if ($success)
						{
							$watchingid = $ilance->GPC['projectid'];
							$userid = $ilance->GPC['userid'];
						
							$sql = $ilance->db->query("SELECT watchlistid FROM " . DB_PREFIX . "watchlist_log
									WHERE watching_project_id = '" . intval($watchingid) . "'	", 0, null, __FILE__, __LINE__);
		
							$tracking_users_count = $ilance->db->num_rows($sql);
						
								
								$status = 	'addedwatchlist||'.$watchingid.'||'.$userid.'||'.$tracking_users_count;
								$ilance->xml->add_tag('status', $status);
						}
						else
						{
								$ilance->xml->add_tag('status', 'alreadyaddedwatchlist');        
						}
					}
					
					if(isset($ilance->GPC['cmd']) && $ilance->GPC['cmd'] == 'removing_watchlist')
					{
							...
							$sql = $ilance->db->query("
                                ...
                                {
									$res = $ilance->db->fetch_array($sql);
									$watchlist_id = $res['watchlistid'];

									$del=$ilance->db->query("DELETE FROM " . DB_PREFIX . "watchlist WHERE watchlistid = '".$watchlist_id."' ");	
                                }
							$status = 	'removed_watchlist||'.$watchingid.'||'.$userid;
							$ilance->xml->add_tag('status', $status);
					}
       ...
       /*  if (isset($ilance->GPC['userid']) AND !empty($ilance->GPC['userid']) AND $ilance->GPC['userid'] > 0)
        {*/
				...
				else
				{
					...
					$first_ltr = $username[0];
					$name_valid = preg_replace('/\s+/', '', $username);
					if($name_valid == '' OR $first_ltr == ' ')
					{
						$val_eror = 1;
					}
				}
				
                if (isset($ilance->GPC['projectid']) AND $ilance->GPC['projectid'] > 0 AND $useremail != '' AND $username != '' AND $val_eror != 1)
                {
				//suku
				if(!empty($ilance->GPC['comment']))
				{
				...
				$txt .= '<div style= "margin-top:0px;"><table style="border: 1px solid #000000; border-collapse: collapse;" cellpadding=10><tr><td width ="175px" style="background-color: #5B5F68; border: 1px solid #000000; color:#ffffff;"><b>Item ID: </b></td><td  style="border: 1px solid #000000; ">'.$ilance->GPC['projectid'].'</td></tr>';
				
				if($Item_type=='fixed')
				{
				$txt .= '<tr><td style="background-color: #5B5F68; border: 1px solid #000000; height: 30px; color:#ffffff;"><b>Buy Now Price: </b></td><td style="border: 1px solid #000000; ">'.$ilance->currency->format(fetch_auction('buynow_price',$ilance->GPC['projectid'])). '' .'</td></tr>';
				}
                else
				{
				$txt .= '<tr><td style="background-color: #5B5F68; border: 1px solid #000000; height: 30px; color:#ffffff;"><b>Item Current Bid: </b></td><td style="border: 1px solid #000000; ">'.$ilance->currency->format(fetch_auction('currentprice',$ilance->GPC['projectid'])). '' .'</td></tr>';
				}
				$txt .='<tr><td style="background-color: #5B5F68; border: 1px solid #000000; height: 30px; color:#ffffff;"><b>Item Title:</b></td><td  style="border: 1px solid #000000; " colspan="3"><a href="'.$_SERVER['SERVER_NAME'].'/Coin/'.$ilance->GPC['projectid'].'/'.construct_seo_url_name(fetch_auction('project_title',$ilance->GPC['projectid'])).'">'.fetch_auction('project_title',$ilance->GPC['projectid']).'</a></td></tr>';
				if($Item_type=='fixed')
				{
				$txt .= '<tr><td style="background-color: #5B5F68; border: 1px solid #000000; height: 30px;color:#ffffff;"><b>End Date: </b></td><td  style="border: 1px solid #000000; ">' . date("l, F d, Y h:i:s A",strtotime($date_end)) .'(Pacific Time)'.'</td></tr>';
				}
				else
				{
				$txt .= '<tr><td style="background-color: #5B5F68; border: 1px solid #000000; height: 30px;color:#ffffff;"><b>Bidding Ends: </b></td><td  style="border: 1px solid #000000; ">' . date("l, F d, Y h:i:s A",strtotime($date_end)) . '(Pacific Time)'.'</td></tr>';
				}
				$txt .= '<tr><td style="background-color: #5B5F68; border: 1px solid #000000;color:#ffffff;"><b>Username: </b></td><td  style="border: 1px solid #000000; ">'.$username. '</td></tr><tr><td style="background-color: #5B5F68; border: 1px solid #000000;color:#ffffff;"><b>E mail id: </b></td><td  style="border: 1px solid #000000; ">'.$useremail. '</td></tr><tr><td style="background-color: #5B5F68; border: 1px solid #000000; color:#ffffff;"><b>IP Address: </b></td><td  style="border: 1px solid #000000; ">'.$ipaddress.'</td></tr><tr><td style="background-color: #5B5F68; border: 1px solid #000000; color:#ffffff; height: auto;" valign="top"><b>Question: </b></td><td  style="border: 1px solid #000000; " colspan="3" valign="top">'.$comment. '</td></tr></table></div>';							
				...
				$success= send_email_enquiry($to,$subject, $txt,OWNER_EMAIL,SITE_NAME,true,'alert',$ilance->GPC['projectid']);
...
if (isset($ilance->GPC['do']) AND $ilance->GPC['do'] == 'tellfriend')
{
	...
	// #### SAVE WATCHLIST SUBSCRIPTION PREFERENCES ########################
        ...
        {*/
				...
				$success= send_email_enquiry($to,$subject, $txt,OWNER_EMAIL,SITE_NAME,1);
				//$success=mail($to,$subject,$txt,$headers);	
				if ($success == '1')
				{
...
//vijay starts 
// #### ADD WATCHLIST USING STAR  #######################################################
if (isset($ilance->GPC['do']) AND $ilance->GPC['do'] == 'add_watchlist_star' AND isset($_SESSION['ilancedata']['user']['userid']) AND $_SESSION['ilancedata']['user']['userid'] > 0)
{
        $ilance->xml = construct_object('api.xml');
		$ilance->watchlist = construct_object('api.watchlist');
        
        ($apihook = $ilance->api('ajax_watching_project_id_start')) ? eval($apihook) : false;

                if (isset($ilance->GPC['watching_project_id']) AND $ilance->GPC['watching_project_id'] > 0)
                {
                    //suku
                    $comment='n/a';
					$ilance->GPC['watching_project_id'] = intval($ilance->GPC['watching_project_id']);
                    $ilance->GPC['value'] ;
                
                    if ($ilance->GPC['value'] == 'on')
                    {
					
                         $success = $ilance->watchlist->insert_item(intval($ilance->GPC['uid']), intval($ilance->GPC['watching_project_id']), 'auction', $comment, 0, 0, 0, 0);
                        if ($success)
                        {
                            $watchingid = $ilance->GPC['watching_project_id'];
                            $userid = $ilance->GPC['uid'];
                                                                              
                            $status =   'addedwatchlist';
                            $ilance->xml->add_tag('status', $status);
                        }
                        else
                        {
