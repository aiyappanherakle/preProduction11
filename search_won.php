...
$navcrumb = array("search-won" => 'Search Items I won');
...
if (!empty($ilance->GPC['mode']) AND $show['searcherror'] == 0) {
	...
	$scriptpage = HTTP_SERVER . 'search_won.php' . print_hidden_fields(true, array('do', 'cmd', 'page', 'budget', 'searchid', 'list'), true, '', '', $htmlentities = true, $urldecode = false);
...
//        $show['widescreen'] = true;
	...
	if (isset($ilance->GPC['series']) && $ilance->GPC['series'] > 0) {
		...
		$coin_sers = $ilance->categories_parser->fetch_coin_series($denomi_id, $ilance->GPC['series']);
		...
		if (!empty($coin_sers)) {
			if (!isset($ilance->GPC['denomination']) OR ((isset($ilance->GPC['denomination'])) && count($ilance->GPC['denomination']) == 1)) {
				$is_series_page = '<div class="block" style="margin-bottom: 20px;">
					...
					<div class="block-content" style="background:url(https://www.greatcollections.com/developer/images/gc/icons/bg_gradient_yellow_1x1000.gif) repeat-x; padding:14px">
		...
		case 'product':
			{
				...
				if (isset($ilance->GPC['ended'])) {
					...
					$sqlquery['orderby'] = "ORDER BY UNIX_TIMESTAMP(p.date_end) DESC";
				} else {
					...
					$sqlquery['orderby'] = "ORDER BY UNIX_TIMESTAMP(p.date_end) ASC";
				}
			...
			$keyword_text = '★';
				...
				$ilance->GPC['listing_type'] = isset($ilance->GPC['listing_type']) ? $ilance->GPC['listing_type'] : 4;
				...
				if ($show['mode_product']) {
					...
					if (!empty($ilance->GPC['grading_service']) || !empty($ilance->GPC['fromyear']) || !empty($ilance->GPC['toyear']) || !empty($ilance->GPC['grade_range_1']) || !empty($ilance->GPC['grade_range_2']) || !empty($ilance->GPC['frombid']) || !empty($ilance->GPC['tobid']) || !empty($ilance->GPC['listing_type']) || !empty($ilance->GPC['denom_all']) || !empty($ilance->GPC['denomination'])) {
						...
						if (!empty($ilance->GPC['grading_service'])) {
							...
							if ($gpc_sql_grading_service_arr[0] == 'ALL') {
								...
								if (in_array('QA', $gpc_sql_grading_service_arr)) {
									...
									$sqlquery_grading_service_cac = " OR p.QA=1 ";
								} else {
						...
						if (!empty($ilance->GPC['frombid']) || (isset($ilance->GPC['tobid']) AND $ilance->GPC['tobid'] >= 0)) {
							...
							if ($ilance->GPC['frombid'] == 0 AND $ilance->GPC['tobid'] != 500) {
				...
				$sqlquery['won'] = " AND (p.winner_user_id=".$_SESSION['ilancedata']['user']['userid']." ) ";

				//sukumar
				// #### build sql query ################################

				$SQL = "$sqlquery[select] $sqlquery[timestamp] $sqlquery[projectstatus] $sqlquery[won] $sqlquery[keywords] $sqlquery[categories] $sqlquery[projectdetails] $sqlquery[projectstate] $sqlquery[grading_service] $sqlquery[year_range] $sqlquery[grade_range] $sqlquery[bid_range] $sqlquery[listing_type] $sqlquery[denomination] $sqlquery[options] $sqlquery[pricerange] $sqlquery[location] $sqlquery[userquery] $sqlquery[hidequery] $sqlquery[genrequery] $sqlquery[orderby] $sqlquery[limit]";

				//echo $SQL;exit;
				$SQL2 = "$sqlquery[select_count] $sqlquery[timestamp] $sqlquery[projectstatus] $sqlquery[won] $sqlquery[keywords] $sqlquery[categories] $sqlquery[projectdetails] $sqlquery[projectstate] $sqlquery[grading_service] $sqlquery[year_range] $sqlquery[grade_range] $sqlquery[bid_range] $sqlquery[listing_type] $sqlquery[denomination] $sqlquery[options] $sqlquery[pricerange] $sqlquery[location] $sqlquery[userquery] $sqlquery[hidequery] $sqlquery[genrequery] $sqlquery[orderby]";
										...
										if ($row['bids'] > 0) {
											...
											if (!empty($_SESSION['ilancedata']['user']['userid'])) {
												...
												if ($pbit > 0) {
													...
													if ($highbidderidtest == $_SESSION['ilancedata']['user']['userid']) {
														$td['proxybit'] = (!empty($selected['proxybit']) AND $selected['proxybit'] == 'true')
														? '<div class="smaller green" style="padding-top:4px"> Won this item : ' . $ilance->currency->format($pbit, $row['currencyid']) . ' <em> ' . $phrase['_invisible'] . '</em></div>'
														: '';
													} else {
														$td['proxybit'] = (!empty($selected['proxybit']) AND $selected['proxybit'] == 'true')
														? '<div class="smaller red" style="padding-top:4px"> Won this item : ' . $ilance->currency->format($pbit, $row['currencyid']) . ' <em> ' . $phrase['_invisible'] . '</em></div>'
														: '';
													}
	...
	//5189 starts
							...
							if (isset($_SESSION['ilancedata']['user']['userid'])) {
								...
								$watch_qry = $ilance->db->query("SELECT watching_project_id,comment FROM " . DB_PREFIX . "watchlist WHERE watching_project_id='" . $row['project_id'] . "' AND user_id='" . $_SESSION['ilancedata']['user']['userid'] . "' ", 0, null, __FILE__, __LINE__);

								if ($ilance->db->num_rows($watch_qry) > 0) {
									while ($watch_row = $ilance->db->fetch_array($watch_qry)) {
										$watch_cmnts = "<div><strong>Notes: </strong>" . $watch_row['comment'] . "</div>";
										//print_r($watch_row);exit;
									}
							...
							if (isset($row['hotlists']) AND $row['hotlists'] > 0) {
								$hotlistimg = '<img border="0" title="This coin has been selected for the GreatCollections HotList" alt="This coin has been selected for the GreatCollections HotList" src="https://www.greatcollections.com/images/gc/hotlist.png"  style="width: 25px; height: 35px;">';
							}
	...
	////5189 ends
				...
				$search_results_table = print_search_results_table($search_results_rows, $project_state, $prevnext, '', '', 'search_won');
				...
				if (isset($show['mode_service']) AND $show['mode_service'] OR isset($show['mode_providers']) AND $show['mode_providers']) {
					$v3left_nav = $ilance->template->print_left_nav('service', $cid, $dosubcats = 1, $displayboth = 0, $ilconfig['globalfilters_enablecategorycount'], true, 0,'won');
				} else {
					...
					$v3left_nav = $ilance->template->print_left_nav('product', $cid, $dosubcats = 1, $displayboth = 0, $ilconfig['globalfilters_enablecategorycount'], true, 0,'won');
				...
				// #### DISPLAY SEARCH RESULTS TEMPLATE ################
					...
					$ilance->template->fetch('main', 'search_results_won.html');
					$ilance->template->parse_hash('main', array('ilpage' => $ilpage));
...
exit();
	...
	refresh(HTTPS_SERVER . $ilpage['login'] . '?redirect=' . urlencode('search_won.php'. print_hidden_fields($string = true, $excluded = array(), $questionmarkfirst = true)));
	exit();
}
		...
		if ($ilance->db->num_rows($sqlproductauctions) > 0) {
			...
			while ($res = $ilance->db->fetch_array($sqlproductauctions, DB_ASSOC)) {
				...
				if (!empty($_SESSION['ilancedata']['user']['userid'])) {
					...
					$res['watchlist'] = '<a href="javascript:void(0)" onclick="login_watch(' . $res['project_id'] . ');" class="tooltip" style="text-decoration:underline"><img src="' . $ilconfig['template_relativeimagepath'] . $ilconfig['template_imagesfolder'] . 'star_off.gif" alt=""   border="0" />
                                    <span>
                                        <img class="callout" src="' . $ilconfig['template_relativeimagepath'] . $ilconfig['template_imagesfolder'] . 'callout.gif"   />
