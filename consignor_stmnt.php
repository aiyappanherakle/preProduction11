...
if (!empty($_SESSION['ilancedata']['user']['userid']) AND $_SESSION['ilancedata']['user']['userid'] > 0 )
{
	 ...
	 $user_details['user_id'] = $ilance->GPC['user_id'];
	 $start = $ilance->GPC['start'];
	 $stmt_date = $ilance->GPC['stmt_date'];
	...
	if($ilance->GPC['start'] != '' AND $ilance->GPC['stmt_date'] != '')
	{
	...
	$sql="SELECT u.user_id, u.username, u.email,u.Check_Payable FROM " . DB_PREFIX . "_users u  WHERE u.user_id='".$user_details['user_id']."' ";
	...
	if($ilance->db->num_rows($user_query) > 0)
	{
		...
		while($coins_list_line=$ilance->db->fetch_array($select))
		{
			...
			switch ($coins_list_line['filtered_auctiontype'])
				...
				// //echo $coins_list_line['Buy_it_now'];exit;
				...
				$statement_seller_total+=$coin_consignor_total;	
					...
					if($coin_consignor_total > 0){
						...
						$data_pdf['net_consignor'] = '$'.$coin_consignor_total_csv. '.00';
					}
					...
					$data_to_pdf .= '<tr>								
										...
										<td width = "45%">
											'.$data_pdf['Title'].'<br/>
												Certificate No.: '.$data_pdf['Certification_No'].'';
...
// $table1.='<table width="100%"><tr><td>Thank you for consigning with GreatCollections. Please see your consignor statements below, which will also be sent to you with your check within 30 days of the auction date. We pay most consignors early - usually within 7-14 days. Checks are sent automatically. Your unsold coins will be relisted (with no additional listing fees charged).</td></tr></table>';
...
$p->output('Consignor Statement '.DATETIME24H.'.pdf','D');
	...
	{
		  print_action_failed("Please Login to Continue",login.'.php');
