...
if (!empty($_SESSION['ilancedata']['user']['userid']) AND $_SESSION['ilancedata']['user']['userid'] > 0 AND $_SESSION['ilancedata']['user']['isstaff'] == '1')
{
	...
	$sql="SELECT *,DATE_ADD(scheduled_date, INTERVAL 5 DAY) as payment_due   FROM " . DB_PREFIX . "invoices WHERE date(scheduled_date) between '".$start_date."' and '".$end_date."'  and combine_project!='' order by scheduled_date";
	...
	while($test=$ilance->db->fetch_array($result))
	{
		...
		$res[] = $composed;
				...
				$composed['amount']='';
				$composed['quantity']='';
		//all items related to this invoice
		$sql5="SELECT i.invoiceid as iteminvoiceid,i.projectid,i.createdate,i.amount as itemamount, b.qty as buynowqty  FROM " . DB_PREFIX . "invoices i
		left join ilance_buynow_orders b on b.orderid=i.buynowid and b.invoiceid=i.invoiceid
		WHERE  i.invoiceid in (".$test['combine_project'].")";
		$res5 = $ilance->db->query($sql5, 0, null, __FILE__, __LINE__);
		if($ilance->db->num_rows($res5)>0)
		{
			while($line5=$ilance->db->fetch_array($res5))
			{
				$composed['Invoice_Number']	=$line5['iteminvoiceid'];
				$composed['Invoice_Date']	='';
				$composed['Payment_Due']	='';
				$composed['User_ID']		='';
				$composed['Summary']		='';
				$composed['Description']	='';
				$composed['Account_Name']	='';
				$composed['Account_Number']	='';
				$composed['Line_Item_Total']='';
				$composed['Tags']='';
				$composed['projectid']=$line5['projectid'];
				$composed['enddate']=$line5['createdate'];
				$composed['amount']=$line5['itemamount'];
				$composed['quantity']=$line5['buynowqty'];
				$res[]=$composed;
			}
		}
		$composed['projectid']='';
