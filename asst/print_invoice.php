...
if (!empty($_SESSION['ilancedata']['user']['userid']) AND ($_SESSION['ilancedata']['user']['isadmin'] == '1' OR ($_SESSION['ilancedata']['user']['userid'] == 12037 AND $_SESSION['ilancedata']['user']['isstaff'] == '1')))
{
$total_paid=0;
...
while($line1=$ilance->db->fetch_array($sql))
	...
	$buy = $ilance->db->query("SELECT qty FROM " . DB_PREFIX . "buynow_orders WHERE invoiceid = '".$line1['invoiceid']."'");
		...
		$amount=$ilance->currency->format($line1['amount']);
		 ...
		 $title=$ilance->db->fetch_field(DB_PREFIX . "projects", "project_id = '".$line1['projectid']."'", "project_title");
