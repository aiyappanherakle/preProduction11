...
else
{
	...
	if (!empty($_COOKIE[COOKIE_PREFIX . 'username']))
	{
        ...
        if (!empty($_SESSION['ilancedata']['user']['userid']) AND $_SESSION['ilancedata']['user']['isstuff'] == '1')
        {
                header("Location: " . HTTPS_SERVER . 'adminstuff/'.$ilpage['dashboard']);
                exit();
        }
	...
	$ilance->template->fetch('main', 'login_asst_2449.html', 3);
	$ilance->template->parse_hash('main', array('ilpage' => $ilpage));
	$ilance->template->parse_loop('main', array('v3nav','subnav_settings'));
	$ilance->template->parse_if_blocks('main');
	$ilance->template->pprint('main', $pprint_array);
