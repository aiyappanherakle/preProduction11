...
if (!empty($_SESSION['ilancedata']['user']['userid']) AND $_SESSION['ilancedata']['user']['userid'] > 0 AND $_SESSION['ilancedata']['user']['isstaff'] == '1')
{
	...
	$DATETODAY=$ilance->GPC['start_date'];
		 //unbooked
                 $query="
		SELECT sum(i.totalamount-i.taxamount) as unbooked_buynow,i.projectid,c.End_Date ,i.user_id,r.last_listed_time,i.paiddate  FROM " . DB_PREFIX . "invoices i 
		...
		and date(i.createdate) > date(ifnull(r.last_listed_time,'0000-00-00 00:00:00') ) and date(i.createdate) <= date('".$DATETODAY."')
                and date(i.paiddate)<=date('".$DATETODAY."')
	 ...
	 $headings[0]='COIN_ID';
	$headings[1]='END_DATE';
	$headings[2]='last_listed';
	$headings[3]='BUYER_ID';
	$headings[4]='paid_date';
	$headings[5]='AMOUNT';
