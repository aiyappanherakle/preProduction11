		...
		group by c.coin_id ORDER BY  c.coin_id ASC";
			...
			{ 
				...
				$coin_insertion_fee=0;	 
					...
					$row[0]=$coins_list_line['coin_id'];
					$row[1]=$coins_list_line['Title'];
					$row[2]=$coins_list_line['user_id'];
					$row[3]=$user_details['username'];
					$row[4]=$coins_list_line['cost'];
					$row[5]=$sold_quantity;
					$row[6]=$cost;
					$row[7]=$coin_final_price;
					$row[8]=$coin_consignor_total;
			 ...
			 $headings[4]='COST';
			 $headings[5]='QUANTITY';
			 $headings[6]='TOTAL COST';
			 $headings[7]='TOTAL HAMMER';
			 $headings[8]='TOTAL TO CONSIGNOR';
				$reportoutput = $ilance->admincp->construct_csv_data($data, $headings);
				$timeStamp = date("Y-m-d-H-i-s");
				$date_formatted=date('mdY',strtotime($ilance->GPC['to_payment']));
				$fileName = $date_formatted."inhouse_details-$timeStamp";
				$action = 'csv';
					if ($action == 'csv')
					{
						header("Pragma: cache");
