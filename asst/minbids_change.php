...
{	
	...
	if((isset($ilance->GPC['submit']) && $ilance->GPC['submit']=='Change') || (isset($ilance->GPC['submit_percent']) && $ilance->GPC['submit_percent']=='Change') ){
		...
		$pprint_array = array('buildversion','ilanceversion','login_include_admin','currentmotd','currentmotd_preview','admincpnews','totalusers','cbaddons','page','catid','module','input_style','remote_addr','rid','login_include','headinclude','area_title','page_title','site_name','https_server','http_server','lanceads_header','lanceads_footer');
	
		$ilance->template->fetch('main', 'minbids_change.html', 3);
