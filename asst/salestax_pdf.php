...
$headings=array('Invoice ','Created Date','Paid Date','Sub Total','Buyers Fee','Shipping','Sales Tax','Discount','Total Order','County','Name','Address','City','State','Zip Code');
...
header("Pragma: cache");
				...
				header('Content-Disposition: attachment; filename="'.'SalesTax_'.date('Y-m-d h-i-s').'.csv"');
				echo $reportoutput;
				echo $reportoutput1;
				die();
...
else
{
	...
	// BUG # 4649 kumaravel start
	if($ilance->db->num_rows($county_query) > 0)
	{
