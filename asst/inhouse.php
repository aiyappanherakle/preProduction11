...
if (!empty($_SESSION['ilancedata']['user']['userid']) AND $_SESSION['ilancedata']['user']['userid'] > 0 AND $_SESSION['ilancedata']['user']['isstaff'] == '1') {
	...
	if (isset($ilance->GPC['cmd']) AND $ilance->GPC['cmd'] == 'calc_inhouse_profit') {
		...
		(select count(distinct(coin_id)) from " . DB_PREFIX . "coins coin where u.user_id = coin.user_id and date(coin.End_Date)='" . $stmt_date . "' ) as coin_count,
		(select count(distinct(coin_id)) from " . DB_PREFIX . "coin_relist where u.user_id = user_id and date(actual_end_date)='" . $stmt_date . "') as relist_coin_count,
		(select count(distinct(project_id)) from " . DB_PREFIX . "buynow_orders where u.user_id = owner_id and date(orderdate)>='" . $stmt_date . "' and date(orderdate)<='" . $stmt_date . "') buynow_coin_count
		FROM ilance_users u
		" . $user_where . "
				...
				if ($ilance->db->num_rows($coins_list_query) > 0) {
					...
					while ($coins_list_line = $ilance->db->fetch_array($coins_list_query)) {
						...
						if (!empty($coin_final_price)) {
							...
							$hammer_sold_item .= '<td>' . $coin_final_price . '</td><td>' . $cost . '</td></tr>';
							$sno_count++;
						}
					...
					//$html.='<tr><td>S.NO</td><td>ITEM</td><td>HAMMER PRICE</td><td>Cost </td></tr>';
