...
function iL_Inline(varname, cbtype, formobjid, submitname, ilancecookie)
{
	...
	this.init = function(elements)
	{
		...
		if (this.fetch_ids())
		{
			for (i in this.cookieids)
			{
				if (this.cookieids[i] != '')
				{
					if (checkbox = fetch_js_object(this.list + this.cookieids[i]))
					{
	...
	this.set_button_counters = function()
	{
		if (this.submitname != '')
		{
			if (obj = fetch_js_object('inlinebutton'))
			{
				obj.value = this.submitname + " (" + this.cookiearray.length + ")";
			}
			if (obj = fetch_js_object('inlinebutton2'))
			{
				obj.value = this.submitname + " (" + this.cookiearray.length + ")";
			}	
		}
...
/*======================================================================*\
|| ####################################################################
|| # CVS: $RCSfile: functions_inline.js,v $ - $Revision: 1.00 $
|| ####################################################################
\*======================================================================*/