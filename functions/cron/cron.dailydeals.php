...
// #### NEW REGISTRATIONS ######################################################
 ...
 $deal = ilanc->db->query("SELECT * FROM " . DB_PREFIX . "dailydeals
...
", 0, null, __FILE__, __LINE__);
    ...
    //ensure that the user have subscribed for daily deals
	
	  $email_notify = fetch_user('emailnotify',$userid);
	  
      $query_deal = $ilance->db->query("SELECT dailydeal FROM " . DB_PREFIX . "email_preference 
						                                    WHERE user_id ='".$userid."'");
						
      $row_deal = $ilance->db->fetch_array($query_deal);							
				 
	  if( $row_deal['dailydeal'] == '1' AND $email_notify =='1')
	  {
