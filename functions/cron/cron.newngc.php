...
$sql = $ilance->db->query("
                        SELECT p.cid, c.coin_id, c.grading_service
FROM " . DB_PREFIX . "projects  p,  " . DB_PREFIX . "coins  c
WHERE DATE( p.date_starts ) >=  '2011-04-24'
AND DATE( p.date_end ) <=  '2011-12-31'
AND p.status =  'expired'
AND p.project_id = c.coin_id
AND c.grading_service =  'NGC'
		       ...
		       $details_coins = $ilance->db->query("
                        ...
                        WHERE category_id = '".$fetch_cid ['cid']."'");
						   ...
						   $attchment_details = $ilance->db->fetch_array($attchment);
							  ...
							  $textfile['auction_end'] = $auction_end;
...
exit; */
                 $ngc_pdf1 ='NGC Coin Number,PCGS Coin Number,Service,Certification Number,Auction Date,AuctionName,SaleNumber,LotNumber,LotURL,PricesRealized,Grade,Plus,Star,URL'."\n";
				  ...
				  $File = '/home/public_html/functions/newngc/date_sold.txt'; 
