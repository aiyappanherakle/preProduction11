...
$consign1 = $ilance->db->query("select CONVERT(GROUP_CONCAT(coins SEPARATOR ',')USING utf8) as consign_coins,CONVERT(GROUP_CONCAT(consign_type SEPARATOR ',')USING utf8) as consign_types, receive_date,user_id FROM " . DB_PREFIX . "consignments WHERE receive_date = '2014-09-04' GROUP BY user_id ");
	//echo $ilconfig['globalserversettings_developer_email'];exit;
echo 	"select CONVERT(GROUP_CONCAT(coins SEPARATOR ',')USING utf8) as consign_coins,CONVERT(GROUP_CONCAT(consign_type SEPARATOR ',')USING utf8) as consign_types, receive_date,user_id FROM " . DB_PREFIX . "consignments WHERE receive_date = '2014-09-04' GROUP BY user_id <br/>";
