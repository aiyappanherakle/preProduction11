...
($apihook = $ilance->api('cron_ship_unpaid_start')) ? eval($apihook) : false;
require_once(DIR_CORE . 'functions_shipping.php');

/* vijay  for bug :#6237 - Email 218  * start 12.2.16 */	

 
$inv_unpd_sql="SELECT u.user_id,u.country,u.username,u.issalestaxreseller,u.state
FROM " . DB_PREFIX . "invoices i
				...
				and u.user_id in ('5213','23600','25111')
				GROUP BY u.user_id	
				";

$sql_uid= $ilance->db->query($inv_unpd_sql);
		...
		while($sql_uid_coin = $ilance->db->fetch_array($sql_uid))
		{
			...
			{	
				...
				{  
					...
					$ilance->email->get('unpaid_invoice_with_base_shipping_notification');		
					$ilance->email->set(array(
					'{{customer}}' =>$username,
					'{{amount}}' => $pending_inv_amt,
					));
...
function print_shippment_nethod_pulldown($projects,$selected,$name,$totqty=0,$giv_user_id,$giv_countryid,$totalamount_pending_in)
	...
	else
	{
       //invoice  over $10,000	
		...
		if($first_shipment==true AND $only_buynow)
		{
...
($apihook = $ilance->api('cron_ship_unpaid_end')) ? eval($apihook) : false;
