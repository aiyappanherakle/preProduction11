...
$query="insert into ilance_daily_report (report_date,payment_per_method,total_payments,total_amount,total_shipping_cost,total_tax,buyer_fee_total,inhouse_hammer_total,noninhouse_hammer_total,total_revenue,unpaid_invoices_buyerfees,unpaid_item_amount,pending_total,checkedout_invoices_buyerfees,checkedout_amount,checked_out_total,total_Ar,house_acc_sold_coin_cost,profit_sold_house_acc_coin,remaining_house_acc_inventory,unbooked_buynow) values ('".$reportdate."',
...
$unpaid_invoices_buyerfees_query="SELECT sum(CASE WHEN IFNULL(n.invoiceid,0) THEN i.totalamount ELSE 0 END) as unpaid_invoices_buyerfees FROM " . DB_PREFIX . "invoices i
...
//unpaid invoices
$unpaid_item_amount_query="SELECT sum(i.amount) as unpaid_item_amount FROM " . DB_PREFIX . "invoices i 
...
$html.= "\r\nA/R Pending: $".$pending_total."(Hammer: $".$unpaid_item_amount." buynow: $".$unpaid_invoices_buyerfees." )";
...
function unbookedbuynow_data($DATEYESTERDAY)
{
global $ilance,$unbooked_buynow;
$html='';
$unbooked_buynow=0;
$last_monday=$this->last_monday($DATEYESTERDAY);
$query="SELECT sum(i.totalamount-i.taxamount) as unbooked_buynow FROM " . DB_PREFIX . "invoices i
left join " . DB_PREFIX . "coins c on c.coin_id=i.projectid and date(i.createdate)<date(c.End_Date)
WHERE i.buynowid > 0 and i.isfvf=0 and i.isif=0 and i.isbuyerfee=0 and i.ismis=0 and date(i.createdate)>='".$last_monday."'";
$result=$ilance->db->query($query);
if($ilance->db->num_rows($result))
{
	while($line=$ilance->db->fetch_array($result))
