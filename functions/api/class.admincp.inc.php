...
class admincp
{
        ...
        function construct_csv_data($data, $headings)
	{
                ...
                foreach ($data as $row)
                {
                        ...
                        {
                              $csv .= "\"" . $cell . "\",";
								//$csv .= "\"" . mb_convert_case($cell, MB_CASE_TITLE, "UTF-8"). "\",";
                        }
                        $csv = mb_substr($csv, 0, -1);
                        $csv .= LINEBREAK;
    ...
    //BUG #6148
    function construct_csv_data_mygc($data, $headings,$date='')
    {
                $csv = 'Consignor Statement -,"'.$date.'"';
                $csv .= LINEBREAK;
                $csv .= LINEBREAK;
	...
	//For Bug #4133
	function construct_csv_data_quickbooks($data, $headings)
	{
                $csv = '';
                foreach ($headings as $cell)
                {
	...
	/**
        ...
        */
        ...
        function construct_new_member($username = '', $password = '', $salt = '', $secretquestion = '', $secretanswer = '', $email = '', $first = '', $last = '',$Check_Payable='', $address = '', $address2 = '', $city = '', $state = '', $zipcode = '', $phone = '',$client_representative ='', $country = '', $dob = '', $rid = '', $languageid = '', $currencyid = '', $usertimezone = '', $usertimezone_dst = 0, $categories = '', $isadmin = 0, $issalestaxreseller = 0)
	{
                ...
                $ilance->db->query("
                        ...
                        (user_id, ipaddress, username, password, salt, secretquestion, secretanswer, email, first_name, last_name,Check_Payable,address, address2,
                        city, state, zip_code, phone,client_representative,country, date_added, status, dob, rid, account_number, available_balance, total_balance,
                        languageid, currencyid, timezoneid, timezone_dst, notifyservices, notifyproducts, notifyservicescats, emailnotify, isadmin, issalestaxreseller)
                        VALUES(
                        NULL,
						...
						'" . $ilance->db->escape_string($Check_Payable) . "',
						...
						'" . $ilance->db->escape_string($client_representative) . "',
                        '" . intval($country) . "',
                        '" . DATETIME24H . "',
                        'active',
        ...
        function print_version()
	{
        ...
        */
		...
		//arsath added for staff settings on 02-oct-2010
		function construct_admin_input_staff($configgroup = '', $returnurl = '')
        {
                global $ilance, $myapi, $ilconfig, $ilcollapse, $phrase, $page_title, $area_title, $ilpage, $show;
                ...
                if ($ilance->db->num_rows($sqlgrp) > 0)
                {
                        ...
                        while ($resgrpties = $ilance->db->fetch_array($sqlgrp, DB_ASSOC))
                        {
                                ...
                                $html .= '
                                <form method="post" action="'.$ilpage['staffsettings'] . '" name="updatesettings" accept-charset="UTF-8" style="margin: 0px;">
                                <input type="hidden" name="cmd" value="globalupdate" />
                                <input type="hidden" name="subcmd" value="_update-config-settings" />
                                <input type="hidden" name="return" value="' . $returnurl . '" />
                                <div class="block-wrapper">
	...
	*/
		...
		// murugan added sep 7 for auction wiin notification
		
		        /**
	* Function to suspend single or multiple auction from the marketplace
	*
	* @param	array	        array with user ids flagged for suspension
	*
	* @return       string          Returns HTML string of users suspended separated by a comma for display purposes
	*/
        function suspend_auction($ids = array())
        {
                global $ilance;
                ...
                foreach ($ids AS $inc => $userid)
                {
                        ...
                        if ($ilance->db->num_rows($sql) > 0)
                        {
                                $suspendauction .= '<strong>' . fetch_user('username', intval($userid)) . '</strong>, ';                     
	...
	* @return       string          Returns HTML string of users activated separated by a comma for display purposes
	*/
        function activate_auction($ids = array())
        {
                ...
                foreach ($ids AS $inc => $userid)
                {
                        ...
                        if ($ilance->db->num_rows($sql) > 0)
                        {
                                $activatedauction .= '<strong>' . fetch_user('username', intval($userid)) . '</strong>, ';
