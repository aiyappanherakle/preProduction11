...
class accounting_p2b extends accounting
{
	/**
        ...
        */
        function payment($userid = 0, $invoiceid = 0, $invoicetype = 'p2b', $amount = 0, $method = 'account', $gateway = '', $gatewaytxn = '', $silentmode = false)
        {
                ...
                if ($method == 'ipn')
                {
					...
					AND status = 'unpaid'
                        ...
                        if ($ilance->db->num_rows($sql) > 0)
                        {
                                ...
                                // track income reported for provider
                                insert_income_reported($res_invoice['p2b_user_id'], sprintf("%01.2f", $amount), 'credit');
                                ...
                                if ($ilance->db->num_rows($sql_sellerbalance) > 0)
                                {
                                        ...
                                        $ilance->email->get('buyer_payment_online_account_admin');		
                ...
                else if ($method == 'account')
                {
                        ...
                        {
                                $res_buyerbalance = $ilance->db->fetch_array($sql_buyerbalance);
					...
					if ($silentmode)
					{
						...
						if ($silentmode)
						{
							return true;
                                                ...
                                                $area_title = $phrase['_generated_invoice_payment_complete_menu'];
                                                $page_title = SITE_NAME . ' - ' . $phrase['_generated_invoice_payment_complete_menu'];
                                                
                                                print_notice($phrase['_invoice_payment_complete'], $phrase['_your_invoice_has_been_paid_in_full']."<br /><br />", $ilpage['accounting'], $phrase['_my_account']);
                                                exit();
                                        }
                                        else
                                        {
	...
	/**
        ...
        */
		...
		$ilance->email = construct_dm_object('email', $ilance);
                ...
                $area_title = $phrase['_generating_invoice_to_buyer'];
                $page_title = SITE_NAME . ' - ' . $phrase['_generating_invoice_to_buyer'];
        
                // #### invoice due dates ######################################
                $inv_due_date = gmdate('Y-m-d', mktime(0, 0, 0, gmdate('m', time() + 3600 * ($ilconfig['globalserverlocale_officialtimezone'] + $ilconfig['globalserverlocale_officialtimezonedst'])), gmdate('d', time() + 3600 * ($ilconfig['globalserverlocale_officialtimezone'] + $ilconfig['globalserverlocale_officialtimezonedst'])) + $ilconfig['invoicesystem_maximumpaymentdays'], gmdate('Y', time() + 3600 * ($ilconfig['globalserverlocale_officialtimezone'] + $ilconfig['globalserverlocale_officialtimezonedst']))));
                ...
                {
                        $new_txn_transaction = construct_transaction_id();
            
                        // construct unpaid transaction fee invoice to the customer generating the invoice
                        $txn_description = $phrase['_transaction_fee_for_rfp'] . ' #' . intval($projectid) . ': ' . fetch_auction('project_title', intval($projectid)) . ': ' . $phrase['_invoice_generation_to'] . ' ' . fetch_user('username', intval($buyerid));
                        ...
                        if ($instantpay OR fetch_user('autopayment', $sellerid))
                        {
                                ...
                                ", 0, null, __FILE__, __LINE__);
                                $resgetacc = $ilance->db->fetch_array($sqlgetacc);
                                ...
                                if ($resgetacc['available_balance'] >= $txnfee)
                                {
                                        ...
                                        // track income spent
                                        insert_income_spent($sellerid, sprintf("%01.2f", $txnfee), 'credit');
                ...
                $new_inv_transaction = construct_transaction_id();
                ...
                $p2b_invoice_id = $this->insert_transaction(
                        ...
                        0,
                        0,
                        $phrase['_invoice_generated_for_rfp'] . ' #' . intval($projectid) . ': ' . fetch_auction('project_title', intval($projectid)),
