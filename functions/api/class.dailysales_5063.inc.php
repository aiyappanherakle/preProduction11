...
echo $query="insert into ilance_daily_report_split (report_date,payment_per_method,total_payments,total_amount,total_shipping_cost,total_tax,buyer_fee_total,inhouse_hammer_total,noninhouse_hammer_total,total_revenue,unpaid_invoices_buyerfeesb4_13,unpaid_invoices_buyerfees_after_13,unpaid_invoices_buyerfees_14,unpaid_invoices_buyerfees_15,unpaid_item_amountb4_13,unpaid_item_amount_after_13,unpaid_item_amount_14,unpaid_item_amount_15,pending_total_2012,pending_total_2013,pending_total_2014,pending_total_2015,checkedout_invoices_buyerfees,checkedout_amount,checked_out_total,total_Ar,house_acc_sold_coin_cost,profit_sold_house_acc_coin,remaining_house_acc_inventory,unbooked_buynow) values ('".$reportdate."',
...
left join (select invoiceid ,user_id,projectid,totalamount,p2b_user_id from ilance_invoices where isfvf=0 and isif=0 and isenhancementfee=0 and ismis=0 and status='unpaid' and combine_project='' group by invoiceid) n on i.user_id=n.user_id and i.projectid=n.projectid  
left join ilance_users u on u.user_id=i.user_id
...
sum(CASE WHEN year(i.createdate)='2015' THEN i.amount ELSE 0 END) as unpaid_item_amount_15
 FROM ilance_invoices i 
