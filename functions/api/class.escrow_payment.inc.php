...
class escrow_payment extends escrow
{
	/**
        ...
        */
        function payment($userid = 0, $invoiceid = 0, $invoicetype = 'escrow', $amount = 0, $method = 'account', $gateway = '', $gatewaytxn = '', $silentmode = false)
        {
				...
				//suku modified for paypal
				//herakle 3:29 PM 3/28/2011
				if(!empty($res_invoice['combine_project']))
				{
					combine_invoice_payment($invoiceid,$gateway);
					return true;
				}
                                                        ...
                                                        if ($fee > 0)
                                                        {
                                                                ...
                                                                if ($availablebalance >= $escrowfee AND $autopayment)
                                                                {
                                                                        ...
                                                                        $ilance->db->query("
                                                                                ...
                                                                                '" . $ilance->db->escape_string($phrase['_escrow_transaction_fee_securing_funds_for_auction']) . ': ' . $ilance->db->escape_string(fetch_auction('project_title', $res_escrow['project_id'])) . ' #' . $res_escrow['project_id'] . "',
                                                                        ...
                                                                        insert_income_spent($res_escrow['project_user_id'], sprintf("%01.2f", $escrowfee), 'credit');
                                                                }
                                                                else
                                                                {
                                                                        $ilance->db->query("
                                                ...
                                                if ($ilconfig['escrowsystem_escrowcommissionfees'] AND $res_escrow['fee2invoiceid'] == 0)
                                                {
                                                        ...
                                                        if ($fee > 0)
                                                        {
                                                                ...
                                                                if ($availablebalance >= $escrowfee AND $autopayment)
                                                                {
                                                                        ...
                                                                        // additionally we'll track this fee payment for the total spending amount for the user
                                                                        insert_income_spent($res_escrow['user_id'], sprintf("%01.2f", $escrowfee), 'credit');
                                                                }
                                                                else
                                                                {
                                                                        $ilance->db->query("
							...
							'{{paymethod}}' => $gateway
                                        ...
                                        {
						$currencyid = fetch_auction('currencyid', $escrowprojectid);
						...
						", 0, null, __FILE__, __LINE__);
                                                ...
                                                $existing = array(
                                                        ...
                                                        '{{escrow_amount}}' => $ilance->currency->format(($res_escrow['bidamount'] + $res_escrow['shipping']), $currencyid),
							'{{paymethod}}' => $gateway
                                                );
                                                ...
                                                $ilance->email->mail = fetch_user('email', $res_escrow['user_id']);
                                                $ilance->email->slng = fetch_user_slng($res_escrow['user_id']);
                                                $ilance->email->get('product_escrow_payment_forward_bidders');		
                                                $ilance->email->set($existing);
                                                $ilance->email->send();
                                        }
                ...
                else if ($method == 'account')
                {
                        ...
                        if ($ilance->db->num_rows($sql_balance) > 0)
                        {
								...
								", 0, null, __FILE__, __LINE__);
                                                                ...
                                                                $existing = array(
                                                                        ...
                                                                        '{{project_title}}' => fetch_auction('project_title', $res_escrow['project_id']),
                                                                        '{{escrow_amount}}' => $ilance->currency->format(($res_escrow['bidamount'] + $res_escrow['shipping']), $currencyid),
									'{{paymethod}}' => $phrase['_account_balance']
                                                                );
							...
							if ($silentmode)
							{
								return true;
                                                        ...
                                                        print_notice($phrase['_invoice_payment_complete'], $phrase['_your_invoice_has_been_paid_in_full'], $ilpage['escrow'] . '?cmd=management&amp;bidsub=product-escrow', $phrase['_product_escrow_payments_out']);
                                                        exit();
                                                }
                                                else
                                                {
						...
						if ($silentmode)
						{
							return false;
                                                ...
                                                print_notice($phrase['_invoice_payment_warning_insufficient_funds'], $phrase['_were_sorry_this_invoice_can_not_be_paid_due_to_insufficient_funds'], $ilpage['accounting'], $phrase['_my_account']);
                                                exit();
                                        }                        
                                }
                                else
