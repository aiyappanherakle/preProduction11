...
class auction_award extends auction
{
        ...
        */
        function award_service_auction($projectid = 0, $bidid = 0, $ownerid = 0, $vendorid = 0, $notifybidders = 0, $slng = 'eng')
        {
                ...
                if ($ilance->db->num_rows($sel) > 0)
                {
                        ...
                        $winner_bid_amount = ($winner_bidamounttype == 'entire' OR $winner_bidamounttype == 'lot' OR $winner_bidamounttype == 'weight')
                                ? $result['bidamount']
                                : ($result['bidamount'] * $winner_bid_estimate_days);
                }
                ...
                if ($ilance->db->num_rows($sql_winner) > 0)
                {
                        ...
                        $winner_user_country = print_user_country($winner_user_id, $slng);
                }
                ...
                if ($ilance->db->num_rows($sql_owner) > 0)
                {
                        ...
                        $project_user_country = print_user_country($project_user_id, $slng);
                }
                ...
                if ($ilconfig['escrowsystem_enabled'] AND $usingescrow AND isset($winner_bid_amount) AND $winner_bid_amount > 0)
                {
                        ...
                        $escrow_invoice_id = $ilance->accounting->insert_transaction(
                                ...
                                0,
                                0,
                                $phrase['_escrow_payment_forward'] . ': ' . $project_title . ' #' . $projectid,
                        ...
                        $ilance->db->query("
                                ...
                                (escrow_id, bid_id, project_id, invoiceid, project_user_id, user_id, date_awarded, bidamount, fee, fee2, isfeepaid, isfee2paid, feeinvoiceid, fee2invoiceid, status)
                                VALUES(
                                NULL,
                                '" . intval($bidid) . "',
                                '" . intval($projectid) . "',
                ...
                build_category_count($result_project['cid'], 'subtract', "award_service_auction(): subtracting increment count category id $result_project[cid]");
                
                // #### REFERRAL SYSTEM TRACKER ################################
                update_referral_action('awardauction', intval($project_user_id));							
        ...
        * @return      nothing
        */
        function unaward_service_auction($bidid = 0)
        {
                global $ilance, $myapi, $ilconfig, $phrase, $page_title, $area_title, $ilconfig, $ilpage;
                ...
                if ($ilance->db->num_rows($bidsql) > 0)
                {
                        ...
                        if ($ilance->db->num_rows($prosql) > 0)
                        {
                                ...
                                if ($ilance->db->num_rows($select_bidders_wait_approval) > 0)
                                {
                                        ...
                                        while ($res2 = $ilance->db->fetch_array($select_bidders_wait_approval))
                                        {
                                                ...
                                                if ($ilance->db->num_rows($sql_rfpinfo) > 0)
                                                {
                                                        ...
                                                        if ($auctionisopen)
                                                        {
                                                                ...
                                                                build_category_count($res_rfpinfo['cid'], 'add', "unaward_service_auction(): adding increment count category id $res_rfpinfo[cid]");
                                                        }
                                                        ...
                                                        $ilance->accounting_fees->construct_final_value_fee(intval($bidid), $res_rfpinfo['cid'], $res_rfpinfo['project_id'], 'refund', $res_rfpinfo['project_state']);
                                                        ...
                                                        {
                                                                while ($res3 = $ilance->db->fetch_array($select_vendors_reviewing))
                                                                {
                                                                        ...
                                                                        {
                                                                                $run_project_results = $ilance->db->fetch_array($run_project_query);
                                                                                ...
                                                                                {
                                                                                        while ($res_emails = $ilance->db->fetch_array($select_all_previous_bidders_info))
                                                                                        {
                                                                                                // email admin
        ...
        */
        function service_auction_award_accept($bidid = 0, $userid = 0)
        {
                global $ilance, $myapi, $ilconfig, $phrase, $page_title, $area_title, $ilconfig;
                ...
                if ($ilance->db->num_rows($sel) > 0)
                {
                        while ($result = $ilance->db->fetch_array($sel))
                        {
                                ...
                                {
                                        $result_winner_user_email = $ilance->db->fetch_array($sel_winner_user_email);
                                        ...
                                        $winner_user_country = print_user_country($winner_user_id, fetch_user_slng($winner_user_id));
                                        ...
                                        if ($ilance->db->num_rows($sel_project_info) > 0)
                                        {
                                                while ($result_p = $ilance->db->fetch_array($sel_project_info, DB_ASSOC))
                                                {
                                                        ...
                                                        if ($ilance->db->num_rows($sel_project_email) > 0)
                                                        {
                                                                ...
                                                                $project_user_country = print_user_country(intval($ownerid), fetch_user_slng(intval($ownerid)));
																...
																//$ilance->accounting_fees->construct_final_value_fee(intval($bidid), $result_p['cid'], $result_p['project_id'], 'charge', 'service');
																$ilance->accounting_fees->construct_final_value_fee(intval($bidid), $result['user_id'], $result_p['project_id'], 'charge', 'service');
                                                                
                                                                
                                                                $ilance->email->mail = array($project_user_email, $winner_user_email, SITE_EMAIL);
        ...
        * @return      nothing
        */
        function service_auction_award_decline($bidid = 0, $userid = 0)
        {
                ...
                if ($ilance->db->num_rows($sel) > 0)
                {
                        ...
                        while ($result = $ilance->db->fetch_array($sel))
                        {
                                ...
                                if ($ilance->db->num_rows($sel_project_email) > 0)
                                {
                                        $result_email = $ilance->db->fetch_array($sel_project_email);
                                        ...
                                        if ($ilance->db->num_rows($sel_project_info) > 0)
                                        {
                                                while ($result_p = $ilance->db->fetch_array($sel_project_info))
                                                {
                                                        ...
                                                        $ilance->email->get('provider_declined_award_admin');		
                                                        $ilance->email->set($existing);
        ...
        * @return      nothing
        */
        function service_auction_bid_decline($bidid = 0, $userid = 0, $slng = 'eng')
        {
                global $ilance, $ilconfig, $phrase, $page_title, $area_title, $ilconfig, $ilpage;
                ...
                if ($ilance->db->num_rows($bidsql) > 0)
                {
                        ...
                        $cid = fetch_auction('cid', $bidres['project_id']);
                        ...
                        if ($ilance->categories->bidgrouping($cid))
                        {
