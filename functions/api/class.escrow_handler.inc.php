        ...
        */
		...
		require_once(DIR_CORE . 'functions_shipping.php');
			...
			// escrow will not be cancelled IF admin option $ilconfig['escrowsystem_payercancancelfundsafterrelease'] == false ...
					...
					$new_project_status = ($ilance->db->num_rows($sql_rfp) > 0)
						? 'expired'
						: 'open';
					...
					if ($escrowfinished)
					{
						$canremove = (($ilconfig['escrowsystem_payercancancelfundsafterrelease'] == 0)
							? false
							: true);
					}
					...
					if ($canremove)
					{
						if ($resultescrow['escrowamount'] > 0)
						{
							...
							insert_income_spent($resultescrow['project_user_id'], sprintf("%01.2f", $resultescrow['escrowamount']), 'debit');
							...
							$ilance->accounting->insert_transaction(
								...
								0,
								0,
								$phrase['_escrow_refund_credit'] . ': ' . stripslashes($ilance->db->fetch_field(DB_PREFIX . "invoices", "invoiceid = '" . $resultescrow['invoiceid'] . "'", "description")),
					...
					$currencyid = fetch_auction('currencyid', $res_escrow['project_id']);
                                        ...
                                        if ($ilance->db->num_rows($sql_account) > 0)
                                        {
                                                ...
                                                if ($res_escrow['escrowamount'] > 0)
                                                {
                                                        ...
                                                        // #### track income spent
                                                        insert_income_spent($res_escrow['project_user_id'], sprintf("%01.2f", $res_escrow['escrowamount']), 'debit');
					...
					$currencyid = fetch_auction('currencyid', $res_escrow['project_id']);
                                        ...
                                        if ($ilance->db->num_rows($sql_account2) > 0)
                                        {
                                                ...
                                                if ($escrow['accountamount'] > 0)
                                                {
                                                        ...
                                                        // #### track income spent
                                                        insert_income_reported($res_escrow['user_id'], sprintf("%01.2f", $escrow['accountamount']), 'debit');
                                                        ...
                                                        $ilance->accounting->insert_transaction(
                                                                ...
                                                                0,
                                                                0,
                                                                $phrase['_refund_from_online_account'] . ' #' . $res_escrow['project_id'] . ': ' . fetch_auction('project_title', $res_escrow['project_id']) . ': ' . $phrase['_back_to'] . ': ' . fetch_user('username', $res_escrow['project_user_id']),
                                                                ...
                                                                DATEINVOICEDUE,
                                                                DATETIME24H,
                                                                $phrase['_refund_forced_by_administration_please_note_this_refund_would_have_included_any_fees_you_paid_during_the_escrow_process'],
                                                                0,
                                                                0,
                                                                0
                                                        );
                                                ...
                                                if ($ilance->db->num_rows($sql_account) > 0)
                                                {
                                                        ...
                                                        if ($res_escrow['bidamount'] > 0)
                                                        {
                                                                ...
                                                                // #### track income spent
                                                                insert_income_spent($res_escrow['project_user_id'], sprintf("%01.2f", $res_escrow['bidamount']), 'debit');
                                                                ...
                                                                $ilance->accounting->insert_transaction(
                                                                        ...
                                                                        0,
                                                                        0,
                                                                        $phrase['_refund_to_online_account'] . ' #' . $res_escrow['project_id'] . ': ' . fetch_auction('project_title', $res_escrow['project_id']) . ': ' . $phrase['_from'] . ': ' . fetch_user('username', $res_escrow['user_id']),
			...
			// #### buyer releases funds to service provider ########
					...
					$currencyid = fetch_auction('currencyid', $res['project_id']);
                                        ...
                                        $escrow['commissionfee'] = ($res['fee2'] > 0)
						? sprintf("%01.2f", $res['fee2'])
						: 0;
						...
						if ($escrow['commissionfee'] <= $avail_balance AND $autopayment)
						{
							...
							// to the service providers online account balance
							...
							$fee2invoiceid = $ilance->accounting->insert_transaction(
								...
								0,
								0,
								$phrase['_commission_fee_for_escrow_deposit'] . ' #' . $res['project_id'] . ': ' . $phrase['_from'] . ' ' . fetch_user('username', $res['project_user_id']),
							...
							insert_income_spent($res['user_id'], sprintf("%01.2f", $escrow['commissionfee']), 'credit');
							$isfee2paid = 1;
						...
						{
							// create unpaid escrow commission fee based on the release of this escrow payment
							$fee2invoiceid = $ilance->accounting->insert_transaction(
								0,
								$res['project_id'],
								0,
						...
						");
                                        ...
                                        if ($escrow['totalamount'] > 0)
                                        {
                                                $ilance->accounting->insert_transaction(
                                                        ...
                                                        0,
                                                        0,
                                                        $phrase['_escrow_payment_release_for'] . ' #' . $res['project_id'] . ': ' . fetch_auction('project_title', $res['project_id']) . ': ' . $phrase['_from'] . ': ' . fetch_user('username', $res['project_user_id']),
                                                ...
                                                insert_income_reported($res['user_id'], sprintf("%01.2f", $escrow['totalamount']), 'credit');
					...
					if ($silentmode)
					{
						return false;
                                        ...
                                        $area_title = $phrase['_rfp_escrow_management'] . ' - ' . $phrase['_release_of_funds_error'];
                                        $page_title = SITE_NAME . ' - ' . $phrase['_rfp_escrow_management'] . ' - ' . $phrase['_release_of_funds_error'];
                                        
                                        if (isset($adminmode) AND $adminmode)
                                        {
                                                print_action_failed($phrase['_were_sorry_there_was_a_problem_releasing_the_funds_from_within_this_escrow_account_to_your_service_provider'], $ilpage['accounting'] . '?cmd=escrows');
                                                exit();
                                        }
					
                                        print_notice($area_title, $phrase['_were_sorry_there_was_a_problem_releasing_the_funds_from_within_this_escrow_account_to_your_service_provider'], HTTPS_SERVER . $ilpage['escrow'] . '?cmd=management&amp;sub=rfp-escrow', $phrase['_return_to_the_previous_menu']);
                                        exit();
                                }
                        }        
                }
                else if ($type == 'product' AND $escrowid > 0)
                {
					...
					$currencyid = fetch_auction('currencyid', $res_escrow['project_id']);
                                        ...
                                        if ($ilance->db->num_rows($sql_account) > 0)
                                        {
                                                ...
                                                insert_income_spent($res_escrow['user_id'], sprintf("%01.2f", $res_escrow['escrowamount']), 'debit');
                                                
                                                // #### adjust seller total amount received from this bidder > debit the spendings for this refund
                                                //insert_income_reported($res_escrow['project_user_id'], sprintf("%01.2f", $res_escrow['escrowamount']), 'debit');
                                                ...
                                                $ilance->accounting = construct_object('api.accounting');
                                                        ...
                                                        0,
                                                        0,
                                                        $phrase['_product_escrow_delivery_cancellation'] . ': ' . $phrase['_item_id'] . ' #' . $res_escrow['project_id'] . ': ' . $phrase['_refund_from_escrow_to_online_account'],
                                                        ...
                                                        DATETIME24H,
                                                        DATETIME24H,
                                                        $phrase['_merchant_cancelled_delivery_on'] . ' ' . DATETIME24H,
                                                        0,
                                                        0,
                                                        0
                                                );
						...
						$ilance->db->query("
							...
							paid = '0.00',
							paiddate = '0000-00-00 00:00:00',
							custommessage = '" . $ilance->db->escape_string($phrase['_merchant_cancelled_delivery_on'] . ' ' . DATETIME24H) . "'
							WHERE invoiceid = '" . $res_escrow['invoiceid'] . "'
						", 0, null, __FILE__, __LINE__);
						...
						", 0, null, __FILE__, __LINE__);
                                                ...
                                                mark_listing_as_unshipped($res_escrow['project_id'], $escrowid, $res_escrow['project_user_id'], $res_escrow['user_id'], 'escrow');
					...
					$currencyid = fetch_auction('currencyid', $res_escrow['project_id']);
							...
							$ilance->accounting = construct_object('api.accounting');
								...
								DATETIME24H,
								DATETIME24H,
								$phrase['_buyer_cancelled_escrow_and_returned_funds_back_to_account_balance_on'] . ' ' . DATETIME24H,
								0,
								0,
								0
							);
							...
							$ilance->db->query("
								...
								paid = '0.00',
								paiddate = '0000-00-00 00:00:00',
								custommessage = '" . $ilance->db->escape_string($phrase['_buyer_cancelled_escrow_and_returned_funds_back_to_account_balance_on'] . ' ' . DATETIME24H) . "'
								WHERE invoiceid = '" . $res_escrow['invoiceid'] . "'
							", 0, null, __FILE__, __LINE__);
                                                }
						...
						", 0, null, __FILE__, __LINE__);
                                                ...
                                                $ilance->email->mail = fetch_user('email', $res_escrow['user_id']);
                                                $ilance->email->slng = fetch_user_slng($res_escrow['user_id']);
                                                $ilance->email->get('product_escrow_release_cancelled_bidder');		
                                                $ilance->email->set($existing);
                                                $ilance->email->send();
						...
						$ilance->email->mail = fetch_user('email', $res_escrow['project_user_id']);
                                                $ilance->email->slng = fetch_user_slng($res_escrow['project_user_id']);
                                                $ilance->email->get('product_escrow_release_cancelled_merchant');		
                                                $ilance->email->set($existing);
                                                $ilance->email->send();
					...
					$currencyid = fetch_auction('currencyid', $res_escrow['project_id']);
                                        ...
                                        if ($ilance->db->num_rows($sql_account2) > 0)
                                        {
                                                ...
                                                if ($escrow['accountamount'] > 0)
                                                {
                                                        ...
                                                        // #### track income spent
                                                        insert_income_reported($res_escrow['project_user_id'], sprintf("%01.2f", $escrow['accountamount']), 'debit');
                                                        ...
                                                        $ilance->accounting->insert_transaction(
                                                                ...
                                                                0,
                                                                0,
                                                                $phrase['_refund_from_online_account'] . ' ' . $phrase['_item_id'] . ' #' . $res_escrow['project_id'] . ': ' . fetch_auction('project_title', $res_escrow['project_id']) . ': ' . $phrase['_back_to'] . ': ' . fetch_user('username', $res_escrow['user_id']),
                                                                sprintf("%01.2f", $escrow['accountamount']),
                                                                sprintf("%01.2f", $escrow['accountamount']),
                                                                'paid',
                                                                'debit',
                                                ...
                                                if ($ilance->db->num_rows($sql_account) > 0)
                                                {
                                                        ...
                                                        if ($res_escrow['bidamount'] > 0)
                                                        {
                                                                ...
                                                                // #### track income spent
                                                                insert_income_spent($res_escrow['user_id'], sprintf("%01.2f", $res_escrow['bidamount']), 'debit');
                                                                ...
                                                                $ilance->accounting->insert_transaction(
                                                                        ...
                                                                        0,
                                                                        0,
                                                                        $phrase['_refund_to_online_account'] . ' ' . $phrase['_item_id'] . ' #' . $res_escrow['project_id'] . ': ' . fetch_auction('project_title', $res_escrow['project_id']) . ': ' . $phrase['_from'] . ': ' . fetch_user('username', $res_escrow['project_user_id']),
                                                                        sprintf("%01.2f", $res_escrow['bidamount']),
                                                                        sprintf("%01.2f", $res_escrow['bidamount']),
                                                                        'paid',
                                                                        'credit',
					...
					$currencyid = fetch_auction('currencyid', $res_escrow['project_id']);
                                        ...
                                        mark_listing_as_shipped($res_escrow['project_id'], $escrowid, $res_escrow['project_user_id'], $res_escrow['user_id'], 'escrow');
                                        ...
                                        $ilance->email->mail = SITE_EMAIL;
                                        $ilance->email->slng = fetch_site_slng();
                                        $ilance->email->get('product_escrow_delivery_confirmed_admin');		
                                        $ilance->email->set($existing);
                                        $ilance->email->send();
					...
					$currencyid = fetch_auction('currencyid', $res_escrow['project_id']);
                                        ...
                                        if ($ilance->db->num_rows($sql_account) > 0)
                                        {
                                                ...
                                                insert_income_reported($res_escrow['project_user_id'], sprintf("%01.2f", $res_escrow['total']), 'credit');
                                                ...
                                                $new_escrowcredit_id = $ilance->accounting->insert_transaction(
                                                        ...
                                                        0,
                                                        0,
                                                        $phrase['_escrow_payment_release_for_rfp'] . ' #' . $res_escrow['project_id'] . ': ' . fetch_auction('project_title', $res_escrow['project_id']) . ': ' . $phrase['_from_bidder'] . ': ' . fetch_user('username', $res_escrow['user_id']),
                                                ...
                                                if ($res_escrow['fee'] > 0 AND $res_escrow['isfeepaid'] == 0)
                                                {
                                                        ...
                                                        if ($ilance->db->num_rows($sql_account2) > 0)
                                                        {
                                                                ...
                                                                if ($avail_balance >= $res_escrow['fee'] AND $res_account2['autopayment'])
                                                                {
                                                                        ...
                                                                        insert_income_spent($res_escrow['project_user_id'], sprintf("%01.2f", $res_escrow['fee']), 'credit');
                                
                                                                        // #### create new paid invoice to seller showing escrow commission fee debit
                                                                        $feeinvoiceid = $ilance->accounting->insert_transaction(
                                                                                0,
                                                                                $res_escrow['project_id'],
                                                                                0,
                                                                                $res_escrow['project_user_id'],
                                                                                ...
                                                                                0,
                                                                                0,
                                                                                $phrase['_commission_fee_for_rfp'] . ' #' . $res_escrow['project_id'] . ': ' . fetch_auction('project_title', $res_escrow['project_id']) . ': ' . $phrase['_from_bidder'] . ': ' . fetch_user('username', $res_escrow['user_id']),
                                                                                sprintf("%01.2f", $res_escrow['fee']),
                                                                                sprintf("%01.2f", $res_escrow['fee']),
                                                                                'paid',
                                                                                'commission',
                                                                                'account',
                ...
                else if ($type == 'buynow' AND $escrowid > 0)
                {
					...
					$currencyid = fetch_auction('currencyid', $res_escrow['project_id']);
                                        ...
                                        mark_listing_as_shipped($res_escrow['project_id'], $res_escrow['orderid'], $res_escrow['owner_id'], $res_escrow['buyer_id'], 'buynow');
                                        ...
                                        // #### we should also check if any fees are pending, and remind everyone including admin!
                                        return true;
                                }
					...
					$winnermarkedaspaidmethod = print_fixed_payment_method($res['buyerpaymethod'], false);
						...
						winnermarkedaspaidmethod = '" . $ilance->db->escape_string($winnermarkedaspaidmethod) . "'
                                ...
                                for ($i=0; $i<$length; $i++)
                                {
                                    ...
                                    $char2 = $char1;
										...
										//########## Set paiddate and status for invoice ###########
										 $ilance->db->query("
                                                UPDATE " . DB_PREFIX . "invoices
                                                SET status = 'paid',
						                         paiddate = '" . DATETIME24H . "',
                                                ...
                                                WHERE invoiceid='" . intval($escrowid) . "'
                                       ");
                                        // Herakle End 
										
                                        mark_listing_as_shipped($res['project_id'], $escrowid, $res['owner_id'], $res['buyer_id'], 'buynow');
					...
					$currencyid = fetch_auction('currencyid', $res_escrow['project_id']);
                                        ...
                                        if ($ilance->db->num_rows($sql_account) > 0)
                                        {
                                                ...
                                                // #### calculate new balances for seller
							...
							available_balance = '" . $new_avail_balance . "'
                                                ...
                                                insert_income_reported($res_escrow['owner_id'], sprintf("%01.2f", $res_escrow['amount']), 'credit');
                                    
                                                // #### adjust buyers total amount spent to sellers
                                                insert_income_spent($res_escrow['buyer_id'], sprintf("%01.2f", $res_escrow['amount']), 'credit');
                                                ...
                                                $ilance->accounting->insert_transaction(
                                                        ...
                                                        0,
                                                        0,
                                                        $phrase['_escrow_payment_release_for_purchase_now_item'] . ' ' . $res_escrow['project_id'] . ": " . fetch_auction('project_title', $res_escrow['project_id']) . ": " . $phrase['_from_bidder'] . ": " . fetch_user('username', $res_escrow['buyer_id']),
                                                        ...
                                                        DATEINVOICEDUE,
                                                        DATETIME24H,
                                                        $phrase['_purchase_now_escrow_credit_to_online_account_balance'] . ' - ' . DATETIME24H,
                                                        0,
                                                        0,
                                                        0
                                                );
								...
								// #### track income spending
                                                                insert_income_spent($res_escrow['owner_id'], sprintf("%01.2f", $escrowfeeamount), 'credit');
                                                        }
                                                }
								...
								// #### track income spending
                                                                insert_income_spent($res_escrow['owner_id'], sprintf("%01.2f", $fvfamount), 'credit');
                                                        }
                                                }
					...
					$currencyid = fetch_auction('currencyid', $res_escrow['project_id']);
                                        ...
                                        if ($ilance->db->num_rows($sql_account) > 0)
                                        {
                                                ...
                                                $ilance->accounting->insert_transaction(
                                                        ...
                                                        0,
                                                        0,
                                                        $phrase['_purchase_now_delivery_cancellation'] . ' - ' . $phrase['_order_id'] . ' #' . $res_escrow['orderid'] . ': ' . $phrase['_refund_from_escrow_to_online_account'],
                                                        ...
                                                        DATEINVOICEDUE,
                                                        DATETIME24H,
                                                        $phrase['_merchant_cancelled_purchase_now_delivery_on'] . ' ' . DATETIME24H,
                                                        0,
                                                        0,
                                                        0
                                                );
                                                ...
                                                insert_income_spent($res_escrow['buyer_id'], sprintf("%01.2f", $escrowamount), 'debit');
                                                ...
                                                if ($res_escrow['escrowfeebuyer'] > 0 AND $res_escrow['escrowfeebuyerinvoiceid'] > 0)
                                                {
                                                        ...
                                                        if ($res_escrow['isescrowfeebuyerpaid'])
                                                        {
                                                                ...
                                                                if ($ilance->db->num_rows($sql_account) > 0)
                                                                {
                                                                        ...
                                                                        // #### adjust bidders total amount spent to this merchant > debit the spendings for this refund
                                                                        insert_income_spent($res_escrow['buyer_id'], sprintf("%01.2f", $escrowfeebuyeramount), 'debit');
                                                                        ...
                                                                        $ilance->accounting->insert_transaction(
                                                                                ...
                                                                                0,
                                                                                0,
                                                                                $phrase['_purchase_now_delivery_cancellation'] . " - " . $phrase['_order_id'] . " #" . $res_escrow['orderid'] . ": " . $phrase['_purchase_now_buyer_escrow_fee_refund'],
                                                                                ...
                                                                                DATEINVOICEDUE,
                                                                                DATETIME24H,
                                                                                $phrase['_purchase_now_escrow_fee_refund_completed_on'] . " " . DATETIME24H,
                                                                                0,
                                                                                0,
                                                                                0
                                                                        );
                                                        ...
                                                        else
                                                        {
                                                                ...
                                                                $ilance->db->query("
                                                                        ...
                                                                        custommessage = '" . $ilance->db->escape_string($phrase['_fee_was_cancelled_due_to_buy_now_order_cancellation']) . "'
                                                ...
                                                if ($res_escrow['escrowfee'] > 0 AND $res_escrow['escrowfeeinvoiceid'] > 0)
                                                {
                                                        ...
                                                        if ($res_escrow['isescrowfeepaid'])
                                                        {
                                                                ...
                                                                if ($ilance->db->num_rows($sql_account) > 0)
                                                                {
                                                                        ...
                                                                        // #### adjust bidders total amount spent to this merchant > debit the spendings for this refund
                                                                        insert_income_spent($res_escrow['owner_id'], sprintf("%01.2f", $escrowfeeselleramount), 'debit');
                                                                        ...
                                                                        $ilance->accounting->insert_transaction(
                                                                                ...
                                                                                0,
                                                                                0,
                                                                                $phrase['_purchase_now_delivery_cancellation'] . " - " . $phrase['_order_id'] . " #" . $res_escrow['orderid'] . ": " . $phrase['_purchase_now_seller_escrow_fee_refund'],
                                                ...
                                                if ($res_escrow['fvf'] > 0 AND $res_escrow['fvfinvoiceid'] > 0)
                                                {
                                                        ...
                                                        if ($res_escrow['isfvfpaid'])
                                                        {
                                                                ...
                                                                if ($ilance->db->num_rows($sql_account) > 0)
                                                                {
                                                                        ...
                                                                        // #### adjust bidders total amount spent to this merchant > debit the spendings for this refund
                                                                        insert_income_spent($res_escrow['owner_id'], sprintf("%01.2f", $fvfselleramount), 'debit');
                                                                        ...
                                                                        $ilance->accounting->insert_transaction(
                                                                                ...
                                                                                0,
                                                                                0,
                                                                                $phrase['_purchase_now_delivery_cancellation'] . " - " . $phrase['_order_id'] . " #" . $res_escrow['orderid'] . ": Purchase Now Seller Final Value Fee Refund",
                                                                                ...
                                                                                DATEINVOICEDUE,
                                                                                DATETIME24H,
                                                                                $phrase['_purchase_now_final_value_fee_refund_completed_on'] . " " . DATETIME24H,
                                                                                0,
                                                                                0,
                                                                                0
                                                                        );
                                                ...
                                                // #### mark as unshipped (listing table)
                                                mark_listing_as_unshipped($res_escrow['project_id'], $escrowid, $res_escrow['owner_id'], $res_escrow['buyer_id'], 'buynow');
