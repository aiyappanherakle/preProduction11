...
class xml
{
	...
	function construct_xml_array($encoding = 'UTF-8', $emptyafter = true, $xmlfile)
	{
        ...
        */
        ...
        {
                $ilversion = $langcode = $charset = $author = $emailname = $emailsubject = $emailbody = $emailtype = $emailvarname = $emailproduct = $emailcansend = $emaildepartmentid = '';                
                $emailarray = array();
                $arraycount = count($a);
                ...
                for ($i = 0; $i < $arraycount; $i++)
                {
                        ...
                        if (!empty($emailvarname) AND !empty($emailname) AND !empty($emailsubject) AND !empty($emailbody) AND !empty($emailtype) AND !empty($emailproduct) AND !empty($emailcansend) AND !empty($emaildepartmentid))
                        {
                                ...
                                $emailname = $emailsubject = $emailbody = $emailtype = $emailvarname = $emailproduct = $emailcansend = $emaildepartmentid = '';
                        }
                }
        ...
        function process_addon_xml($a = array(), $e)
	{
                ...
                $emailname = $emailsubject = $emailbody = $emailtype = $emailvarname = $csselement = $elementdescription = $csstype = $cssstatus = $cssauthor = $styleids = $csscontent = $csssort = '';
                ...
                for ($i = 0; $i < $count; $i++)
                {
                        ...
                        if (!empty($emailvarname) AND !empty($emailname) AND !empty($emailsubject) AND !empty($emailtype) AND !empty($emailbody))
                        {
                                ...
                                // reset for next email
                                $emailname = $emailsubject = $emailbody = $emailtype = $emailvarname = '';
                        }
			...
			else if ($a[$i]['tag'] == 'CSSSORT')
			{
                ...
                $product[] = array($version, $versioncheckurl, $url, $minversion, $maxversion);
                ...
                if (!empty($array) AND is_array($array))
                {
                        ...
                        foreach ($array as $key => $value)
                        {
                                ...
                                if (isset($value) AND !empty($value) AND is_array($value))
                                {
                                        ...
                                        {
                                                //$xml .= "\t\t<$field><![CDATA[" . htmlentities($data, ENT_QUOTES) . "]]></$field>" . LINEBREAK;
