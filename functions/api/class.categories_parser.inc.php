...
class categories_parser extends categories {
	...
	function print_subcategory_columns_coin_series($denomination_id) {
		...
		$sql1="select s.coin_series_unique_no,s.coin_series_name, COUNT( p.project_id ) AS sold_count from " . DB_PREFIX . "catalog_second_level s
		left join " . DB_PREFIX . "projects p on p.coin_series_unique_no=s.coin_series_unique_no  AND (p.haswinner = '1' OR p.hasbuynowwinner = '1') 
		where s.coin_series_denomination_no='".$denomination_id."' group by s.coin_series_unique_no order by s.coin_series_sort";
		//"SELECT COUNT( p.project_id ) AS sold_coun FROM  WHERE p.visible = '1' AND (p.status != 'open') AND p.coin_series_unique_no =  '".$coin_series['coin_series_unique_no']."'
		...
		if($ilance->db->num_rows($result1)>0)
		{
			...
			$check = 0;
			while($coin_series= $ilance->db->fetch_array($result1))
			{
				if ($check % 4 == 0) { $html .= '</tr><tr>'; }$check++;
				if ($ilconfig['globalauctionsettings_seourls']) 
				{
					$url=HTTP_SERVER . 'CoinPrices/SeriesCoin/' . $coin_series['coin_series_unique_no'] . '/' . construct_seo_url_name($coin_series['coin_series_name']);
				}else
				{	
					$url=HTTP_SERVER . 'search.php?mode=product&series=' . $coin_series['coin_series_unique_no'];
				}
				$html .= '<td><a  href="' . $url . '" >' . $coin_series['coin_series_name'] . '&nbsp;(' . $coin_series['sold_count'] . ')' . '</a></td>';
...
//karthik on may30
	function print_subcategory_columns_denominations_price($columns = 1, $cattype = 'product', $dosubcats = 1, $slng = 'eng', $cid = 0, $extra = '', $showcount = 1, $iscatmap = 0, $parentstyle = '', $childstyle = '', $subcatdepth = 0, $cacheid = '') {
		...
		$sql1="select id,denomination_long,case when p.auction_count is null then 0 else p.auction_count end as auction_count  from " . DB_PREFIX . "catalog_toplevel d left join (select count(project_id) as auction_count,coin_series_denomination_no from " . DB_PREFIX . "projects p force index (archive_coins) where p.haswinner=1 AND p.status='expired' group by coin_series_denomination_no ) p on p.coin_series_denomination_no =d.denomination_unique_no order by denomination_sort";
		$result1 = $ilance->db->query($sql1, 0, null, __FILE__, __LINE__);
		if($ilance->db->num_rows($result1)>0)
		{
			$check = 0;
			while($denom_detail= $ilance->db->fetch_array($result1))
			{
				if ($check % $columns == 0) {
				$html .= '</tr><tr>';
				}
...
//		exit();
	...
	function fetch_coin_class($series_id = 0, $coin_class_id = 0, $pcgs = 0, $columns = ' * ') {
		...
		if ($coin_class_id != 0) 
		{
			...
			{
				$sub_query = ' id=' . $coin_class_id;
			}
		}

		if($series_id > 0)
		{
			$sub_querys = " where coin_series_unique_no='" . $series_id . "' " . $sub_query . "";
		} 
		else if ($series_id == 0)
		{
		    $sub_querys = " where" . $sub_query . "";
		}
		else
		{
			$sub_querys = "";
		...
		$result = $ilance->db->query("select " . $columns . " from " . DB_PREFIX . "catalog_coin" . $sub_querys . " order by coin_detail_sort", 0, null, __FILE__, __LINE__);
