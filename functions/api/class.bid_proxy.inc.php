        ...
        */
        function do_proxy_bidder($projectid = 0, $bidderid = 0, $ownerid = 0, $skipbidder, $skipbidderid)
        {
                ...
                if ($ilance->db->num_rows($sql) > 0)
                {
                        ...
                        $bidtotal = $ilance->subscription->check_access($bidderid, 'bidlimitperday');
                        ...
                        while ($bids = $ilance->db->fetch_array($sql, DB_ASSOC))
                        {
                                ...
                                if ($this->fetch_highest_bidder(intval($projectid)) != $bids['user_id'] AND $bidsleft > 0)
                                {
                                        ...
                                        if ($bids['maxamount'] >= $min_bidamount)
                                        {
                                                ...
                                                $ilance->db->query("
                                                        ...
                                                        '" . intval($projectid) . "',
                                                        '" . intval($ownerid) . "',
                                                        '" . $ilance->db->escape_string($phrase['_bid_placed_via_proxy_agent']) . "',
										...
										if($highestbid>$bids[maxamount] and $min_bidamount>=$highestbid)
										{
									$bid_check=$ilance->db->query("select bid_id from " . DB_PREFIX . "project_bids where user_id=".$bids['user_id']." and project_id=".intval($projectid)." and bidamount=". sprintf("%01.2f", $bids['maxamount']) ."");
										if($ilance->db->num_rows($bid_check)==0)
										{
                                ...
                                else
                                {
                                        if ($bidsleft <= 0)
                                        {
                                                DEBUG("PROXY: bidder ID #$bidderid cannot place any more bids today (bidlimit) subscription restriction", 'NOTICE');
                                        }
                                        else
                                        {
                                                DEBUG("PROXY: Nothing to do", 'NOTICE');
