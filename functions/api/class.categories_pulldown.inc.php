	...
	function fetch_levelprofile_options($selected = '', $result = array(), $type = '', $cattype = '', $forcenocount, $dojs, $uid)
        {
                ...
                for ($i = 0; $i < $count; $i++)
                {
                        if ($result[$i]['visible'])
                        {
                                $questioncount = $this->fetch_profile_question_count($result[$i]['cid']);
                                if (isset($selected) AND $selected == $result[$i]['cid'])
                                {
                                        if ($questioncount > 0)
                                        {
                                                if ($dojs == 0)
                                                {
                                                        $html .= '<option value="' . $result[$i]['cid'] . '" selected="selected">' . (($result[$i]['level'] > 1) ? str_repeat(' &nbsp; ', $result[$i]['level']) : '' ) . stripslashes($result[$i]['title']) . ' (' . $questioncount . ' ' . mb_strtolower($phrase['_questions']) . ', ' . $this->fetch_profile_answer_count($result[$i]['cid'], $uid).' '.mb_strtolower($phrase['_answered']).', '.$this->fetch_profile_verification_count($result[$i]['cid'], $uid).' '.mb_strtolower($phrase['_verified']).')</option>';
                                                }
                                                else 
                                                {
                                                        $html .= '<option value="' . HTTP_SERVER . $ilpage['selling'] . '?cmd=profile-specifics&amp;cid=' . $result[$i]['cid'] . '" selected="selected">' . (($result[$i]['level'] > 1) ? str_repeat(' &nbsp; ', $result[$i]['level']) : '' ) . stripslashes($result[$i]['title']) . ' (' . $questioncount . ' ' . mb_strtolower($phrase['_questions']).', '.$this->fetch_profile_answer_count($result[$i]['cid'], $uid).' '.mb_strtolower($phrase['_answered']).', '.$this->fetch_profile_verification_count($result[$i]['cid'], $uid).' '.mb_strtolower($phrase['_verified']).')</option>';
                                                }
                                        }
                                }
                                else 
                                {
                                        if ($questioncount > 0)
                                        {
                                                if ($dojs == 0)
                                                {
                                                        $html .= '<option value="' . $result[$i]['cid'] . '">' . (($result[$i]['level'] > 1) ? str_repeat(' &nbsp; ', $result[$i]['level']) : '' ) . stripslashes($result[$i]['title']) . ' (' . $questioncount . ' ' . mb_strtolower($phrase['_questions']).', '.$this->fetch_profile_answer_count($result[$i]['cid'], $uid).' '.mb_strtolower($phrase['_answered']).', '.$this->fetch_profile_verification_count($result[$i]['cid'], $uid).' '.mb_strtolower($phrase['_verified']).')</option>';
                                                }
                                                else 
                                                {
                                                        $html .= '<option value="' . HTTP_SERVER . $ilpage['selling'] . '?cmd=profile-specifics&amp;cid=' . $result[$i]['cid'] . '">' . (($result[$i]['level'] > 1) ? str_repeat(' &nbsp; ', $result[$i]['level']) : '' ) . stripslashes($result[$i]['title']) . ' (' . $questioncount . ' ' . mb_strtolower($phrase['_questions']).', '.$this->fetch_profile_answer_count($result[$i]['cid'], $uid).' '.mb_strtolower($phrase['_answered']).', '.$this->fetch_profile_verification_count($result[$i]['cid'], $uid).' '.mb_strtolower($phrase['_verified']).')</option>';						
                                                }
                                        }
                                }
                        }
        ...
        */
        function print_root_category_pulldown($selected = '', $cattype = '', $fieldname = 'cid', $slng = 'eng', $categorycache = array(), $showselectall = true, $showcanassigntoall = false)
        {
                global $ilance, $ilconfig, $phrase, $ilpage;
		...
		if ($showcanassigntoall AND defined('LOCATION') AND LOCATION == 'admin')
                {
                        $html .= '<option value="-1" style="background:yellow; color:#000">' . $phrase['_assign_to_all_available_categories'] . '</option>';
			...
			if ($showselectall)
			{
				$html .= '<option value="">&#9492; ' . $phrase['_all_categories_upper'] . '</option>';
			}
			$html .= '<option value="">----------------------------------</option>';
                }
                else
			...
			$html .= '<option value="">----------------------------------</option>';
                ...
                for ($i = 0; $i < $count; $i++)
                {
                        ...
                        {
                                $html .= '<option value="' . $this->cats[$i]['cid'] . '">' . $this->cats[$i]['title'] . '</option>';
                        }
                }
        ...
        */
        ...
        {
                global $ilance, $myapi, $phrase, $ilconfig, $ilpage, $headinclude, $show;
                ...
                if ($mode == 0)
                {
                        ...
                        for ($i = 0; $i < count($results); $i++)
                        {
                                ...
                                if ($expertspulldown)
                                {
                                        ...
                                        {
                                                while ($exclude = $ilance->db->fetch_array($count))
                                                {
                                                        // for each user opt'ed, check if they can be listed in search results..
                                                        if ($ilance->subscription->check_access($exclude['user_id'], 'searchresults') == 'yes' AND display_profile($exclude['user_id']))
                                                        {
                                                                $counter++;
                                                        }                                                        
                                                }
                ...
                if ($dojs)
                {
                        ...
                        $html = '<select name="' . $fieldname . '" onchange="openURL();" style="font-family: verdana">';
                        
                        // specific javascript include
                        $headinclude .= "
...
//-->
                ...
                if ($type == 'levelmulti' OR $type == 'levelmultisearch')
                {
                        ...
                        // allowing multiple selections of categories within the select menu
                        $html = '<select ' . $divid . ' name="' . $fieldname . (($type == 'levelmulti') ? '[]" multiple' : '"') . ' size="10" style="width:' . $width . '; font-family: verdana" ' . $onclickjs . '>';	
                }
                
                if ($showpleaseselectoption)
                {
                        // showing "please select" or "best matching" as default pulldown menu options
                        if ($showbestmatching)
                        {
                                $phr = $phrase['_best_matching'];
                        }
                        else
                        {
                                if ($cattype == 'skills')
                ...
                if ($canassigntoall AND defined('LOCATION') AND LOCATION == 'admin')
                {
                        ...
                        $html .= '<optgroup label="' . $phrase['_all_categories'] . '">';
                        $html .= '<option value="-1" style="background:#ebebeb">' . $phrase['_assign_to_all_available_categories'] . '</option>';
                        $html .= '</optgroup>';
                }
                ...
                else if ($type == 'levelmulti' OR $type == 'levelmultisearch')
                {
                                ...
                                switch ($prepopulate)
                                {
                                        case 'notifyservicescats':
                                        {
                                                $popsql = $ilance->db->query("
                                                        SELECT notifyservicescats
                                                        FROM " . DB_PREFIX . "users
                                                        WHERE user_id = '" . intval($userid) . "'
                                                ", 0, null, __FILE__, __LINE__);
                                                if ($ilance->db->num_rows($popsql) > 0)
                                                {
                                                        $cats = $ilance->db->fetch_array($popsql);
                                        ...
                                        case 'sellingprofile':
                                        {
                                                ...
                                                if ($ilance->db->num_rows($popsql) > 0)
                                                {
                                                        while ($cats = $ilance->db->fetch_array($popsql))
                                                        {
                        ...
                        if ($showbestmatching)
                        {
                                $html .= '<option value="0">' . $phrase['_best_matching'] . '</option>';
                        }
