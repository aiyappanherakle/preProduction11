...
//			echo '<br>'.$line1->coin_id.'[<b>'.$line1->ebay_id.'</b>]'.$line1->end_date.'&nbsp;&nbsp;&nbsp;&nbsp;';
			...
			if(!$responce)
			{
				...
				//echo $this->html;
				$fp=fopen('ebay_log_invoices.txt','a');
				fwrite($fp,$query1."\n");
				fwrite($fp,$line1->coin_id."coin id\n");
				fwrite($fp,$this->html."\n");
				//echo '<br>';
			}else
			{
...
{
 global $devID, $appID, $certID, $serverUrl, $userToken,$compatabilityLevel,$coin,$ebay,$ilance;
...
//for bug #5314 - get buynow offer price
//$sale->CurrentPrice 			= $ebay->CurrentPrice;	
$BestOfferSale = $this->nodeval($response,"BestOfferSale");
if(isset($BestOfferSale) && $BestOfferSale )
{
	$sale->CurrentPrice = $this->nodeval($response,"TransactionPrice");
}	
else
{
	$sale->CurrentPrice = $ebay->CurrentPrice;
...
$sale->ConvertedAmountPaid 		= $this->mysql_date($this->nodeval($response,"ConvertedAmountPaid "));
...
//and ebay_id='".$coin->ebay_id."'
	$query="SELECT master_invoice_id FROM " . DB_PREFIX . "ebay_listing_rows WHERE ( ".$sql_transaction_part." ExternalTransactionID='".$ExternalTransactionID."') and (ExternalTransactionID>0 or ebay_Transaction_id>0)";
	$result=$ilance->db->query($query);
	if($ilance->db->num_rows($result)>0)
	{
		while($line=$ilance->db->fetch_array($result))
		{
			return $line['master_invoice_id'];	
		}
	}else
	{
	return 0;
...
function create_invoice()
{
	...
	$flag_id=$this->transaction_not_accounted($sale->transaction_id,$sale->ExternalTransactionID);
	if($flag_id==0)
	{
	...
	{
	//there is already a transaction and combine them now
		$sale->invoiceid=$this->build_child_invoice(); 
		$fvf_invoice=$this->build_fvf_invoice();
		if($sale->invoiceid>0)
		{
			$sale->parent_invoice=$this->update_parent_invoice($sale->invoiceid,$flag_id);
		}
		$this->ebay_listing_row($flag_id,$sale->invoiceid);
		$this->update_ebay_listing();
	}
...
function build_parent_invoice($invoiceid)
{
	...
	}else if($sale->Payment_details=='unpaid')
	{
		...
		'" . intval($sale->buyer_id). "',                        
		'" . $ilance->db->escape_string($description). "',
		'" . $ilance->db->escape_string($sale->shippingcost+$sale->item_total ) . "',
		'0',
		'" . $ilance->db->escape_string($sale->shippingcost+$sale->item_total) . "',
		'1',
		'" .$ilance->db->escape_string($sale->salestax). "',
		...
		'scheduled',
		'escrow',
		'" .$ilance->db->escape_string('paypal'). "',
		'" . $ilance->db->escape_string(IPADDRESS) . "',
		'" . $ilance->db->escape_string($referer_Page) . "',
		'" . $ilance->db->escape_string($ebay->createdate) . "',
		'" . $ilance->db->escape_string(DATETIME24H) . "',  
		...
		$ilance->db->query($combined_invoice);
		$invoiceid = $ilance->db->insert_id();
		//$ilance->db->query("update " . DB_PREFIX . "invoices set status='completed' where invoiceid='".$invoiceid."'");
...
function update_parent_invoice($child_invoiceid,$parent_invoiceid)
{
global $ilance;
$query="update " . DB_PREFIX . "invoices set combine_project=concat(combine_project,',',".$child_invoiceid.")WHERE invoiceid = ".$parent_invoiceid;
$result=$ilance->db->query($query);

$query="SELECT totalamount,combine_project FROM " . DB_PREFIX . "invoices WHERE invoiceid = ".$parent_invoiceid;
	$result=$ilance->db->query($query);
	if($ilance->db->num_rows($result))
	{
		while($line=$ilance->db->fetch_array($result))
		...
		{
		$query2="SELECT * FROM " . DB_PREFIX . "invoices WHERE invoiceid = ".$child;
		...
		if($total+$shipping_cost!=$line['totalamount'])
		{
		echo $total;
		}
		}
...
function ebay_listing_row($parent_invoice,$invoiceid)
{
	...
	//update life time sold/bought details in user table for seller and buyer
	
	$query2="update " . DB_PREFIX . "users set productsold=productsold+".$ilance->db->escape_string($sale->QuantityPurchased)." 
	where user_id='".$coin->user_id."'";
	$sql2 = $ilance->db->query($query2, 0, null, __FILE__, __LINE__);
	
	$query3="update " . DB_PREFIX . "users set productawards=productawards+".$ilance->db->escape_string($sale->QuantityPurchased)." 
	where user_id='".$sale->buyer_id."'";
	$sql3 = $ilance->db->query($query3, 0, null, __FILE__, __LINE__);
}
