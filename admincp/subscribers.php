...
if (!empty($_SESSION['ilancedata']['user']['userid']) AND $_SESSION['ilancedata']['user']['userid'] > 0 AND $_SESSION['ilancedata']['user']['isadmin'] == '1')
{
        ...
        if (isset($ilance->GPC['cmd']) AND $ilance->GPC['cmd'] == 'nonprofits')
        {
                ...
                if (isset($ilance->GPC['subcmd']) AND $ilance->GPC['subcmd'] == 'remove-nonprofit' AND isset($ilance->GPC['id']) AND $ilance->GPC['id'] > 0)
                {
                        ...
                        print_action_success($phrase['_the_selected_nonprofit_was_removed_from_the_system'], $ilpage['subscribers'] . '?cmd=nonprofits');
                        exit();
                }
                ...
                if (isset($ilance->GPC['subcmd']) AND $ilance->GPC['subcmd'] == 'add-nonprofit' AND !empty($ilance->GPC['title']) AND !empty($ilance->GPC['description']) AND !empty($ilance->GPC['url']))
                {
                        ...
                        print_action_success($phrase['_the_selected_nonprofit_was_added_to_the_system_ready_to_receive_donations_from_sellers'], $ilpage['subscribers'] . '?cmd=nonprofits');
                        exit();
                }
                ...
                if (isset($ilance->GPC['subcmd']) AND $ilance->GPC['subcmd'] == 'update-nonprofit' AND !empty($ilance->GPC['id']) AND $ilance->GPC['id'] > 0 AND !empty($ilance->GPC['do']) AND $ilance->GPC['do'] == 'update')
                {
                        ...
                        print_action_success($phrase['_the_selected_nonprofit_was_updated'], $ilpage['subscribers'] . '?cmd=nonprofits');
                        exit();         
                }
        ...
        {
                $area_title = $phrase['_abuse_report_management'];
                $page_title = SITE_NAME . ' - ' . $phrase['_abuse_report_management'];
                ...
                if (isset($ilance->GPC['subcmd']) AND $ilance->GPC['subcmd'] == 'dismiss' AND isset($ilance->GPC['abuseid']) AND $ilance->GPC['abuseid'] > 0)
                {
                        ...
                        print_action_success($phrase['_the_selected_abuse_report_was_dismissed'], $ilpage['subscribers'] . '?cmd=abuse');
                        exit();
                }
                ...
                if ($ilance->db->num_rows($sql) > 0)
                {
                        ...
                        {
                                $res['action'] = '<a href="' . $ilpage['subscribers'] . '?cmd=abuse&amp;subcmd=dismiss&amp;abuseid=' . $res['abuseid'] . '">' . $phrase['_dismiss'] . '</a>';
                ...
                $pprint_array = array('buildversion','ilanceversion','login_include_admin','submit','subcmd','subcat_pulldown','question_inputtype_pulldown','questionid','cid','slng','categoryname','language_pulldown','slng','checked_question_cansearch','checked_question_active','checked_question_required','subcategory_pulldown','formdefault','multiplechoice','question','description','formname','sort','submit_category_question','question_id_hidden','question_subcmd','question_inputtype_pulldown','subcatid','subcatname','catname','service_subcategories','product_categories','subcmd','id','submit','description','name','checked_profile_group_active','input_style','remote_addr','rid','login_include','headinclude','area_title','page_title','lanceads_header','lanceads_footer');
                
                ($apihook = $ilance->api('admincp_abuse_reports_end')) ? eval($apihook) : false;
                
                $ilance->template->fetch('main', 'abuse_reports.html', 1);               
                $ilance->template->parse_hash('main', array('ilpage' => $ilpage));
                $ilance->template->parse_loop('main', array('v3nav','subnav_settings','listingabuse','biddingabuse','portfolioabuse','profilesabuse','feedbackabuse','pmbabuse'));
                $ilance->template->parse_if_blocks('main');
                $ilance->template->pprint('main', $pprint_array);
                exit();
        ...
        {
                $area_title = $phrase['_skills_management'];
                $page_title = SITE_NAME . ' - ' . $phrase['_skills_management'];
		...
		$counter = ($ilance->GPC['page'] - 1) * $ilance->GPC['pp'];
                ...
                $ilance->categories_skills->build_array_skills($_SESSION['ilancedata']['user']['slng']);
                ...
                if (isset($ilance->GPC['subcmd']) AND ($ilance->GPC['subcmd'] == 'editskill' OR $ilance->GPC['subcmd'] == 'addskill'))
                {
                        ...
                        if ($ilance->GPC['subcmd'] == 'editskill')
                        {
                                ...
                                // we are editing a particular skill
                                $submit = '<input type="submit" name="submit" value=" ' . $phrase['_save'] . ' " style="font-size:15px" class="buttons" />';
                                $subcmd = 'updateskill';
                                ...
                                if ($ilance->db->num_rows($sql) > 0)
                                {
                                        while ($res = $ilance->db->fetch_array($sql))
                                        {
                                                $res['keywords'] = '<input type="text" name="keywords" value="' . $ilance->categories_skills->keywords(fetch_site_slng(), $cid, $commaafter = false, $showinputkeywords = false) . '" style="font-family: verdana; width:98%" />';
                                                $servicecategory[] = $res;
                                        }
                                }
                        ...
                        else if ($ilance->GPC['subcmd'] == 'addskill')
                        {
                                ...
                                $cid = intval($ilance->GPC['cid']);
                                $categorycache = $ilance->categories_skills->build_array_skills($_SESSION['ilancedata']['user']['slng']);
                                $subcat_pulldown = $ilance->categories_pulldown->print_cat_pulldown($cid, 'skills', 'level', 'pid', $showpleaseselectoption = 1, fetch_site_slng(), $nooptgroups = 1, $prepopulate = '', $mode = 4, $showallcats = 1, $dojs = 0, $width = '540px', $uid = 0, $forcenocount = 1, $expertspulldown = 0, $canassigntoall = false, $showbestmatching = false, $categorycache);
                                
                                foreach ($categorycache AS $key => $value)
                                {
                                        if ($value['cid'] == $cid AND $value['level'] > 1)
                                        {
                                                print_action_failed($phrase['_sorry_you_can_only_create_new_skill_categories_based_on_2_levels'], $ilpage['subscribers'] . '?cmd=skills');
                                                exit();         
                                        }
                                }
                                unset($categorycache);
                                ...
                                $submit = '<input type="submit" value=" ' . $phrase['_save'] . ' " style="font-size:15px" class="buttons" />';
                                $subcmd = 'insertskill';
                                
                                $res['keywords'] = '<input type="text" name="keywords" value="" style="font-family: verdana; width:98%" />';
                                $res['sort'] = '0';
                ...
                else if (isset($ilance->GPC['subcmd']) AND $ilance->GPC['subcmd'] == 'insertskill')
                {
                        ...
                        if (!empty($ilance->GPC['title']))
                        {
                                ...
                                if ($titleerror)
                                {
                                        print_action_failed($phrase['_sorry_to_create_a_new_skill_category_you_must_define_a_title'], $ilpage['subscribers'] . '?cmd=skills&subcmd=addskill&cid=' . $cid);
                                        exit();	
                                }
                        }
                        else 
                        {
                                print_action_failed($phrase['_sorry_to_create_a_new_skill_category_you_must_define_a_title_for_all_available'], $ilpage['subscribers'] . '?cmd=skills&subcmd=addskill&cid=' . $cid);
                                exit();		
                        }
                        ...
                        $ilance->db->query("
                                ...
                                (parentid, $titlefields $descriptionfields keywords, sort)
                                VALUES(
                                '" . $pid . "',
                                $titlevalues
                                $descriptionvalues
                        ...
                        print_action_success($phrase['_new_skill_category_was_added'], $ilance->GPC['return']);
                        exit();
                }
                ...
                else if (isset($ilance->GPC['subcmd']) AND $ilance->GPC['subcmd'] == 'removeskill' AND isset($ilance->GPC['cid']))
                {
                        if ($ilance->categories_skills->can_remove_skill_categories())
                        {
                                ...
                                print_action_success($phrase['_the_selected_skill_category_was_removed_from_the_category_system'], $ilpage['subscribers'] . '?cmd=skills');
                                exit();
                        }
                        else 
                        {
                                print_action_failed($phrase['_sorry_you_must_have_at_least_1_skill_category_in_the_system_please_update_or_modify_this_category_instead'], $ilpage['subscribers']."?cmd=skills");
                                exit();	
                        }        
                }
		
		// #### SAVE SKILLS ORDER #################################
                else if (isset($ilance->GPC['subcmd']) AND $ilance->GPC['subcmd'] == '_update-service-sort')
                {
			...
			print_action_success($phrase['_skills_sorting_display_order_has_been_updated'], $ilpage['subscribers'] . '?cmd=skills');
			exit();
                }
		...
		// #### SKILLS MAIN LANDING PAGE ###############################
				...
				$ilance->categories->cats = $ilance->categories_skills->build_array_skills($_SESSION['ilancedata']['user']['slng'], -1, $counter);
					...
					if (isset($ilance->categories->cats[$i]['cid']) AND !empty($ilance->categories->cats[$i]['cid']) AND $ilance->categories->cats[$i]['cid'] > 0)
					{
						...
						if ($ilance->categories->cats[$i]['level'] == 1)
						{
							if ($ilance->categories->cats[$i]['parentid'] == 0)
							{
								...
								$ilance->categories->cats[$i]['title'] = '<a href="' . $ilpage['subscribers'] . '?cmd=skills&amp;subcmd=editskill&amp;cid=' . $ilance->categories->cats[$i]['cid'] . '" title="' . $ilance->categories->cats[$i]['description'] . '"><strong>' . $ilance->categories->cats[$i]['title'] . '</strong></a>';
							}
							else
							{
								$ilance->categories->cats[$i]['auctioncount'] = $ilance->categories_skills->fetch_skills_category_count($ilance->categories->cats[$i]['cid']);
								$ilance->categories->cats[$i]['title'] = '<a href="' . $ilpage['subscribers'] . '?cmd=skills&amp;subcmd=editskill&amp;cid=' . $ilance->categories->cats[$i]['cid'] . '" title="' . $ilance->categories->cats[$i]['description'] . '">' . $ilance->categories->cats[$i]['title'] . '</a>';
							}
						}
						else if ($ilance->categories->cats[$i]['level'] > 1)
						{
							if ($ilance->categories->cats[$i]['parentid'] == 0)
							{
								// treat category as main non-posting category
								$ilance->categories->cats[$i]['auctioncount'] = $ilance->categories_skills->fetch_skills_category_recursive_count($ilance->categories->cats[$i]['cid']);
								$ilance->categories->cats[$i]['title'] = str_repeat('<span class="gray">--</span> ', $ilance->categories->cats[$i]['level']) . '<a href="' . $ilpage['subscribers'] . '?cmd=skills&amp;subcmd=editskill&amp;cid=' . $ilance->categories->cats[$i]['cid'].'" title="' . $ilance->categories->cats[$i]['description'] . '"><strong>' . $ilance->categories->cats[$i]['title'] . '</strong></a>';
							}
							else 
							{
								$ilance->categories->cats[$i]['auctioncount'] = $ilance->categories_skills->fetch_skills_category_count($ilance->categories->cats[$i]['cid']);
								$ilance->categories->cats[$i]['title'] = str_repeat('<span class="gray">--</span> ', $ilance->categories->cats[$i]['level']) . '<a href="' . $ilpage['subscribers'] . '?cmd=skills&amp;subcmd=editskill&amp;cid=' . $ilance->categories->cats[$i]['cid'].'" title="' . $ilance->categories->cats[$i]['description'] . '">' . $ilance->categories->cats[$i]['title'] . '</a>';
							}
						}
...
function category_jump(catinfo)
{
    ...
    if (action != '')
    {
		    ...
		    case 'addtest': page = \"" . $ilpage['subscribers'] . "?cmd=skills&subcmd=addskilltest&cid=\"; break;
...
//-->
                ...
                $pprint_array = array('pp','prevnext','buildversion','ilanceversion','login_include_admin','configuration_skills','incrementgrouppulldown','incsort','inchidden','incform','incsubmit','incamount','incfrom','incto','hiddenincrementgroupid2','incrementgroupname','incrementgroupdescription','submitincrement','subcmdincrementgroup','language_pulldown','slng','checked_question_cansearch','checked_question_active','checked_question_required','subcategory_pulldown','formdefault','multiplechoice','question','description','formname','sort','submit_category_question','question_id_hidden','question_subcmd','question_inputtype_pulldown','subcatid','subcatname','catname','service_subcategories','product_categories','subcmd','id','submit','description','name','checked_profile_group_active','input_style','remote_addr','rid','login_include','headinclude','area_title','page_title','lanceads_header','lanceads_footer');
                
                ($apihook = $ilance->api('admincp_skills_end')) ? eval($apihook) : false;

                $ilance->template->fetch('main', 'skills.html', 1);
                $ilance->template->parse_hash('main', array('ilpage' => $ilpage));
                $ilance->template->parse_loop('main', array('v3nav','subnav_settings','servicecategories'));
                $ilance->template->parse_if_blocks('main');
                $ilance->template->pprint('main', $pprint_array);
                exit();
        }
        ...
        else if (isset($ilance->GPC['subcmd']) AND $ilance->GPC['subcmd'] == '_update-customer-profile-questions' AND isset($ilance->GPC['uid']) AND $ilance->GPC['uid'] > 0)
        {
                ...
                print_action_success($phrase['_profile_answers_were_updated_for_this_profile_an_email_was_not_sent'], $ilpage['subscribers'] . '?subcmd=_update-customer&id='.intval($ilance->GPC['uid']));
                exit();        
        }
        ...
        else if (isset($ilance->GPC['subcmd']) AND $ilance->GPC['subcmd'] == '_update-customer-register-questions' AND isset($ilance->GPC['uid']) AND $ilance->GPC['uid'] > 0)
        {
                ...
                print_action_success($phrase['_registration_answers_were_updated_for_this_profile_an_email_was_not_sent_to_the_customer'], $ilpage['subscribers'].'?subcmd=_update-customer&id='.intval($ilance->GPC['uid']));
                exit();        
        }
        ...
        else if (isset($ilance->GPC['subcmd']) AND $ilance->GPC['subcmd'] == '_remove-answer' AND isset($ilance->GPC['id']) AND $ilance->GPC['id'] > 0 AND isset($ilance->GPC['uid']) AND $ilance->GPC['uid'] > 0)
        {
                ...
                print_action_success($phrase['_you_have_removed_a_registration_answer_for_this_user_an_email_was_not_sent'], $ilpage['subscribers'].'?subcmd=_update-customer&id='.intval($ilance->GPC['uid']));
                exit();
        }
        ...
        else if (isset($ilance->GPC['cmd']) AND $ilance->GPC['cmd'] == '_do-role-change')
        {
                ...
                print_action_success($phrase['_you_have_updated_the_role_for_this_user'], $ilpage['subscribers'].'?subcmd=_update-customer&id='.intval($ilance->GPC['uid']));		
        }
            
        //bug1736 starts
		//ip_address change starts
		
			else if (isset($ilance->GPC['cmd']) AND $ilance->GPC['cmd'] == '_do-ip_address')
			{
		   
					if (!empty($ilance->GPC['uid']))
					{	    
						  $ip_restrict=(isset($ilance->GPC['ip_restrict']) AND $ilance->GPC['ip_restrict']=='1')?1:0;
						  $sel=$ilance->db->query("select staff_id,ip_address from " . DB_PREFIX . "staff_ipaddress WHERE staff_id='".$ilance->GPC['uid']."'");
						  if ($ilance->db->num_rows($sel) > 0)
						  {
							 $res = $ilance->db->fetch_array($sel, DB_ASSOC);
								// admin is changing the role for this user
										$ilance->db->query("
												UPDATE " . DB_PREFIX . "staff_ipaddress
												SET ip_address = '".$ilance->GPC['ip_address']."',
												ip_restrict='".$ip_restrict."'
												WHERE staff_id = '".intval($ilance->GPC['uid'])."'
						  ...
						  $sql = $ilance->db->query("
						INSERT INTO " . DB_PREFIX . "staff_ipaddress(id,staff_id,ip_address,ip_restrict) values(NULL,'".intval($ilance->GPC['uid'])."','".$ilance->GPC['ip_address']."','".$ip_restrict."') ");
						
						}
					}
			
			   
					print_action_success($phrase['_you_have_updated_the_role_for_this_user'], $ilpage['subscribers'].'?subcmd=_update-staff&id='.intval($ilance->GPC['uid']));		
			}
		
		//ip_address change ends
		
		//delete permission for user starts
			else if(isset($ilance->GPC['cmd']) AND $ilance->GPC['cmd'] == 'delete_permission' AND isset($ilance->GPC['uid']) AND $ilance->GPC['uid'] > 0)
			{
					foreach ($ilance->GPC['check_property'] as $key => $value)
					{
						$ilance->db->query("
						DELETE FROM " . DB_PREFIX . "staff_access
						WHERE staff_id = '" . intval($ilance->GPC['uid']) . "'
						AND permission_id = '".$value."'
						LIMIT 1
						");
					}
					print_action_success($phrase['You have successfully deleted the permissions for the staff'], $ilpage['subscribers'].'?subcmd=_update-staff&id='.intval($ilance->GPC['uid']));
			}
		//delete permission for user ends
	
		//add permission for staff starts
			else if(isset($ilance->GPC['cmd']) AND $ilance->GPC['cmd'] == 'add_property' AND isset($ilance->GPC['uid']) AND $ilance->GPC['uid'] > 0)
			{			
			
				if (!empty($ilance->GPC['uid']) AND !empty( $ilance->GPC['rem_perm_name']) )
					{
						foreach($ilance->GPC['rem_perm_name'] as $key=>$value)		  
						{
							$split=split('&',$value);
							
							$sql=$ilance->db->query("INSERT INTO " . DB_PREFIX . "staff_access(staff_id,permission_id,permission_name,value) 
												VALUES('".$ilance->GPC['uid']."' , '".$split[0]."' , '".$split[1]."' , '1')
												");
							
						}
						
						
						print_action_success($phrase['Functionality for staff is added'], $ilpage['subscribers'].'?subcmd=_update-staff&id='.intval($ilance->GPC['uid']));	
					}
			
			}
		//add permission for staff ends	
	...
	//bug1736 ends
		...
		if (!empty($removedusers))
		{
			...
			print_action_success($phrase['_the_selected_users_were_removed_from_the_marketplace_indefinately'] . " " . $removedusers . ". " . $phrase['_these_customers_will_not_be_able_to_login_to_the_marketplace_unless'], $ilpage['subscribers']);
			exit();
		}
		else
		{
			print_action_failed($phrase['_the_selected_user_was_not_found_to_be_removed_from_the_marketplace'], $ilpage['subscribers']);
			exit();	
		}
        }
		...
		set_cookie('inlinemembers', '', false);
			...
			{
				print_action_failed($phrase['_no_customers_were_selected_for_removal_please_try_again'], $ilpage['subscribers']);
				exit();	
			}
                }
                else
        ...
        else if (isset($ilance->GPC['subcmd']) AND $ilance->GPC['subcmd'] == '_remove-exemption' AND isset($ilance->GPC['id']) AND $ilance->GPC['id'] > 0 AND isset($ilance->GPC['uid']) AND $ilance->GPC['uid'] > 0)
        {
                ...
                print_action_success($phrase['_the_selected_exemption_was_removed_from_the_customers_subscription'], $ilpage['subscribers'].'?subcmd=_update-customer&amp;id='.intval($ilance->GPC['uid']));
                exit();
        }
		...
		if (!empty($suspendusers))
		{
			...
			print_action_success($phrase['_the_selected_users_have_been_suspended'].' '.$suspendusers, $ilpage['subscribers']);
			exit();
		}
		else
		{
			print_action_failed($phrase['_could_not_suspend_one_or_more_users_please_try_again'], $ilpage['subscribers']);
			exit();	
		}
        }
	...
	// #### ban users ######################################################
		...
		if (isset($ilance->GPC['user_id']) AND is_array($ilance->GPC['user_id']))
			...
			if (!empty($bannedusers))
			{
				...
				print_action_success($phrase['_the_selected_users_have_been_banned'].' '.$bannedusers, $ilpage['subscribers']);
				exit();
			}
			else
			{
				print_action_failed($phrase['_could_not_place_a_ban_one_or_more_users_please_try_again'], $ilpage['subscribers']);
				exit();	
			}
		}
		else
	...
	// #### cancel users ###################################################
		...
		if (isset($ilance->GPC['user_id']) AND is_array($ilance->GPC['user_id']))
			...
			if (!empty($cancelledusers))
			{
				...
				print_action_success($phrase['_the_selected_users_have_been_cancelled'].' '.$cancelledusers, $ilpage['subscribers']);
				exit();
			}
			else
			{
				print_action_failed($phrase['_could_not_cancel_one_or_more_users_please_try_again'], $ilpage['subscribers']);
				exit();	
			}
		}
		else
		...
		if (!empty($activatedusers))
		{
			...
			print_action_success($phrase['_the_selected_users_have_been_activated'].' '.$activatedusers, $ilpage['subscribers']);
			exit();
		}
		else
		{
			print_action_failed($phrase['_could_not_activate_one_or_more_users_please_try_again'], $ilpage['subscribers']);
			exit();	
		}
        }
	...
	// #### unverify multiple users ########################################
		...
		// empty inline cookie
			...
			if (!empty($unverifiedusers))
			{
				...
				print_action_success($phrase['_the_selected_users_have_been_unverified_and_will_need_to_verify_their_email_again_to_become_activated'].' '.$unverifiedusers, $ilpage['subscribers']);
				exit();
			}
			else
			{
				print_action_failed($phrase['_could_not_unverify_one_or_more_users_please_try_again'], $ilpage['subscribers']);
				exit();	
			}
                }
                else
        ...
        else if (isset($ilance->GPC['subcmd']) AND $ilance->GPC['subcmd'] == '_create-new-customer')
        {
        	...
        	{
        		print_action_failed($phrase['_passwords_are_empty_or_do_not_match'], $ilpage['subscribers']);
            	exit();
        	}
        	else if(!isset($ilance->GPC['username']) OR empty($ilance->GPC['username']))
        	{
        		print_action_failed($phrase['_please_enter_correct_username'], $ilpage['subscribers']);
            	exit();
        	}     
		    else if(!isset($ilance->GPC['email']) OR empty($ilance->GPC['email'])) 
		    {         
		    	print_action_failed($phrase['_please_enter_correct_email'], $ilpage['subscribers']);
            	exit();
		    }
		    else 
		    {
                ...
                {
                	print_action_failed($phrase['_this_username_is_banned'], $ilpage['subscribers']);
            		exit();
                }
        ...
        else if (isset($ilance->GPC['subcmd']) AND $ilance->GPC['subcmd'] == '_update-customer-profile' AND isset($ilance->GPC['id']) AND $ilance->GPC['id'] > 0)
        {
			...
			if ($ilconfig['globalauctionsettings_serviceauctionsenabled'])
			{
                ...
                {
                        print_action_failed($phrase['_sorry_the_username_you_entered_appears_to_be_in_the_username_ban_list'], $ilpage['subscribers'] . '?subcmd=_update-customer&id=' . intval($ilance->GPC['id']));
                        exit();
                }
        ...
        else if (isset($ilance->GPC['subcmd']) AND $ilance->GPC['subcmd'] == '_create-subscription' AND $ilance->GPC['id'] > 0)
        {
                ...
                print_action_success($phrase['_the_customer_has_been_reassigned_with_the_selected_subscription_plan'], $ilpage['subscribers'].'?subcmd=_update-customer&amp;id='.intval($ilance->GPC['id']));
                exit();
        }
        ...
        else if (isset($ilance->GPC['subcmd']) AND $ilance->GPC['subcmd'] == '_create-exemption' AND isset($ilance->GPC['id']) AND $ilance->GPC['id'] > 0)
        {
                ...
                {
                        print_action_success($phrase['_the_customer_has_been_assigned_with_the_selected_subscription_permission_exemption'], $ilpage['subscribers'].'?subcmd=_update-customer&amp;id='.intval($ilance->GPC['id']));
                        exit();
                }
                else 
                {
                        print_action_failed($phrase['_there_was_a_problem_with_the_action_selected_this_may_be_due_to_the_customer_not_having_sufficient_funds'], $ilpage['subscribers'].'?subcmd=_update-customer&amp;id='.intval($ilance->GPC['id']));
                        exit();
                }
        }
        ...
        else if (isset($ilance->GPC['subcmd']) AND $ilance->GPC['subcmd'] == '_authorize-creditcard' AND $ilance->GPC['id'] > 0 AND $ilance->GPC['uid'] > 0)
        {
                ...
                {
                        print_action_success($phrase['_the_selected_credit_card_was_manually_authorized_verified_from_administration'], $ilpage['subscribers'] . '?subcmd=_update-customer&amp;id=' . intval($ilance->GPC['uid']));
                        exit();
                }
        }
        ...
        else if (isset($ilance->GPC['subcmd']) AND $ilance->GPC['subcmd'] == '_unauthorize-creditcard' AND isset($ilance->GPC['id']) AND $ilance->GPC['id'] > 0  AND isset($ilance->GPC['uid']) AND $ilance->GPC['uid'] > 0)
        {
                ...
                {
                        print_action_success($phrase['_the_selected_credit_card_was_manually_unauthorized_and_this_customer_will_be_required_to_manually_verify'], $ilpage['subscribers'] . '?subcmd=_update-customer&amp;id=' . intval($ilance->GPC['uid']));
                        exit();
                }
        }
        ...
        else if (isset($ilance->GPC['subcmd']) AND $ilance->GPC['subcmd'] == '_remove-creditcard' AND $ilance->GPC['id'] > 0 AND $ilance->GPC['uid'] > 0)
        {
                ...
                {
                        print_action_success($phrase['_the_selected_credit_card_was_removed_from_the_customers_profile_this_customer_will_be_required_to_verify_any'], $ilpage['subscribers'] . '?subcmd=_update-customer&amp;id=' . intval($ilance->GPC['uid']));
                        exit();
                }
        }
        else if (isset($ilance->GPC['subcmd']) AND $ilance->GPC['subcmd'] == '_remove-bankaccount' AND $ilance->GPC['id'] > 0 AND $ilance->GPC['uid'] > 0)
        {
                ...
                print_action_success($phrase['_the_selected_bank_account_was_removed_from_the_customers_profile'], $ilpage['subscribers'].'?subcmd=_update-customer&amp;id='.intval($ilance->GPC['uid']));
                exit();
        }
        else if (isset($ilance->GPC['subcmd']) AND $ilance->GPC['subcmd'] == '_remove-transaction' AND $ilance->GPC['id'] > 0 AND $ilance->GPC['uid'] > 0)
        {
                ...
                print_action_success($phrase['_the_selected_transaction_was_removed_from_the_transaction_system'], $ilpage['subscribers'].'?subcmd=_update-customer&amp;id='.intval($ilance->GPC['uid']));
                exit();
        }
		 ...
		 else if (isset($ilance->GPC['subcmd']) AND $ilance->GPC['subcmd'] == '_create-note' AND $ilance->GPC['id'] > 0 )
        {
                if (trim($ilance->GPC['note'])<>'') 
				 { 
				  if ($ilance->GPC['note_id']>0)
				    {
				  $ilance->db->query("
                        update  " . DB_PREFIX . "users_note
                        SET note='" . addslashes($ilance->GPC['note']) . "', modified =now() where note_id='".intval($ilance->GPC['note_id'])."'
                ");
				   print_action_success($phrase['_the_selected_note_was_updated_successfully'], $ilpage['subscribers'].'?subcmd=_update-customer&amp;id='.intval($ilance->GPC['id']));
                exit(); 
				    }
				 else
				   {
				   $ilance->db->query("
                        ...
                        SET user_id = '" . intval($ilance->GPC['id']) . "',note='" . addslashes($ilance->GPC['note']) . "', created=now(),modified =now()
                "); 
				    print_action_success($phrase['_the_note_was_created_successfully'], $ilpage['subscribers'].'?subcmd=_update-customer&amp;id='.intval($ilance->GPC['id']));
                exit(); 
				   }	        
		...
		else if (isset($ilance->GPC['subcmd']) AND $ilance->GPC['subcmd'] == '_remove-note' AND $ilance->GPC['id'] > 0 AND $ilance->GPC['uid'] > 0 )
		{
		     $ilance->db->query("
                ...
                print_action_success($phrase['_the_selected_note_was_deleted_successfully'], $ilpage['subscribers'].'?subcmd=_update-customer&amp;id='.intval($ilance->GPC['uid']));
                exit();
	   }
                ...
                if (isset($ilance->GPC['action']) AND $ilance->GPC['action'] == "debit" AND $ilance->GPC['amount'] > 0)
                {
                        ...
                        if ($ilance->db->num_rows($sql) > 0)
                        {
                                ...
                                if ($ilance->db->num_rows($sqlemail) > 0)
                                {
                                        ...
                                        print_action_success($phrase['_a_new_transaction_debit_was_successfully_created_and_the_customers'], $ilpage['subscribers'].'?subcmd=_update-customer&amp;id='.intval($ilance->GPC['id']));
                                        exit();
                                }
                        }
                }
                else if (isset($ilance->GPC['action']) AND $ilance->GPC['action'] == 'credit' AND $ilance->GPC['amount'] > 0)
                {
                        ...
                        if ($ilance->db->num_rows($sql) > 0)
                        {
                                ...
                                if ($ilance->db->num_rows($sqlemail) > 0)
                                {
                                        ...
                                        print_action_success($phrase['_a_new_transaction_credit_was_successfully_created_and_the_customers'], $ilpage['subscribers'].'?subcmd=_update-customer&amp;id='.intval($ilance->GPC['id']));
                                        exit();						
                                }
                        }
                }
        ...
        else
        {
				...
				//bug1736
					...
					if (isset($ilance->GPC['subcmd']) AND $ilance->GPC['subcmd'] == '_update-staff' AND isset($ilance->GPC['id']) AND $ilance->GPC['id'] > 0)
					{
						...
						$sql_ip_address=$ilance->db->query("
							SELECT * 
							FROM " . DB_PREFIX . "staff_ipaddress
							WHERE staff_id = '" . intval($ilance->GPC['id']) . "'
							LIMIT 1
						");
						
						$res_ip_address= $ilance->db->fetch_array($sql_ip_address);
						$ip_add=$res_ip_address['ip_address'];
						$check_restrict=$res_ip_address['ip_restrict'];
						if($check_restrict=='0')
						{
							$check='<input type="checkbox" name="ip_restrict" id="ip_restrict" value="1"/>';
							$check_text='Check here if you want to restrict ip address for this member';
						}
						else
						{
							$check='<input type="checkbox" name="ip_restrict" id="ip_restrict" value="0" checked="checked"/>';
							$check_text='Uncheck here if you dont want to restrict any ip address for this member';
						}
						$id=$ilance->GPC['id'];
						
						//Edit property for a particular staff  starts
						
							$sql_permission_name=$ilance->db->query("
								SELECT * 
								FROM " . DB_PREFIX . "staff_permissions	P
								INNER JOIN " . DB_PREFIX . "staff_access A ON P.id=A.permission_id AND A.staff_id='" . intval($ilance->GPC['id']) . "'
							");
							$count=1;
							while($res_permission_name= $ilance->db->fetch_array($sql_permission_name))
							{
							
								
								$res['s_no']=$count;
								$res['permission_id']=$res_permission_name['permission_id'];
								$res['permission_name']=$res_permission_name['permission_name'];
								$res['permission_desc']=$res_permission_name['permission_desc'];
								$res['permission_check']='<input type="checkbox" name="check_property[]" id="check_property" value='.$res['permission_id'].'>';
								$permission_name[]=$res;
								$count++;					
							}
							
							$sql_permission_name_rem=$ilance->db->query("SELECT p.id,p.permission_name,p.permission_desc FROM " . DB_PREFIX . "staff_permissions p
																		LEFT JOIN " . DB_PREFIX . "staff_access a 
																		ON p.id=a.permission_id  AND a.staff_id='" . intval($ilance->GPC['id']) . "' 
																		WHERE a.staff_id IS NULL
																		");
							$rem_perm_list='<select multiple="multiple" name="rem_perm_name[]" style="height:200px;width:200px;">';											
							while($res_permission_name_rem= $ilance->db->fetch_array($sql_permission_name_rem))
							{
								// $res['remaining_perm']=$res_permission_name_rem['permission_name'];
								
								$rem_perm_list.='<option value='.$res_permission_name_rem['id'].'&'.$res_permission_name_rem['permission_name'].' >'.$res_permission_name_rem['permission_name'].'</option>';
								
								
							}
							$rem_perm_list.='</select>';
						
						//Edit property for a particular staff ends
					
						$pprint_array = array('check','check_text','id','ip_add','rem_perm_list','option_name','ilanceversion','login_include_admin');
						$ilance->template->fetch('main', 'staff_edit.html', 1);
						$ilance->template->parse_hash('main', array('ilpage' => $ilpage));
						$ilance->template->parse_loop('main', array('v3nav','subnav_settings','staff','customers','searchcustomers','registration_questions','profile','profileaccount','transaction_rows','subscription_rows','creditcards','bankaccounts','exemptions','auditlog','notes','permission_name'));
						$ilance->template->parse_if_blocks('main');
						$ilance->template->pprint('main', $pprint_array);
						exit();				
					}
					...
					//e                
                        ...
                        if ($ilance->db->num_rows($res) > 0)
                        {
                                ...
                                while ($row = $ilance->db->fetch_array($res))
                                {
                                        ...
                                        {
                                                $row['action'] = "<a href='".$ilpage['subscribers']."?subcmd=_remove-transaction&amp;id=".$row['invoiceid']."&amp;uid=" . intval($ilance->GPC['id']) . "' target='_self' onClick=\"return confirm('".$phrase['_please_take_a_moment_to_confirm_your_action']."')\"><img src='" . $ilconfig['template_relativeimagepath'] . $ilconfig['template_imagesfolder'] . "icons/delete.gif' border=0 alt=''></a>";
                        ...
                        if ($ilance->db->num_rows($sqlexempt) > 0)
                        {
                                ...
                                while ($resexempt = $ilance->db->fetch_array($sqlexempt))
                                {
                                        ...
                                        if ($resexempt['active'])
                                        {
                                                ...
                                                $resexempt['action'] = '<a href="' . $ilpage['subscribers'] . '?subcmd=_remove-exemption&amp;id='.$resexempt['exemptid'].'&amp;uid='.$resexempt['user_id'].'" onClick="return confirm(\'' . $phrase['_please_take_a_moment_to_confirm_your_action'] . '\')">'.$phrase['_remove'].'</a>';
                                        }
                                        else
                                        {
                        ...
                        if ($ilance->db->num_rows($sql) > 0)
                        {
                                ...
                                while ($res = $ilance->db->fetch_array($sql))
                                {
                                        ...
                                        if ($res['authorized'] == 'yes')
                                        {
                                                ...
                                                $res['authenticated'] = '<a href="' . $ilpage['subscribers'] . '?subcmd=_unauthorize-creditcard&amp;id=' . $res['cc_id'] . '&amp;uid=' . intval($ilance->GPC['id']) . '&amp;ccmgr=1" onclick="return confirm(\'' . $phrase['_please_take_a_moment_to_confirm_your_action'] . '\')"><img src="' . $ilconfig['template_relativeimagepath'] . $ilconfig['template_imagesfolder'] . 'checked.gif" alt="Click to unauthorize credit card" border="0"></a>';
                                        }
                                        else
                                        {
                                                $res['status'] = ucfirst($res['creditcard_status']) . ' &amp; Unauthorized';
                                                $res['authenticated'] = '<a href="' . $ilpage['subscribers'] . '?subcmd=_authorize-creditcard&amp;id=' . $res['cc_id'] . '&amp;uid=' . intval($ilance->GPC['id']) . '&amp;ccmgr=1" onclick="return confirm(\'' . $phrase['_please_take_a_moment_to_confirm_your_action'] . '\')"><img src="' . $ilconfig['template_relativeimagepath'] . $ilconfig['template_imagesfolder'] . 'unchecked.gif" alt="Click to authorize credit card" border="0"></a>';
                                        }
                                        ...
                                        $res['remove'] = '<a href="' . $ilpage['subscribers'] . '?subcmd=_remove-creditcard&amp;id='.$res['cc_id'].'&amp;uid='.intval($ilance->GPC['id']).'" onClick="return confirm(\'' . $phrase['_please_take_a_moment_to_confirm_your_action'] . '\')"><img src="' . $ilconfig['template_relativeimagepath'] . $ilconfig['template_imagesfolder'] . 'icons/delete.gif" alt="" border="0" /></a>';
                                        $res['class'] = ($row_count % 2) ? 'alt2' : 'alt1';
                                        $creditcards[] = $res;
                                        $row_count++;
                                }
                        ...
                        if ($ilance->db->num_rows($sql) > 0)
                        {
                                ...
                                while ($res = $ilance->db->fetch_array($sql))
                                {
                                        ...
                                        $res['remove'] = '<a href="' . $ilpage['subscribers'] . '?subcmd=_remove-bankaccount&amp;id='.$res['bank_id'].'&amp;uid='.intval($ilance->GPC['id']).'" onClick="return confirm(\'' . $phrase['_please_take_a_moment_to_confirm_your_action'] . '\')"><img src="' . $ilconfig['template_relativeimagepath'] . $ilconfig['template_imagesfolder'] . 'icons/delete.gif" alt="" border="0" /></a>';
                                        $res['class'] = ($row_count % 2) ? 'alt2' : 'alt1';
                                        $bankaccounts[] = $res;
                                        $row_count++;
                                }
						...
						if ($ilance->db->num_rows($sql) > 0)
                        {
                                ...
                                while ($res = $ilance->db->fetch_array($sql))
                                {
										...
										$res['edit'] = '<a href="#" onClick="shownote(\''.$noteid.'\',\''.$note.'\'); return false;"><img src="'.$ilconfig['template_relativeimagepath'].$ilconfig['template_imagesfolder'].'icons/pencil.gif" alt="" border="0" /></a>';
										$res['remove'] = '<a href="'.$ilpage['subscribers'].'?subcmd=_remove-note&amp;id='.$res['note_id'].'&amp;uid='.intval($ilance->GPC['id']).'" onClick="return confirm(\''.$phrase['_please_take_a_moment_to_confirm_your_action'].'\')"><img src="'.$ilconfig['template_relativeimagepath'].$ilconfig['template_imagesfolder'].'icons/delete.gif" alt="" border="0" /></a>';
                                        $res['class'] = ($row_count % 2) ? 'alt2' : 'alt1';
                                        $notes[] = $res;
                                        $row_count++;
...
$show['no_notes'] = true;
                ...
                if (isset($ilance->GPC['cmd']) AND $ilance->GPC['cmd'] == 'search')
                {
                        ...
                        $sqlsearchcustomers = $ilance->db->query("
                                SELECT user_id, username, first_name, last_name,Check_Payable, email, phone, city, state, zip_code, status, available_balance, total_balance, isadmin, permissions
                                FROM " . DB_PREFIX . "users
                                $where
                                $orderlimit
                        ");
                        ...
                        if ($ilance->db->num_rows($sqlsearchcustomers) > 0)
                        {
                                ...
                                while ($res = $ilance->db->fetch_array($sqlsearchcustomers, DB_ASSOC))
                                {
                                        ...
                                        $res['remove'] = '<a href="' . $ilpage['subscribers'] . '?subcmd=deleteuser&amp;id=' . $res['user_id'] . '" onClick="return confirm(\'' . $phrase['_please_take_a_moment_to_confirm_your_action'] . '\')"><img src="' . $ilconfig['template_relativeimagepath'] . $ilconfig['template_imagesfolder'] . 'icons/delete.gif" alt="" border="0"></a>';
					...
					$res['login'] = '<a href="' . $ilpage['subscribers'] . '?subcmd=switchuser&amp;id=' . $res['user_id'] . '" onClick="return confirm(\'' . $phrase['_please_take_a_moment_to_confirm_your_action'] . '\')"><img src="' . $ilconfig['template_relativeimagepath'] . $ilconfig['template_imagesfolder'] . 'icons/picture_blue.gif" alt="' . "Switch To Another User". '" border="0"></a>';
                                        $searchcustomers[] = $res;
                                        $row_count++;
                                }
                ...
                else
                {
                        ...
                        $scriptpage = $ilpage['subscribers'] . '?cmd=listing' . $displayorder . $orderby;
                        ...
                        if ($ilance->db->num_rows($sql) > 0)
                        {
                                ...
                                while ($res = $ilance->db->fetch_array($sql, DB_ASSOC))
                                {
                                        ...
                                        {
                                                $res['status'] = '<a href="' . $ilpage['subscribers'] . '?subcmd=suspenduser&amp;id=' . $res['user_id'] . '"><img src="' . $ilconfig['template_relativeimagepath'] . $ilconfig['template_imagesfolder'] . 'checked.gif" alt="Click to suspend customer (cannot log-in)" border="0"></a>';
                                        }
                                        else
                                        {
                                                $res['status'] = '<a href="' . $ilpage['subscribers'] . '?subcmd=activateuser&amp;id=' . $res['user_id'] . '"><img src="' . $ilconfig['template_relativeimagepath'] . $ilconfig['template_imagesfolder'] . 'unchecked.gif" alt="Click to re-activate customer (can log in)" border="0"></a>';
                                        }
                ...
                if (isset($ilance->GPC['do']) AND $ilance->GPC['do'] == 'view')
                {
                        ...
                        if ($ilance->db->num_rows($audit) > 0)
                        {
                                ...
                                while ($res = $ilance->db->fetch_array($audit))
                                {
                                        ...
                                        if ($res['user_id'] > 0)
                                        {
                                                $res['user'] = '<a href="' . $ilpage['subscribers'] . '?subcmd=_update-customer&id=' . $res['user_id'] . '">' . fetch_user('username', $res['user_id']) . '</a>';
                                        }
                                        else
                                        {
                                                $res['user'] = '--';
                ...
                else if (isset($ilance->GPC['do']) AND $ilance->GPC['do'] == 'prune')
                {
                        ...
                        if (!empty($ilance->GPC['admin_id']))
                        {
                                $conds .= " AND admin_id = '" . intval($ilance->GPC['admin_id']) . "'";
                        }
                        ...
                        print_action_success($phrase['_audit_logs_pruned'] . ' ' .$count, $ilance->GPC['return']);
                        exit();    
                }
        
                $profilequestions = $phrase['_not_available_at_this_time'];
				
				//bug1736 fetch staff members
				// field display order
                       
					// ordering by display logic
                        $orderbyfields = array('user_id', 'username');
                        $orderby = '';
                        $orderbysql = 'user_id';
                        if (isset($ilance->GPC['orderby']) AND in_array($ilance->GPC['orderby'], $orderbyfields))
                        {
					...
					$sql_staff = $ilance->db->query("
					SELECT user_id, username, first_name, last_name, email, phone, city, state, zip_code, status, available_balance, total_balance, isstaff, isadmin, permissions
					FROM " . DB_PREFIX . "users WHERE (isstaff='1' 
					or isstaff='2')
					ORDER BY $orderbysql
					$displayordersql
					LIMIT " . (($ilance->GPC['page'] - 1) * $ilconfig['globalfilters_maxrowsdisplaysubscribers']) . "," . $ilconfig['globalfilters_maxrowsdisplaysubscribers']
				...
				$sql_staff2 = $ilance->db->query("
					SELECT user_id, username, first_name, last_name, email, phone, city, state, zip_code, status, available_balance, total_balance, isstaff,isadmin, permissions
					FROM " . DB_PREFIX . "users WHERE (isstaff='1' 
					or isstaff='2')
				");
				if ($ilance->db->num_rows($sql_staff) > 0)
				{
					$number1 = (int)$ilance->db->num_rows($sql_staff2);
					$row_count = 0;
					while ($res = $ilance->db->fetch_array($sql_staff, DB_ASSOC))
					{
						if ($res['status'] == 'moderated')
						{
							$res['class'] = '#FFF7F9';   
						...
						/*vijay #3365 dec5 13 if ($res['isstaff'])
						{
							    $res['isstaff'] = '<a href="' . $ilpage['subscribers'] . '?subcmd=staff_deactivate&amp;id=' . $res['user_id'] . '"><img src="' . $ilconfig['template_relativeimagepath'] . $ilconfig['template_imagesfolder'] . 'checked.gif" border="0" alt="'.$phrase['_yes'].'" /></a>';
						} */
						...
						$res['edit'] = '<a href="' . $ilpage['subscribers'] . '?subcmd=_update-staff&amp;id=' . $res['user_id'] . '"><img src="' . $ilconfig['template_relativeimagepath'] . $ilconfig['template_imagesfolder'] . 'icons/pencil.gif" alt="" border="0"></a>';
						$res['remove'] = '<a href="' . $ilpage['subscribers'] . '?subcmd=deleteuser&amp;id=' . $res['user_id'] . '" onClick="return confirm(\'' . $phrase['_please_take_a_moment_to_confirm_your_action'] . '\')"><img src="' . $ilconfig['template_relativeimagepath'] . $ilconfig['template_imagesfolder'] . 'icons/delete.gif" alt="" border="0"></a>';
					...
					$searchprevnext = '';
					$prevnext = print_pagnation($number1, $ilconfig['globalfilters_maxrowsdisplaysubscribers'], $ilance->GPC['page'], $counter, $scriptpageprevnext);
				}
				else
				{
					$show['no_staff'] = true;
				...
				//bug1736 fetch staff
				//bug1736  fetch all permissions
	      
					$sql_permission=$ilance->db->query("
						SELECT * 
						FROM " . DB_PREFIX . "staff_permissions
						
					");
					
					while($res_permission_name= $ilance->db->fetch_array($sql_permission))
					{
