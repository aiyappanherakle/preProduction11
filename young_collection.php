...
if (!empty($ilance->GPC['mode']) AND $show['searcherror'] == 0)
{
        ...
        $scriptpage = HTTP_SERVER . 'young_collection.php' . print_hidden_fields(true, array('do','cmd','page','budget','searchid','list'), true, '', '', $htmlentities = true, $urldecode = false);
								...
								if(!empty($ilance->GPC['grading_service']) || !empty($ilance->GPC['fromyear']) || !empty($ilance->GPC['toyear']) || !empty($ilance->GPC['grade_range_1']) || !empty($ilance->GPC['grade_range_2']) || !empty($ilance->GPC['frombid']) || !empty($ilance->GPC['tobid']) || !empty($ilance->GPC['listing_type']) || !empty($ilance->GPC['denom_all']) || !empty($ilance->GPC['denomination']) ){
									...
									if(!empty($ilance->GPC['grading_service']) )
									{
										...
										elseif(in_array('Raw/Other',$gpc_sql_grading_service_arr)){
											...
											$sqlquery['grading_service'] = " AND (p.Grading_Service IN(".$gpc_sql_grading_service_arr_1.")) ";
										}
				...
				else
				{
					$sqlquery['genrequery'] = '';
                        ...
                        foreach($project_like_keyword as $piece)
                        {
                         ...
                         $SQL  = "$sqlquery[select] $sqlquery[timestamp] $sqlquery[projectstatus] $sqlquery[keywords] $sqlquery[categories] $sqlquery[projectdetails] $sqlquery[projectstate] $sqlquery[projecttitle_like] $sqlquery[grading_service] $sqlquery[year_range] $sqlquery[grade_range] $sqlquery[bid_range] $sqlquery[listing_type] $sqlquery[denomination] $sqlquery[options] $sqlquery[pricerange] $sqlquery[location] $sqlquery[radius] $sqlquery[userquery] $sqlquery[hidequery] $sqlquery[genrequery] $sqlquery[groupby] $sqlquery[orderby] $sqlquery[limit]";
                      ...
                      //  $page_title = ((isset($keyword_text) AND !empty($keyword_text)) ? $keyword_text . ', ' : '') . '' . $phrase['_find'] . ' ' . (($project_state == 'service') ? $phrase['_services'] : $phrase['_items']) . ' ' . ((!empty($metadescription)) ? $phrase['_in'] . ' ' . $metatitle . ', ' . $metadescription : '') . ' | ' . SITE_NAME;
                        ...
                        if ($ilance->db->num_rows($result) > 0)
                        {
							...
							if(!empty($row['filehash']))
							{
								...
								<a href="' . $url . '">
                                <img src="' . HTTPS_SERVER . 'image/72/55/' . $row['filehash'] . '"
                                border="' . $borderwidth . '" alt="" style="border-color:' . $bordercolor . '" class="gallery-thumbs-image-cluster" /></a>
	...
	: '<div class="smaller blue"> Bid </div>';
					...
					$pbit = $row['maxamount'];
					 ...
					 $featuredproductauctions = ($show['mode_product']) ?fetch_featured_auctions_new('product', 4, 1, $cid, $keyword_text, false, $excludelist,$series) : '';
                        ...
                        unset($budgettemp);
                                ...
                                $v3left_nav = $ilance->template->print_left_nav('product', $cid, $dosubcats = 1, $displayboth = 0, $ilconfig['globalfilters_enablecategorycount'], true,0,'young_collection');
			...
			$hiddenfields = print_hidden_fields(false, array('searchid','sef','cid','buynow','sort','images','freeshipping','listedaslots','budget','publicboard','escrow','underage','endstart','endstart_filter','q','page'), $questionmarkfirst = false, $prepend_text = '', $append_text = '', $htmlentities = true, $urldecode = true);
				...
				$metadescription="Searching 1000s of ".$keywords." certified coins at GreatCollections Coin Auctions & Rare Coin Sales";
							...
							$gpc_grading_arr=$ilance->GPC['grading_service'];							
							...
							$toyear= !empty($ilance->GPC['toyear']) ? $ilance->GPC['toyear'] : '2013';
							...
							$gpc_denom_arr=$ilance->GPC['denomination'];
							...
							$navcrumb = array("young-collection"=>"Young Collection");
							$page_title = "The Young Collection of Silver Commemoratives | GreatCollections Coin Auctions";
