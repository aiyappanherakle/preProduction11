	...
	//year search of pcgs
	 if(isset($ilance->GPC['text']))
		...
		function explodeX( $delimiters, $string )
            ...
            $explodeds = explodeX( array(' ', ' ', ' ', ' '), $ilance->GPC['query']);
		     
           $year = "where concat(coin_detail_year,'-',coin_detail_mintmark,' ',coin_detail_coin_series,' ',coin_detail_major_variety,' ',coin_detail_suffix) like '%".$explodeds['0']."%' AND
concat(coin_detail_year,'-',coin_detail_mintmark,' ',coin_detail_coin_series,' ',coin_detail_major_variety,' ',coin_detail_suffix) like '%".$explodeds['1']."%' AND
concat(coin_detail_year,'-',coin_detail_mintmark,' ',coin_detail_coin_series,' ',coin_detail_major_variety,' ',coin_detail_suffix) like '%".$explodeds['2']."%' AND
concat(coin_detail_year,'-',coin_detail_mintmark,' ',coin_detail_coin_series,' ',coin_detail_major_variety,' ',coin_detail_suffix) like '%".$explodeds['3']."%' AND
concat(coin_detail_year,'-',coin_detail_mintmark,' ',coin_detail_coin_series,' ',coin_detail_major_variety,' ',coin_detail_suffix) like '%".$explodeds['4']."%' 
							";
		...
		$result=$ilance->db->query("SELECT PCGS,coin_detail_year,coin_detail_coin_series,coin_detail_major_variety,coin_detail_mintmark,coin_detail_suffix,coin_detail_denom_short,coin_detail_proof FROM " . DB_PREFIX . "catalog_coin ".$year." ORDER BY  Orderno ASC");
...
//vijay starts 
// #### ADD FAV  #######################################################
if (isset($ilance->GPC['do']) AND $ilance->GPC['do'] == 'add_fav_bold' AND isset($_SESSION['ilancedata']['user']['userid']) AND $_SESSION['ilancedata']['user']['userid'] > 0)
{
        $ilance->xml = construct_object('api.xml');
                ...
                if (isset($ilance->GPC['project_id']) AND $ilance->GPC['project_id'] > 0)
                {
					...
					$ilance->GPC['project_id'] = intval($ilance->GPC['project_id']);
                    $ilance->GPC['value'] ;
                
                    if ($ilance->GPC['value'] == 'on')
                    {
						$upbold = $ilance->db->query("
						UPDATE " . DB_PREFIX . "projects
						SET bold = '1' WHERE  project_id = '".$ilance->GPC['project_id']."'"); 

						$status =   'added';
                        $ilance->xml->add_tag('status', $status);
                    ...
                    else
                    {
                        $upbold = $ilance->db->query("
						UPDATE " . DB_PREFIX . "projects
						SET bold='0' WHERE  project_id = '".$ilance->GPC['project_id']."'"); 
...
if (isset($ilance->GPC['do']) AND $ilance->GPC['do'] == 'add_highlite' AND isset($_SESSION['ilancedata']['user']['userid']) AND $_SESSION['ilancedata']['user']['userid'] > 0)
{
        $ilance->xml = construct_object('api.xml');
                ...
                if (isset($ilance->GPC['project_id']) AND $ilance->GPC['project_id'] > 0)
                {
                    ...
                    if ($ilance->GPC['value'] == 'on')
                    {
						$upbold = $ilance->db->query("
						UPDATE " . DB_PREFIX . "projects
						SET highlite = '1' WHERE  project_id = '".$ilance->GPC['project_id']."'"); 
                    ...
                    else
                    {
                        $upbold = $ilance->db->query("
						UPDATE " . DB_PREFIX . "projects
						SET highlite='0' WHERE  project_id = '".$ilance->GPC['project_id']."'"); 
...
if (isset($ilance->GPC['do']) AND $ilance->GPC['do'] == 'add_featured' AND isset($_SESSION['ilancedata']['user']['userid']) AND $_SESSION['ilancedata']['user']['userid'] > 0)
{
        $ilance->xml = construct_object('api.xml');
                ...
                if (isset($ilance->GPC['project_id']) AND $ilance->GPC['project_id'] > 0)
                {
                    ...
                    if ($ilance->GPC['value'] == 'on')
                    {
						$upbold = $ilance->db->query("
						UPDATE " . DB_PREFIX . "projects
						SET featured = '1' WHERE  project_id = '".$ilance->GPC['project_id']."'"); 
                    ...
                    else
                    {
                        $upbold = $ilance->db->query("
						UPDATE " . DB_PREFIX . "projects
						SET featured='0' WHERE  project_id = '".$ilance->GPC['project_id']."'"); 
...
if (isset($ilance->GPC['do']) AND $ilance->GPC['do'] == 'add_hotlist' AND isset($_SESSION['ilancedata']['user']['userid']) AND $_SESSION['ilancedata']['user']['userid'] > 0)
{
        $ilance->xml = construct_object('api.xml');
                ...
                if (isset($ilance->GPC['project_id']) AND $ilance->GPC['project_id'] > 0)
                {
                    ...
                    if ($ilance->GPC['value'] == 'on')
                    {
						$upbold = $ilance->db->query("
						UPDATE " . DB_PREFIX . "projects
						SET hotlists = '1' WHERE  project_id = '".$ilance->GPC['project_id']."'"); 
                    ...
                    else
                    {
                        $upbold = $ilance->db->query("
						UPDATE " . DB_PREFIX . "projects
						SET hotlists='0' WHERE  project_id = '".$ilance->GPC['project_id']."'"); 
...
// kumaravel bug # 5262 start

	if(isset($_GET['pending_autobuild_con_new_relist']))
	{
								   $con_listing_co = $ilance->db->query("
									SELECT *
									FROM " . DB_PREFIX . "coins 
									...
									if($ilance->db->num_rows($con_listing_co) > 0)
									{
									    while($row_list1 = $ilance->db->fetch_array($con_listing_co))
										{	
										
										
										$row[] = $row_list1['coin_id'];	
	...
	if(isset($_GET['pending_autobuild_con_new_imaged']))
	{
								   $con_listing_co = $ilance->db->query("
									SELECT *
									FROM " . DB_PREFIX . "coins
									...
									if($ilance->db->num_rows($con_listing_co) > 0)
									{
									    while($row_list2 = $ilance->db->fetch_array($con_listing_co))
										{	

											$sel_attach = $ilance->db->query("
													SELECT * FROM " . DB_PREFIX . "attachment
													WHERE project_id ='".$row_list2['coin_id']."' ");
											if($ilance->db->num_rows($sel_attach) > 0)
											{
												$row[] = $row_list2['coin_id'];									
											}
											else
											{
		...
		// kumaravel bug # 5262 end

		//bug #6467
		if(isset($_GET['pending_autobuild_con_Exc_dol1_new']))
		{
			$con_listing_co = $ilance->db->query("
			SELECT *
			FROM " . DB_PREFIX . "coins 
			...
			AND project_id  = '0'
			AND status = '0'
			AND (Minimum_bid > 1 OR Buy_it_now > 1 )
			group by Title,pcgs,Grade,nocoin order by pcgs desc,Grade desc;
			");
	 ...
	 if(isset($_GET['pending_list_user']))
	 {
				    ...
				    <input type="hidden" name="subcmd" value="allcheck_update_date" />
											<table width="100%" cellpadding="9" cellspacing="0">
											<tr class="alt2">
											<td width="12%" align="left">Check &nbsp; <input type="checkbox" id="CtoA" /></td>
											<td width="12%" align="left">Coin ID</td>
									<td width="12%">PCGS</td>
									<td width="12%">Coin</td>
									...
									<td width="12%">Return</td>
									  ...
									  echo '<tr class="alt1" valign="top">
									<td width="12%" align="left"><input type="checkbox" class="checkbox1" value="'.$row_list['coin_id'].'" name="incheckdate[]" id="coin_'.$row_list['coin_id'].'"></td>
									<td width="12%" align="left">'.$row_list['coin_id'].'</td>
									<td width="12%">'.$row_list['pcgs'].'</td>
...
if(isset($_GET['pending_list_user_new']))
	 {
		...
		<strong>'.fetch_user('username', $_GET['pending_list_user_new']).'</strong> 
		<span class="blue" style="cursor:pointer;"  
			onClick="window.open(\'pendings.php?cmd=all_update&subcmd=allcoin&user_id='.$_GET['pending_list_user_new'].'&consignid='.$_GET['pending_list_con'].'\',\'mywindow\',\'width=400,height=200,scrollbars=yes\')">Click Here</span>
	</td>
	<td width="6%"></td>
	<td width="12%">
		<span class="blue" style="cursor:pointer;" 
			onclick="checkautobuildnew_relist('.$_GET['pending_list_con'].','.$_GET['pending_list_user_new'].');"><b>
			Recycles</b></span>
	</td>

	<td width="12%">
		<span class="blue" style="cursor:pointer;" 
				onclick="checkautobuildnew_imaged('.$_GET['pending_list_con'].','.$_GET['pending_list_user_new'].');"><b>
				Imaged</b></span>
	</td>

	<td width="12%"><span class="blue" style="cursor:pointer;" 
		onclick="checkautobuildnew('.$_GET['pending_list_con'].','.$_GET['pending_list_user_new'].');"><b>
		Autobuild</b></span>
	</td>
	<td width="14%" colspan="2">
		<span class="blue" style="cursor:pointer;" 
		onclick="checkautobuildnew_excl_dollar_one('.$_GET['pending_list_con'].','.$_GET['pending_list_user_new'].');"><b>
		Autobuild (Excl $1)</b></span>
	</td>
											</tr></table>
											 <form method="post" action="pendings.php" accept-charset="UTF-8"  style="margin: 0px;">
				    <input type="hidden" name="subcmd" value="allcheck_update_date" />
...
$sqlcat_siteid = $ilance->db->query("
	 ...
	 if(isset($_GET['check_pcgs']))
	 {
		                        ...
		                        $pcg_de = explode('.',$_GET['check_pcgs']);
	 	                        $pcg_no = $pcg_de['0'];
	 	                        $pcg_gr = $pcg_de['1'];
	 	                        $pcg_dex= explode('d',$pcg_gr);
	 	                        $pcg_gs = $pcg_dex['0'];
	 	                        $final  = $pcg_no.".".$pcg_gs; 
		                        $pcg_exp = explode('.',$final);
								
								$os = str_split($pcg_gr); //5244
								...
								if ((in_array("d", $os)) OR (in_array("D", $os) )) 
								{
									...
									$title_enngg = 'D';
									echo '<input type="hidden"  value="'.$title_enngg.'" id="cat_tit_proo">';
							...
							// murugan changes on dec 13
								...
								echo '<input type="hidden" name="coin_dyear" value="'.$rescat_pcg['coin_detail_year'].'" id="coin_dyear">
									  <input type="hidden" value="'.$myverm.'" id="cdmintmark">
									  <input type="hidden" value="'.$coin_series.'" id="coin_ser_val">
									  <input type="hidden" value="'.$myver.'" id="major_variety">
									  <input type="hidden" name="mypert" value="'.$mypert.'" id="mypert">
									  <input type="hidden" name="catproof" value="'.$text_pro.'" id="catproof">
								      <input type="hidden" name="suff" value="'.$suff.'" id="suff">
									  <input type="hidden" name="title_details" value="'.$title_details.'" id="title_details">
...
if(isset($ilance->GPC['cmd']) AND $ilance->GPC['cmd'] == '_check-user' AND isset($ilance->GPC['userid']) AND $ilance->GPC['userid'] > 0)
{
	$result['status'] = '';

	$sql = $ilance->db->query("SELECT user_id,status FROM " . DB_PREFIX . "users WHERE user_id = '".intval($ilance->GPC['userid'])."'");
