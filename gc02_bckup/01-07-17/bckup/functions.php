...
//kkk
function insertion_fee_transaction_new($cid = 0, $cattype = 'service', $amount = 0, $pid = 0, $userid = 0, $isbudgetrange = 0, $filtered_budgetid = 0, $isbulkupload = false, $stmnt_id = 0, $stmnt_projectid = 0)
{
        ...
        if (isset($location))
        {
                switch ($location)
                {
                        ...
                        case 'paidmethod':
                        {
							...
							if ($ilconfig['cc_candeposit'] AND $fieldname == 'paypal card')
							{
...
function combine_invoice_payment($invoiceid,$accountid)
{
	global $ilance,$ilconfig;

	// if($paymentdate!='')
	// {
	// 	$date = explode('-', $paymentdate);
	// 	if(checkdate($date[1],$date[2],$date[0]))
	// 		$paymentdate = $paymentdate.' '.date('H:i:s');
	// 	else
	// 		$paymentdate = 	DATETIME24H;
	// }
	// else
	// 	$paymentdate = 	DATETIME24H;
		 ...
		 if($ilance->db->num_rows($sql1)>0)
		 {
			 while($line1=$ilance->db->fetch_array($sql1))
			 {
				...
				$ilance->db->query("UPDATE " . DB_PREFIX . "invoices
					SET status = 'paid',paymethod='".$accountid."',paiddate='".$paymentdate."'
					WHERE invoiceid = '" . $line1['invoiceid'] . "'", 0, null, __FILE__, __LINE__);
					
					//buynow order paiddate kkk feb8		
				$ilance->db->query("UPDATE " . DB_PREFIX . "buynow_orders
					SET paiddate='".$paymentdate."',
					winnermarkedaspaid = '1',
					winnermarkedaspaiddate = '".$paymentdate."'
					WHERE invoiceid = '" . $line1['invoiceid'] . "'", 0, null, __FILE__, __LINE__);	
				// update shipment
				$ilance->db->query("insert into "  . DB_PREFIX . "shippnig_details (item_id, coin_id, cust_id, buyer_id, shipper_id,  shipment_date, email,invoice_id,final_invoice_id) value ('".$line['project_id']."','".$line['coin_id']."','".$line['seller_id']."','".$line['buyer_id']."','".$line['shipper_id']."','".$paymentdate."','NO','".$line['invoice_id']."','".$invoiceid."')");
				}else
				{
				$ilance->db->query("UPDATE " . DB_PREFIX . "invoice_projects
					SET status = 'paid'
				...
				$ilance->db->query("UPDATE " . DB_PREFIX . "project_bids
					SET winnermarkedaspaiddate='".$paymentdate."'
					WHERE bidstatus = 'awarded' and user_id = '" . $line1['user_id'] . "' and project_id  = '" . $line1['projectid'] . "'", 0, null, __FILE__, __LINE__);	
				// update shipment
				$ilance->db->query("insert into "  . DB_PREFIX . "shippnig_details (item_id, coin_id, cust_id, buyer_id, shipper_id,  shipment_date, email,invoice_id,final_invoice_id) value ('".$line['project_id']."','".$line['coin_id']."','".$line['seller_id']."','".$line['buyer_id']."','".$line['shipper_id']."','".$paymentdate."','NO','".$line['invoice_id']."','".$invoiceid."')");
...
$ilance->db->query("update ".DB_PREFIX."invoices set status='paid',paymethod='".$accountid."',paiddate='".$paymentdate."',amount=0,paid=totalamount where invoiceid 	='".$invoiceid."'");
$ilance->db->query("update ".DB_PREFIX."invoices set scheduled_date=paiddate where invoiceid 	='".$invoiceid."' and scheduled_date='0000-00-00 00:00:00'");
	}
	...
	{
		global $ilance;
		$result=$ilance->db->query("SELECT * FROM ".DB_PREFIX."logo WHERE ((MONTH(from_date) <= MONTH(CURDATE()) AND DAY(from_date) <= DAY(CURDATE())) AND (MONTH(to_date) >= MONTH(CURDATE()) AND DAY(to_date) >= DAY(CURDATE()))) limit 1");
 		$logo='';
		if($ilance->db->num_rows($result))
		{
		while($row=$ilance->db->fetch_array($result))
...
function EncryptAmount($data, $value)
{
    ...
    return trim(safegc_b64encode($crypttext));
}
