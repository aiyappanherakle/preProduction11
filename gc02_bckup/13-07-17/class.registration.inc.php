                        ...
                        if (!empty($user['username']) )
                        {
                                ...
                                if (!is_array($preferences) OR count($preferences) < 5)
                                {
                                 ...
                                 // Murugan Changes on Apr 28 for Double registration
									$check =  $ilance->db->query("SELECT * FROM " . DB_PREFIX . "users
																WHERE username = '".$user['username']."'");
									if($ilance->db->num_rows($check) == 0 )
									{    

									$chkuserssql = $ilance->db->query("
											SELECT username,first_name,ipaddress,zip_code,last_name,phone,email,address,address2,city,state,status FROM " . DB_PREFIX . "users WHERE (ipaddress = '".$user['ipaddress']."') OR (zip_code = '".$user['zipcode']."' AND last_name = '".$user['last_name']."') OR (phone = '".$user['phone']."')
								...
								$sql_regardlist_pi = $ilance->db->query("           
									SELECT i.invoiceid FROM " . DB_PREFIX . "invoices i
								...
								//email notify condition for check box in the registration 
								if($user['optin'] == 1 )
								{
								...
								$sqluser= $ilance->db->query("
                                                        UPDATE " . DB_PREFIX . "users_unregistered
                                                        SET user_id = '" .$member['userid']. "'
                                                         WHERE email = '".$ilance->db->escape_string($user['email'])."' AND first_name= '" . $ilance->db->escape_string($user['first_name']) . "' AND last_name= '" . $ilance->db->escape_string($user['last_name']) . "'");
								if($user['referal_name'] != '')
								{
							   // Murugan Changes On Feb 17 For Refferal name Add							   
							   $selref = $ilance->db->query("SELECT * FROM " . DB_PREFIX . "referal_id WHERE referalcode = '".$user['referal_name']."' ");
