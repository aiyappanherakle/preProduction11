...
if (!empty($_SESSION['ilancedata']['user']['userid']) AND $_SESSION['ilancedata']['user']['userid'] > 0 AND $_SESSION['ilancedata']['user']['isstaff'] == '1')
{
								 ...
								 if($ilance->db->num_rows($sqlcat_coin_detail) > 0)
										...
										{	
										 ...
										 else
										 {
											 ...
											 {
											 $certified = '<strong>Certified Coin</strong> <a href="consignments.php?cmd=add_single_coin&subcmd=update_edit&user_id='.$ilance->GPC['user_id'].'&consignid='.$ilance->GPC['consignid'].'&pro='.$row_coin_list['coin_id'].'&coin_id='.$row_coin_list['coin_id'].'">(Update)</a>';
											 }
											 else
											 {
										...
										$row_coin_list['desci'] = '<span class="blue"><a href="consignments.php?cmd=add_single_coin&subcmd=update_edit&user_id='.$ilance->GPC['user_id'].'&consignid='.$ilance->GPC['consignid'].'&pro='.$row_coin_list['coin_id'].'&coin_id='.$row_coin_list['coin_id'].'">'.$row_coin_list['Title'].'</a></span>';	
												
									    $row_coin_list['coinid'] = 	'<span class="blue"><a href="consignments.php?cmd=add_single_coin&subcmd=update_edit&user_id='.$ilance->GPC['user_id'].'&consignid='.$ilance->GPC['consignid'].'&pro='.$row_coin_list['coin_id'].'&coin_id='.$row_coin_list['coin_id'].'">'.$row_coin_list['coin_id'].'</a></span>';	
    ...
    {        
		        ...
		        $final_value_fee = $listing_fee = false;
                       ...
                       $sql2 = $ilance->db->query("
                        ...
                        if ($ilance->db->num_rows($sql2) > 0)
                        {
                                ...
                                while ($res = $ilance->db->fetch_array($sql2))
                                {
									...
									{
										$listing_fee = true;
										$list_id = getif_drop_down('listingfee','listingfee',$res['listing_fee']);
									}
	...
	if (isset($ilance->GPC['subcmd']) AND $ilance->GPC['subcmd'] == 'search_now')
	{
	 ...
	 if(empty($ilance->GPC['pcgs']) and empty($ilance->GPC['subcmd']))
	 {
		...
		while($search_result = $ilance->db->fetch_array($query_search))
			{
$search_result['desci'] = '<span class="blue"><a href="consignments.php?cmd=add_single_coin&subcmd=update_edit&user_id='.$search_result['user_id'].'&consignid='.$search_result['consignid'].'&pro='.$search_result['coin_id'].'&coin_id='.$search_result['coin_id'].'">'.$search_result['Title'].'</a></span>';	
								$search_result['pc'] = '<span class="blue">'.$search_result['pcgs'].'</span>';				
									    $search_result['coinid'] = 	'<span class="blue"><a href="consignments.php?cmd=add_single_coin&subcmd=update_edit&user_id='.$search_result['user_id'].'&consignid='.$search_result['consignid'].'&pro='.$search_result['coin_id'].'&coin_id='.$search_result['coin_id'].'">'.$search_result['coin_id'].'</a></span>';
	   ...
	   $counter = ($ilance->GPC['page'] - 1) * 25;
		...
		while($search_result = $ilance->db->fetch_array($query_search))
			...
			$search_result['desci'] = '<span class="blue"><a href="consignments.php?cmd=add_single_coin&subcmd=update_edit&user_id='.$search_result['user_id'].'&consignid='.$search_result['consignid'].'&pro='.$search_result['coin_id'].'&coin_id='.$search_result['coin_id'].'">'.$search_result['Title'].'</a></span>';	
									//$search_result['pc'] = '<span class="blue"><a href="catalog.php?cmd=update-catalog&subcmd=update-coinlist&id='.fetch_cat('id',$search_result['pcgs']).'">'.$search_result['pcgs'].'</a></span>';				
									 $search_result['pc'] = '<span class="blue">'.$search_result['pcgs'].'</span>';				
									       $search_result['coinid'] = 	'<span class="blue">'.$search_result['coin_id'].'</span>';
             if($search_result['Site_Id'] == '0')
			 $search_result['er'] = 'Listed in GC';
			 else
			 $search_result['er'] = fetch_user_siteid('site_name',$search_result['Site_Id']);
    ...
    if (isset($ilance->GPC['cmd']) AND $ilance->GPC['cmd'] == 'consign_edit')
	{
	               ...
	               //con date default
		           	$con_date_end = $ilance->db->query("
								SELECT date(end_date) as tese FROM " . DB_PREFIX . "consignments WHERE user_id='".$ilance->GPC['user_id']."' AND consignid ='".$ilance->GPC['consignid']."'", 0, null, __FILE__, __LINE__);
...
{	
   ...
   {
   case "email":
	   $whereclass=" where u.email='".$ilance->db->escape_string($filtervalue)."'";
   break;
   case "user_id":
	   $whereclass=" where u.user_id='".$ilance->db->escape_string($filtervalue)."'";
   break;
   case "username":
	   $whereclass=" where u.username='".$ilance->db->escape_string($filtervalue)."'";
   break;
   case "zip_code":
	   $whereclass=" where u.zip_code='".$ilance->db->escape_string($filtervalue)."'";
   break;
   case "consignid":
	   $whereclass=" where cn.consignid='".$ilance->db->escape_string($filtervalue)."'";
   break;
   default:
		$whereclass='';
   break;
    ...
    $SQL="SELECT cn.consignid,cn.coins,cn.consign_type,c.coincount,u.user_id,u.first_name,u.last_name,u.username,u.email,
			DATE_FORMAT(cn.receive_date,'%b %d %Y') as receive_date,c.consignment_cost,msgcount
			FROM " . DB_PREFIX . "consignments cn 
			left join " . DB_PREFIX . "users u on u.user_id=cn.user_id
			left join (SELECT * , count( o.message ) AS msgcount from (select * from " . DB_PREFIX . "consignment_messages  order by id desc) o group by consignment_id ) m on m.consignment_id=cn.consignid
			left join (select count(coin_id) as coincount,sum(cost*Quantity) as consignment_cost,consignid 
	...
	$ilpage['consignments']=HTTP_SERVER.$_SERVER['SCRIPT_NAME'];		
	...
	$scriptpageprevnext = $ilpage['consignments']. '?cmd=listing'.$query_string;
		...
		if($ilance->db->num_rows($result))
		{
			while($line=$ilance->db->fetch_array($result))
			{
				...
				$row_list['consign_type'] = get_consing_type($line['consign_type']);				
				...
				$row_list['add_new_note'] = '<span style="cursor:pointer; " class="blue" ><a href="consignments_fast.php?cmd=coin_list&user_id='.$line['user_id'].'&consignid='.$line['consignid'].'&msgcmd=new">Add New Note</a></span>';	
				
				if($line['msgcount']==1)
				{

				$row_list['view_note'] = '<span style="cursor:pointer;" class="blue" > <a href="consignments_fast.php?cmd=coin_list&user_id='.$line['user_id'].'&consignid='.$line['consignid'].'">View Note</a></span>';
				
				}
				else if($line['msgcount']>1)
				{
				
				$row_list['view_note'] = '<span style="cursor:pointer;" class="blue" > <a href="consignments_fast.php?cmd=coin_list&user_id='.$line['user_id'].'&consignid='.$line['consignid'].'">View Notes</a></span>';
