...
//Karthik may 04
		...
		if(isset($ilance->GPC['order_by']) AND $ilance->GPC['order_by'] == 'coin_id')
		{
		 ...
		 {
		 $sql="SELECT c.coin_id,c.site_id, c.Title, c.Certification_No,c.Buy_it_now,c.Minimum_bid,c.pcgs,
...
//cs.coin_series_sort,cc.coin_detail_year asc,cd.denomination_sort,
					$listing_items = '<table border="1"><tr><td>Coin_id</td><td>Site</td><td>Sold</td><td>Title</td><td>PCGS</td><td>Certificate No</td><td>Min Bid</td><td>Buy Now</td><td>Bids</td><td>APR</td><td>Total</td></tr>';
...
// print_r($listing);exit;								
							...
							//c.Grade, c.Grading_Service, c.Cac, c.Star, c.plus, 
						             $total =  $ilance->currency->format($cp+$bp);
									 if($listing['site_id']==0)
										 $site_name='Gc';
									 else
										 $site_name='EBay';
									$listing_items.= '<tr><td>'.$projectid.'</td><td>'.$site_name.'</td><td>'.$sold.'</td><td>'.$projecttitle.'</td><td>'.$listing['pcgs'].'</td><td>'.$certnum.'</td><td>'.$current_price.'</td><td>'.$buynow_price.'</td><td>'.$no_bids.'</td><td>'.$coin_apr.'</td><td>'.$total.'</td></tr>';
							...
							if($ilance->db->num_rows($return_list)>0)
						    {
					            while($listing = $ilance->db->fetch_array($return_list))
									...
									$listing_items.= '<tr><td>'.$projectid.'</td><td></td><td>'.$sold.'</td><td>'.$projecttitle.'</td><td>'.$listing['pcgs'].'</td><td>'.$certnum.'</td><td>'.$current_price.'</td><td>'.$buynow_price.'</td><td>'.$no_bids.'</td><td>'.$coin_apr.'</td><td>'.$total.'</td></tr>';
...
function get_coin_history_price($coin_id,$pcgs,$grade,$grading_service,$cac,$star,$plus,$seried_id,$denomination_id)
 ...
 $query="SELECT project_id,date_end,grade,grading_service,currentprice  FROM " . DB_PREFIX . "projects WHERE 
 cid='$pcgs' 
 and date_end>= DATE_SUB(NOW(),INTERVAL 1 YEAR)
 and status='expired' 
 and grade='".$grade."'
 and project_title not like '%DETAILS%'
 and (haswinner=1 or hasbuynowwinner=1)  
 order by 
 case when grading_service='".$grading_service."' then 1 else 2 end, 
 date_end desc, currentprice desc limit 3";
