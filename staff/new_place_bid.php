...
if(isset($ilance->GPC['cmd']) and $ilance->GPC['cmd']=='place_fixed_bid_update')
{
	if (isset($ilance->GPC['place_next_id']) AND is_array($ilance->GPC['place_next_id'])  AND count($ilance->GPC['place_next_id'])>0 ) 
	{
	    ...
	    $alrpdlst_already_placed=array();
		foreach($numbers as $number) 
		{						 
				$ipaddress      = IPADDRESS;
				$createdate     = DATETIME24H;	

				$alredy_SQL="SELECT project_id
				FROM " .DB_PREFIX."project_bids
				WHERE user_id =28 and
				project_id = " . intval($number). "
				GROUP BY project_id
				";
				...
				{ 					
				   ...
				   $alrpdlst_already_placed[]= $number;
			    }
			    else
			    {
		...
		if(count($alrpdlst_already_placed)>0)
		{
	       $details_already_list = "<br>".count($alrpdlst_already_placed)." No of coins already Bided in for 2222 account.";     
		}

		$details_conform= isset($details)?$details:'';
		$details_conform_already_list= isset($details_already_list)?$details_already_list:'';
		if(count($total_coin_list)>0)
			{
				print_action_success("Task have been successfully completed.<br>".count($total_coin_list)." No of bid will be place by a cron job.".$details_conform."".$details_conform_already_list."", $_SERVER['PHP_SELF']);exit();	
			}else
			{
...
elseif(isset($ilance->GPC['cmd']) and $ilance->GPC['cmd']=="place_bids" and isset($ilance->GPC['Submit_Fixed']))
{
	...
	}else
	{
		...
		if($error != 0) 
		{
 		    print_action_failed("Please Check seperate the Item IDs by comma OR You can use a range my mentioning x to y, for ex., 23330 through 23340 .", $_SERVER['PHP_SELF']);exit();
		}
	    ...
	    $SQL="SELECT p.project_id,p.project_title,p.bids,p.startprice,p.currentprice,date_format(p.date_end,'%b %d %Y %h:%i %p') as date_end FROM " . DB_PREFIX . "projects p
			...
			else
			{
				if($tot_coins > 0)
				{
					...
					$alrpd = implode(",",$alrpdlst);
					
					$notcrpb = $var12;
					
					if(count($alrpdlst) > 0)
					{
                       $alrpddetails = "<br>Already having bid ".$alrpd."";
					}
					if(count($notlst) > 0)
					{
                      $notcrpbdetails = "Added Not Listed coins in cron job:- ".$notcrpb."";
                     
				    }
				     $f_alrpddetails = isset($alrpddetails)?$alrpddetails:'';
				     $f_notcrpbdetails = isset($notcrpbdetails)?$notcrpbdetails:'';
				    print_action_success("".$f_notcrpbdetails."".$f_alrpddetails."", $_SERVER['PHP_SELF']);
					exit();
				}
				else
				{
	...
	$number_not_list_final = isset($var1)?$var1:'';
					...
					$nns = 1;
		            $var12s='';
 					foreach($total_coin_not_list as $number_list) 
					{			
								$ipaddress      = IPADDRESS;
								$createdate     = DATETIME24H;
					...
					$alrpds = implode(",",$alrpdlsts);
					
					$notcrpbs = $var12s;
					
					if(count($alrpdlsts) > 0)
					{
                       $alrpddetailss = "<br>Already having bid ".$alrpds."";
					}
					if(count($notlsts) > 0)
					{
                      $notcrpbdetailss = "Added Not Listed coins in cron job:- ".$notcrpbs."";
                     
				    }
				     $f_alrpddetailss = isset($alrpddetailss)?$alrpddetailss:'';
				     $f_notcrpbdetailss = isset($notcrpbdetailss)?$notcrpbdetailss:'';
 



	$result_Not_list    = $f_notcrpbdetailss."".$f_alrpddetailss;
