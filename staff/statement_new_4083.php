...
$last_listed_date="select CASE WHEN count(actual_end_date)>0 THEN date_format(max(actual_end_date),'%Y-%m-%d %H:%i:%s')  ELSE '0000-00-00 00:00:00' END as last_listed_time from ilance_coin_relist 
...
left join 
	...
	where 
	date(actual_end_date)<='".$stmt_date."' and 
	user_id='".$user_details['user_id']."' and 
	DATEDIFF(enddate,startbydate)>1  group by coin_id) r on r.coin_id=c.coin_id
left join
	(select sum(qty) as order_count,project_id,orderdate,item_end_date from ".DB_PREFIX."buynow_orders o where owner_id='".$user_details['user_id']."' and date(item_end_date)='".$stmt_date."' group by project_id) o on c.coin_id=o.project_id 
left join
...
left join
	(
		...
		from ".DB_PREFIX."invoices i where 
		(
			i.user_id=".$user_details['user_id']." or 
			i.p2b_user_id=".$user_details['user_id']."
		) and 
			date(i.createdate)<='".$stmt_date."' and 
			i.createdate>(".$last_listed_date.")  
		group by i.projectid
	) i on c.coin_id=i.projectid 	
left join ".DB_PREFIX."catalog_coin cat on cat.PCGS=c.pcgs
where  
(
	c.user_id=".$user_details['user_id']." and 
	(
		(
			date(r.last_listed_time)>='".$start."' and 
			date(r.last_listed_time)<='".$stmt_date."' and 
			date(o.item_end_date)='".$stmt_date."'
			
		) or 
		(
			date(c.End_Date)>='".$start."' and 
			date(c.End_Date)<='".$stmt_date."' and 
			(
				c.project_id>0 or c.relist_count>0 
			)
		) or
		(
...
//		$html.='<td>'.$sl.'</td>';
		...
		$html.='<td width = "80%">'.$coins_list_line['Title'].'<br />Cert #: '.$coins_list_line['Certification_No'].'&nbsp;&nbsp;';
		...
		$html.='<tr><td colspan=8 align="right">Advance</td><td>'.$ilance->currency->format_real_no($advance_received).'</td></tr>';
		$html.='<tr><td colspan=8 align="right">Miscellaneous</td><td>'.$ilance->currency->format_real_no($miscellanious_debit-$miscellanious_credit).'</td></tr>';
		$html.='<tr><td colspan=8 align="right">Paid</td><td>'.$ilance->currency->format_real_no(0).'</td></tr>';
		$html.='<tr><td colspan=8 align="right">Balance</td><td>'.$ilance->currency->format_real_no($statement_total).'</td></tr>';
		$html.='<tr><td colspan=9>Thank you for consigning to GreatCollections.<br>We appreciate your business.</td></tr></table>';
		$p->AddPage();
		$p->setfont('times','',10);		
	//	echo $html;
	$p->htmltable($html);	
	...
	$html='<table><tr>
			...
			<td>Total Auction price</td><td>'.$ilance->currency->format_real_no($grand_statement_auction_price_total).'</td></tr><tr>
			<td>Total Buynow Price</td><td>'.$ilance->currency->format_real_no($grand_statement_buynow_total).'</td></tr><tr>
			<td>Total Buyer Fees</td><td>'.$ilance->currency->format_real_no($grand_statement_buyer_fee_total).'</td></tr><tr>
...
$p->output($stmt_date.'corrected_Statement_new_'.DATETIME24H.'.pdf','D');
...
<tr >
<td>&nbsp;Consignor Username : '.$name.' <br>E-mail : '.$email;
