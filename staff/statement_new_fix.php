...
$user_where=isset($ilance->GPC['user_id'])?'  WHERE  u.user_id='.$ilance->GPC['user_id']:'';
 ...
 if(isset($ilance->GPC['user_id']) and $ilance->GPC['user_id']>0)
		...
		FROM " . DB_PREFIX . "users u
		 ".$user_where." 
		GROUP BY u.user_id order by u.first_name,u.last_name";
		}
		else
		{
		...
		count( DISTINCT e.coin_id) AS ebay_count,
		...
		FROM " . DB_PREFIX . "users u
		LEFT JOIN " . DB_PREFIX . "coins c ON u.user_id = c.user_id and date(c.End_Date)='".$stmt_date."'
		LEFT JOIN " . DB_PREFIX . "coin_relist r ON u.user_id = r.user_id and date(r.actual_end_date)='".$stmt_date."'
		LEFT JOIN " . DB_PREFIX . "buynow_orders b ON u.user_id = b.owner_id and date(b.orderdate)>='".$start."' and date(b.orderdate)<='".$stmt_date."' 
		LEFT JOIN " . DB_PREFIX . "user_advance a ON u.user_id = a.user_id and a.statusnow = 'paid' and date(a.date_made)<='".$stmt_date."' and date(a.date_made)>='".$start."'
		LEFT JOIN " . DB_PREFIX . "invoices i ON i.user_id = u.user_id and i.ismis =1 and date(i.createdate)<='".$stmt_date."' and date(i.createdate)>='".$start."'
		LEFT JOIN " . DB_PREFIX . "ebay_listing e ON e.user_id = u.user_id and date(e.end_date)<='".$stmt_date."' and date(e.end_date)>='".$start."'

		 ".$user_where." 
		GROUP BY u.user_id having coin_count>0 or relist_coin_count>0 or buynow_coin_count>0 
		order by u.first_name,u.last_name";
}
$user_query=$ilance->db->query($sql, 0, null, __FILE__, __LINE__);


if($seller_count=$ilance->db->num_rows($user_query))
{
while($user_details=$ilance->db->fetch_array($user_query))
	...
	//from coin relist table
	 ...
	 $prime_sql=$ilance->statement->statement_query_fix($user_details['user_id'],$start,$stmt_date);
			...
			switch ($coins_list_line['filtered_auctiontype'])
				...
				case 'regular':
					...
					*/
					...
					$no_buyer[]=$coins_list_line['winner_user_id'];
					if($coins_list_line['winner_user_id'] > 0)
					{
					if(!in_array($coins_list_line['winner_user_id'],$no_buyer))
					{
					 $no_buyer[]=$coins_list_line['winner_user_id'];
					}
					}
				...
				default:
					...
					$no_buyer[]=$coins_list_line['hasbuynowwinner'];
					if($coins_list_line['hasbuynowwinner'] > 0)
					{
					if(!in_array($coins_list_line['hasbuynowwinner'],$no_buyer))
					{
					 $no_buyer[]=$coins_list_line['hasbuynowwinner'];
					}
					}
...
//		$html.='<td size="9">'.$sl.'</td>';
		...
		$html.='<td size="9">'.$coins_list_line['site_name'].'</td>';
	...
	$html='<table><tr>
			...
			<td size="9">Total Net to Consignor</td><td>'.$ilance->currency->format_real_no($grand_statement_seller_total).'</td></tr><tr>
			<td size="9">Total Sales (Auction Price,  Buy Now Price,  Buyers Fee)</td><td>'.$ilance->currency->format_real_no($grand_statement_auction_price_total+$grand_statement_buynow_total+$grand_statement_buyer_fee_total).'</td></tr><tr>
			<td size="9">Average $ Selling Item:</td><td>'.$ilance->currency->format_real_no($grand_statement_seller_total).'</td></tr><tr>
			<td size="9">Total '.$buyer_count.' of Unique Buyers</td></tr><tr>
			<td size="9">Total '.$seller_count.' of Unique Consignors</td></tr><tr>
			</table>';
			$p->AddPage();
		$p->setfont('times','',10);	
	//	echo $html;	
