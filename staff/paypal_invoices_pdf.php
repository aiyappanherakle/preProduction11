...
$sql_bit='';
if(isset($ilance->GPC['start_date']) and isset($ilance->GPC['end_date']))
{
	$startDate = $ilance->GPC['start_date'];	
	$endDate = $ilance->GPC['end_date'];
	$sql_bit="combine_project != ''
							AND (createdate <= '" . $endDate . "' AND createdate >= '" . $startDate . "')
							AND status = 'paid'
							AND paymethod IN ('paypal_cc','paypal','paypal card','card')  ";
							
}
else if(isset($ilance->GPC['invoice_list']))
{
	$sql_bit=' invoiceid in ('.$ilance->GPC['invoice_list'].')';
}
	...
	// Paypal Multi Invoice Print
	$get_paypal_paid_invoice = "SELECT invoiceid
							FROM ".DB_PREFIX."invoices
							WHERE ".$sql_bit."
							order by paiddate asc ";							

$result_paypal_paid_invoice = $ilance->db->query($get_paypal_paid_invoice, 0, null, __FILE__, __LINE__) ;

if($ilance->db->num_rows($result_paypal_paid_invoice) > 0)
{
	while($get_paypal_paid_invoiceid = $ilance->db->fetch_array($result_paypal_paid_invoice))
	{

		//$get_paypal_paid_invoiceid['invoiceid'].'<br/>';
		//remove last blank page
			...
			$invoice_id = $get_paypal_paid_invoiceid['invoiceid'];
			$sql=$ilance->db->query("select user_id,combine_project,taxamount,taxinfo,paid,miscamount,totalamount,status,createdate,paiddate,amount  
				from " . DB_PREFIX . "invoices 
			...
			while($line=$ilance->db->fetch_array($sql))
			{
				...
				if($line['paid']>0)
				{
					$query="SELECT *  FROM ".DB_PREFIX."partial_payment 
							WHERE invoiceid = '".$ilance->GPC['id']."' ORDER BY paymentdate";
			...
			if(count($invoice_list))
			{
				...
				$sql=$ilance->db->query("select * from ".DB_PREFIX."invoices 
					where invoiceid  in ($inv) ORDER BY projectid ASC ");
				...
				while($line1=$ilance->db->fetch_array($sql))
				{
					...
					$item='<a href="consignments.php?cmd=add_single_coin&subcmd=update_edit&coin_id='.$line1['projectid'].'">'.$ilance->db->fetch_field(DB_PREFIX . "projects", "project_id = '".$line1['projectid']."'", "project_title").'</a>';
					       $invoiceid = $line1['invoiceid'];

					$html.= '<tr>
							  <td  family="helvetica" size="10">'.$quantity.'</td><td nowrap family="helvetica" size="10">'.$itemid.'</td><td  family="helvetica" size="10">'.$item.'</td><td nowrap family="helvetica" size="10">'.$amount.'</td><td nowrap family="helvetica" size="10">'.$buyerfees.'</td><td nowrap family="helvetica" size="10">'.$tot.'</td>
							  </tr>';
					//$invoicelist[]=$line1;
					$r_temp+=$total;
				}
				...
				$sql=$ilance->db->query("select * from " . DB_PREFIX . "invoice_projects 
				...
				$sql_shipper_detail=$ilance->db->query("select * from ".DB_PREFIX."shippers 
			...
			{
			 	$added_html=' <tr ><td colspan=3> </td>  <td align="right"  nowrap color="#000000" family="helvetica" size="10">Upgraded Shipping: </td><td  align="left"  nowrap family="helvetica" size="10">'.$shipper_title_upgraded.'</td>
	  </tr>';
			}
			else
			{
...
<tr ><td colspan=3> </td>
<td  align="right"  nowrap color="#000000" family="helvetica" size="10">SALE AMT: </td><td  align="left"  nowrap family="helvetica" size="10">'.$ilance->currency->format(array_sum($totalnew)).'</td>
  </tr>
<tr > <td colspan=3> </td> <td align="right"  nowrap color="#000000" family="helvetica" size="10">SHIPPING: </td><td  align="left"  nowrap family="helvetica" size="10">'. $ilance->currency->format($shippping_cost).'</td>
  </tr>';
  if($show['taxamount']== true)
  {
  $table1 .='<tr ><td colspan=3> </td> <td align="right"  nowrap color="#000000" family="helvetica" size="10">TAX: </td><td  align="left"  nowrap family="helvetica" size="10">'.$taxamount.'</td>
  </tr>';
  }
  if($show['discount']== true)
  {
  $table1 .='<tr ><td colspan=3> </td> <td align="right"  nowrap color="#000000" family="helvetica" size="10">PROMOCODE: </td><td  align="left"  nowrap family="helvetica" size="10">'."US$".$discount.'</td>
  </tr>';
  }
  
  if($show['miscamount']== true)
  {
  $table1 .='<tr ><td colspan=3> </td>  <td align="right"  nowrap color="#000000" family="helvetica" size="10">Misc Amount: </td><td  align="left"  nowrap family="helvetica" size="10">'.$miscamount.'</td>
  </tr>';
  }
  $table1 .='<tr ><td> </td>
  <td rowspan="2" valign=middle align=center  border=1> 
Consign your coins with GreatCollections<br>
www.greatcollections.com/selling</td>
  <td></td>  <td align="right"  nowrap color="#000000" family="helvetica" size="10">TOTAL: </td><td  align="left"  nowrap family="helvetica" size="10">'.$totalamount.'</td>
  </tr>
  <tr ><td colspan=3> </td>  <td align="right"  nowrap color="#000000" family="helvetica" size="10">PAID: </td><td  align="left"  nowrap family="helvetica" size="10">'.$ilance->currency->format($total_paid).'</td>
  </tr>';
  if($show_pending)
  {
    $table1 .='
	  <tr ><td colspan=3> </td>  <td align="right"  nowrap color="#000000" family="helvetica" size="10">Amount Due: </td><td  align="left"  nowrap family="helvetica" size="10">'.$ilance->currency->format($total_due_amount).'</td>
	  </tr>';
  }
