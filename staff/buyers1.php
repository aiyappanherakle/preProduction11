...
if($line['paid']>0)
{
	...
	if($ilance->db->num_rows($result))
	{
		...
		$show['paid_payment_rows']=false;
		while($lin = $ilance->db->fetch_array($result))
		{
			echo '<pre>';
			
			$linees['partial_amount']=$ilance->currency->format($lin['partial_amount']);
			$linees['paymethod']=$lin['paymethod'];
			$linees['payment_date']=print_date($lin['paymentdate']);
			$linees['edit']='<a href="buyers.php?cmd=edit_partial_payment&payment_id='.$lin['id'].'"><img src="../images/gc/icons/pencil.gif"/></a>';
			
			$partial_payment_rows[]= $linees;
...
$new_ship_project = implode(",",$project_id_ship);
	if ($new_ship_project != '')
	{
		$ship_sel = $ilance->db->query("SELECT track_no,shipment_date FROM ".DB_PREFIX."shippnig_details WHERE coin_id IN (".$new_ship_project.") AND track_no != '' AND buyer_id = '".$buyer_id."'GROUP BY track_no ORDER BY track_no DESC");
...
/*$sqlamt = $ilance->db->query("
	...
	$ilance->template->fetch('main', 'buyers_invoice_test.html', 2);
	$ilance->template->parse_hash('main', array('ilpage' => $ilpage));
	$ilance->template->parse_if_blocks('main');
	$ilance->template->parse_loop('main', array('invoicelist','buyerdetail'));
	$ilance->template->parse_loop('main', array('partial_payment_rows'));
