...
left join (select user_id,sum(amount) as scheduledamount ,sum(paid) as paratialamount from ". DB_PREFIX . "invoices where status='scheduled' group by user_id) i on u.user_id=i.user_id
left join (
...
group by u.user_id ORDER BY  sortorder  DESC";
	 ...
	 $sql = $ilance->db->query($first_sql);					
		...
		<tr><td>User ID</td><td  width="50">User Name</td><td>Date Joined</td><td>Unpaid <br>Invoices</td><td>Oldest<br> Date<br>(Before<br> '.date('m/d/Y',strtotime(FIFETEENDAYSAGO)).')</td><td>Latest<br> Date<br>( AFTER <br>'.date('m/d/Y',strtotime(FIFETEENDAYSAGO)).')</td><td>Total Amount</td></tr>';				
