...
if (!empty($_SESSION['ilancedata']['user']['userid']) AND $_SESSION['ilancedata']['user']['userid'] > 0 AND $_SESSION['ilancedata']['user']['isadmin'] == '1')
{
	...
	else if(isset($ilance->GPC['cmd']) AND $ilance->GPC['cmd'] == 'live_item_list_pcgs_ngc')
	{
		...
		while($res = $ilance->db->fetch_array($sql))
		{
			...
			$filename = '/home/gc/public_html/'.$ilconfig['email_image_folder_v2'].'/'.$res['Item'].'-1.jpg';
			if (file_exists($filename)) 
			{
				...
				{
				$uselistra = HTTP_SERVER.$ilpage['attachment'] . '?cmd=thumb&subval=acc_v1&subcmd=itemphoto&id=' . $fetch_newa['filehash'] .'&w=268&h=362'; 			
				} 
			  $uselistra = HTTP_SERVER.$ilconfig['email_image_folder_v2'].'/'.$res['Item'].'-1.jpg';
			...
			{   
				...
				{	
				 $uselistra = HTTP_SERVER.$ilpage['attachment'] . '?cmd=thumb&subval=acc_v1&subcmd=itemphoto&id=' . $fetch_series['filehash'] .'&w=268&h=362'; 
			...
			//echo $uselistra.'<br/>'; //exit;
			$user_detail['filehash']= $uselistra;
			$user_detail['TITLE']=$res['TITLE'];
			$user_detail['URL']=$res['URL'];
