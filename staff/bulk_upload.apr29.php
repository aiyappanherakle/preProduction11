...
if(count($files_list)>0)
{
		...
		for($i=0;$i<count($files_list);$i++)
			...
			$attachment_serial_no=substr($c,strpos($c,"-")+1,strpos($c,".")-strpos($c,"-")-1);;
				...
				/*echo "insert into ".DB_PREFIX."attachment ( attachtype, category_id, date, filename, filetype, visible, filesize, filehash, coin_id, project_id) values ( 'slideshow', ".$category_id.", '".DATETIME24H."', '".$file_name."', '".$filetype."', 1, '".$filesize."', '".$hash."','".$coin_id."','".$coin_id."')";
				*/
				$ilance->db->query("insert into ".DB_PREFIX."attachment ( attachtype, category_id, date, filename, filetype, visible, filesize, filehash, coin_id, project_id) values ( 'slideshow', ".$category_id.", '".DATETIME24H."', '".$file_name."', '".$filetype."', 1, '".$filesize."', '".$hash."','".$coin_id."','".$coin_id."')");
				if($is_item_photo)
				{
				  $ilance->db->query("update ".DB_PREFIX."attachment set attachtype = 'itemphoto' where filehash = '".$hash."'");
				}
			...
			$ilance->db->query("update ".DB_PREFIX."attachment set filesize='".$filesize."', filehash='".$hash."', filename='".$file_name."', date ='".DATETIME24H."' where filename='".$file_name."'");
				if(!is_dir(DIR_AUCTION_ATTACHMENTS.floor($coin_id/100).'00/'.$coin_id))
				mkdir(DIR_AUCTION_ATTACHMENTS.floor($coin_id/100).'00/'.$coin_id,0777,true);
