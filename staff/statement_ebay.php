...
$user_where=isset($ilance->GPC['user_id'])?'  WHERE  u.user_id='.$ilance->GPC['user_id']:'';
 ...
 if(isset($ilance->GPC['user_id']) and $ilance->GPC['user_id']>0)
		...
		LEFT JOIN ilance_ebay_listing e ON e.user_id = u.user_id and date(e.end_date)<='".$stmt_date."' and date(e.end_date)>='".$start."'
...
from 
	".DB_PREFIX."coins c 
	left join ilance_ebay_listing e on e.coin_id=c.coin_id
	left join (select sum(quantity) as ebay_order_count ,coin_id from ilance_ebay_listing_rows where  date(enddate) between date('".$start."') and  date('".$stmt_date."') group by coin_id) el on el.coin_id=c.coin_id
left join
