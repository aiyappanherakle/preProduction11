...
$sql=$ilance->db->query("select user_id,combine_project,miscamount,taxamount,taxinfo,amount,paid,totalamount,status from ".DB_PREFIX."invoices where invoiceid='".$invoice_id."'");
...
while($line1=$ilance->db->fetch_array($sql))
	{
		...
		$line1['item']='<a href="consignments.php?cmd=add_single_coin&subcmd=update_edit&coin_id='.$line1['projectid'].'">'.$ilance->db->fetch_field(DB_PREFIX . "projects", "project_id = '".$line1['projectid']."'", "project_title").'</a>';
		$invoicelist[]=$line1;
	}
}	
$sql = $ilance->db->query("
...
if(isset($ilance->GPC['cmd']) AND $ilance->GPC['cmd'] == 'buyer' AND isset($ilance->GPC['subcmd']) AND $ilance->GPC['subcmd'] == 'paid_not_shipped'){
	...
	$not_shipped_sql_1 = $ilance->db->query("select i.invoiceid,i.combine_project,i.totalamount,DATE_FORMAT(i.paiddate,'%m-%d-%Y') as paid_date,u.username,u.first_name,u.last_name
	...
	// echo $ilance->db->num_rows($not_shipped_sql_1);
	// exit;
	$if=0;
	$else = 0;
	$html_csv = "INVOICE_ID,PAID DATE,USER ID,FIRST NAME,LAST NAME,SHIPPED Y/N,TRACKING NUMBER <br/>";
	while($not_shipped_res_1=$ilance->db->fetch_array($not_shipped_sql_1)){
	// echo '<pre>';
	// print_r($not_shipped_res_1);
	// exit;
		...
		$not_shipped_sql = $ilance->db->query("select s.coin_id,s.track_no from
".DB_PREFIX."invoices i left join
".DB_PREFIX."shippnig_details s  on s.coin_id=i.projectid AND s.cust_id>0
where s.track_no !='' and i.invoiceid='".$one_paid_item."' 
										LIMIT 0,1	");
		$not_shipped['invoice_id']=$not_shipped_res_1['invoiceid'];
		$not_shipped['paid_date']=$not_shipped_res_1['paid_date'];
		$not_shipped['username']=$not_shipped_res_1['username'];				
		$not_shipped['first_name']=$not_shipped_res_1['first_name'];
		$not_shipped['last_name']=$not_shipped_res_1['last_name'];
		// echo "select s.track_no from
// ".DB_PREFIX."invoices i left join
// ".DB_PREFIX."shippnig_details s  on s.coin_id=i.projectid
// where i.invoiceid='".$one_paid_item."'
											// ";
											// echo '<br/><br/>';
		...
		if($ilance->db->num_rows($not_shipped_sql)>0){
			...
			while($proj_shipd=$ilance->db->fetch_array($not_shipped_sql))
			{
				//echo $ilance->db->num_rows($not_shipped_sql).'<br/>';
				if($proj_shipd['track_no'] != '')
				{
					
					$not_shipped['shipped']='Yes';
					$not_shipped['trackno']="'".$ilance->db->escape_string($proj_shipd['track_no'])."'";
					$if++;
				}
		...
		// {
			...
			// $not_shipped['shipped']='No Shipping Required';
			// $not_shipped['trackno']='NO Track No';
		// }
		$not_shipped_arr[]=$not_shipped;
		$html_csv .= $not_shipped_res_1['invoiceid'].",".$not_shipped_res_1['paid_date'].",".$not_shipped_res_1['username'].",".$not_shipped_res_1['first_name'].",".$not_shipped_res_1['last_name'].",".$not_shipped['shipped'].",".$not_shipped['trackno']."<br/>";
	}
	echo $html_csv;
	exit;
	// echo $if.'<br/>';
	// echo $else.'<br/>';
	// echo count($not_shipped_arr);exit;
	$fields = array(
				array('Invoice_id', 'Invoice_id'),
				array('paid_date', 'Paid Date'),
				array('user_id', 'User ID'),
				array('first_name', ',First Name'),
				array('last_name', 'Last Name'),
				array('shipped', 'Shipped Y/N'),
				array('trackno', 'Tracking Number')
								);
				...
				$reportoutput = $ilance->admincp->construct_csv_data($not_shipped_arr, $headings);
				$timeStamp = date("Y-m-d-H-i-s");
				$fileName = "Invoice-$timeStamp";
				$action = 'csv';
					if ($action == 'csv')
					{
						header("Pragma: cache");
			...
			if($ilance->db->num_rows($sql) > 0)
		{
		   ...
		   {	
			...
			if($res['status'] != 'Paid')
			 ...
			 $res['details']='<a href="buyers.php?subcmd=_detail_invoice&paidstatus='.$status_paid.'&amp;id='.$res['invoiceid'].'"">Items</a>';
