	         ...
	         echo $table_row = ' <form method="post" action="listings.php" accept-charset="UTF-8" name="ilform" style="margin: 0px;">
				            ...
				            {	    
								 ...
								 {	
									 ...
									 echo $table_row = '<tr><td nowrap="nowrap"><input type="checkbox" value="'.$res_atc['attachid'].'" name="attach[]"> '.$res_atc['filename'].' -- '.$res_atc['attachtype'].'
									 <img src="'.HTTP_SERVER.$ilpage['image'].'?cmd=thumb&subcmd=results&id='.$res_atc['filehash'].'">
									 </td></tr>';
									 $filename= $res_atc['filename'];
								 ...
								 $path='/home/gc/public_html/image/';
								 $thumbs=str_replace($path,'',preg_split('/\s+/', trim(shell_exec('find '.$path.' -name "'.$filename.'"'))));
									   foreach($thumbs as $thumb)
									   {
									   	echo $table_row = '<tr><td><img src="'.HTTP_SERVER.'/image/'.$thumb.'"></td></tr>';
									   }
								 
							echo $table_row = '<tr><td><input type="submit" value="Delete" /></td></tr>															
									</table>
