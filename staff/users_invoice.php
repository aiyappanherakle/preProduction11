...
if (((!empty($ilance->GPC['id']) AND $ilance->GPC['id'] > 0) OR (!empty($ilance->GPC['userid']) AND $ilance->GPC['userid'] > 0) ) AND $_SESSION['ilancedata']['user']['isadmin'] == '1')
{
	...
	if(isset($ilance->GPC['cmd']) AND $ilance->GPC['cmd'] == 'did_invoice')
	{
		...
		$sql_totatlamount = $ilance->db->query("
			SELECT totalamount, invoiceid
			FROM " . DB_PREFIX . "invoices
			WHERE invoiceid  = '" . $ilance->GPC['id'] . "'");

			$res_totamount = $ilance->db->fetch_array($sql_totatlamount, DB_ASSOC);
			$print_invoice2='<a style="color:blue;" href="print_invoice2.php?id='.$res_totamount['invoiceid'].'"">#'.$res_totamount['invoiceid'].' Click here</a>';

		if(isset($ilance->GPC['subcmd']) AND $ilance->GPC['subcmd'] == 'complete')
		{
			print_action_success('<b>Invoice Payment:</b> &nbsp;Thank you for submitting your payment. Once your payment has been verified, we will mark your invoice as paid and advise you of shipping information.<br><br><span style="font-weight:bold;">Print Invoice</span> - '.$print_invoice2.'<br><br>  Thank you for your business.', $ilpage['users'].'?subcmd=_update-customer&id='.intval($ilance->GPC['uid']));
			exit();
		}
		else if(isset($ilance->GPC['subcmd']) AND $ilance->GPC['subcmd'] == 'cancel')	
		{
			print_action_failed('<b>Invoice Payment:</b> &nbsp;Submitting invoice remains unpaid.  To pay for this pending invoice at a later time, please return to My GC and click on Buyer Invoices. <br><br><span style="font-weight:bold;">Print Invoice</span> - '.$print_invoice2.'<br><br> Thank you for using GreatCollections','_invoice.php?subcmd=view_pending_invoice&uid='.intval($ilance->GPC['uid']));
			exit();
		}
	}
...
if($ilance->db->num_rows($user_invoice_check_result)==0)
{
  ...
  // [cmd] => _do-invoice-payment-checkorder
    ...
    [agreecheck] => on*/
		...
		print_action_success('<b>Thank you for your payment notification</b><br/>Thank you for paying by check.<br><br>The total of your invoice is: $'.$res_totamount['totalamount'].' (including insured shipping).<br><br><span style="font-weight:bold;">Print Invoice</span> - '.$print_invoice2.'<br><br>Please make the check payable to GreatCollections and mail to us at:<br><br>GreatCollections,<br>17500 Red Hill Avenue, Suite 160,<br>Irvine CA 92614.<br><br>We appreciate your business.', $ilpage['users'] . '?subcmd=_update-customer&id='.intval($ilance->GPC['userid']));
  ...
  // [cmd] => _do-invoice-payment-checkorder
    ...
    [agreecheck] => on*/
			...
			$print_invoice2='<a style="color:blue;" href="print_invoice2.php?id='.$res_totamount['invoiceid'].'"">#'.$res_totamount['invoiceid'].' Click here</a>';	
				...
				print_action_success('<b>Invoice Marked as Payment Pending:</b> You have successfully confirmed that your invoice will be paid by bank wire.  If you need our wire instructions, please contact us.  Once we receive your wire, we will mark your invoice as paid and ship the items to you via your preferred method.<br><br><span style="font-weight:bold;">Print Invoice</span> - '.$print_invoice2.'  Thank you for your business.', $ilpage['users'].'?subcmd=_update-customer&id='.intval($ilance->GPC['userid']));
		        exit(); 
...
if($ilance->db->num_rows($user_invoice_check_result)==0)
{
		print_action_failed('User dont have one such invoice<br/>Please try again', $ilpage['users'].'?subcmd=_update-customer&id='.intval($ilance->GPC['userid']));
  ...
  // [cmd] => _do-invoice-payment-trade
    ...
    [agreecheck] => on*/
		...
		print_action_success('<b>Invoice Marked as Payment Pending:</b> You have confirmed that invoice will be paid by a Trade Against Consignor Proceeds.<br><br><span style="font-weight:bold;">Print Invoice</span> - '.$print_invoice2.'<br><br>The total of your invoice is: $'.$res_totamount['totalamount'], $ilpage['users'].'?subcmd=_update-customer&id='.intval($ilance->GPC['userid']));
		exit(); 
