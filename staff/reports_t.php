...
//Karthik may 04
			     ...
			     if (isset($ilance->GPC['cmd']) AND $ilance->GPC['cmd'] == 'statement_print_new')
		       {
					...
					if(isset($ilance->GPC['print']))
					  header("Location:statement_new_t.php?start_date=".$ilance->GPC['start_date']."&option=".$ilance->GPC['display'].$user_id_query);
					else if(isset($ilance->GPC['summary']))
					  header("Location:statement_new_summary_t.php?start_date=".$ilance->GPC['start_date']."&option=".$ilance->GPC['display'].$user_id_query);
...
//cs.coin_series_sort,cc.coin_detail_year asc,cd.denomination_sort,
				...
				{				
				   ...
				   $cp = '-';
					...
					$listing_items.= '<tr><td>'.$projectid.'</td><td></td><td>'.$sold.'</td>
									...
									<td>'.$current_price.'</td><td>'.$buynow_price.'</td><td>'.$no_bids.'</td>';
	...
	$pprint_array = array('tot_mis','showoutput','user_down','user_id','date_down',
	...
	$ilance->template->fetch('main', 'reports_t.html', 2);
