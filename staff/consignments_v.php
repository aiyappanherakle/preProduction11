				 ...
				 $sqlcat_user_detail = $ilance->db->query("
								...
								");
								 ...
								 if($ilance->db->num_rows($sqlcat_coin_detail) > 0)
										...
										$catlogorder = '<span style="cursor:pointer; margin-left: 350px;" class="blue" onClick="window.open(\'consignments_pdf_v.php?cmd=pdf&subcmd=catlog&user_id='.$ilance->GPC['user_id'].'&consignid='.$ilance->GPC['consignid'].'&nocoin='.$coins.'&noposted='.$posted.'&list='.$listed_count.'&raw='.$raw_count.'&send='.$send_count.'\',\'mywindow\',\'width=400,height=200,scrollbars=yes\')">Catalog Sort Order</span>';
										
										
										$coinorder = '<span style="cursor:pointer; margin-left: 120px;"  class="blue" onClick="window.open(\'consignments_pdf_v.php?cmd=pdf&subcmd=coinord&user_id='.$ilance->GPC['user_id'].'&consignid='.$ilance->GPC['consignid'].'&nocoin='.$coins.'&noposted='.$posted.'&list='.$listed_count.'&raw='.$raw_count.'&send='.$send_count.'\',\'mywindow\',\'width=400,height=200,scrollbars=yes\')">Coin Id Sort Order</span>';
	...
	{	
	   ...
	   if (isset($ilance->GPC['subcmd']) AND $ilance->GPC['subcmd'] == 'search_list')
       {
			...
			if (!empty($filtervalue) AND !empty($filterby))
			{
				if($filterby == 'consignid')
				{
					...
					if($ilance->db->num_rows($get_user_details) > 0)
					{
						...
						$res_consign_details['View'] = '<a href="consignments.php?cmd=coin_list&user_id='.$res_consign_details['user_id'].'&consignid='.$res_consign_details['consignid'].'">View</a>'; 
						$res_consign_details['edit'] = '<a href="consignments.php?cmd=consign_edit&user_id='.$res_consign_details['user_id'].'&consignid='.$res_consign_details['consignid'].'">Edit</a>'; 
				...
				$res_consign_details['coinorder'] = '<span style="cursor:pointer;"  class="blue" onClick="window.open(\'consignments_pdf_v.php?cmd=pdf&subcmd=coinord&user_id='.$res_consign_details['user_id'].'&consignid='.$res_consign_details['consignid'].'&nocoin='.$res_consign_details['coins'].'&noposted='.$total_posted_coin.'&list='.$listed_count.'&raw='.$raw_count.'&send='.$send_count.'\',\'mywindow\',\'width=400,height=200,scrollbars=yes\')">PDF</span>';															
				...
				$row_list['coinorder'] = '<span style="cursor:pointer;"  class="blue" onClick="window.open(\'consignments_pdf_v.php?cmd=pdf&subcmd=coinord&user_id='.$row_list['user_id'].'&consignid='.$row_list['consignid'].'&nocoin='.$row_list['coins'].'&noposted='.$total_value.'&list='.$listed_count.'&raw='.$raw_count.'&send='.$send_count.'\',\'mywindow\',\'width=400,height=200,scrollbars=yes\')">PDF</span>';
		...
		$totcoin = $ilance->db->query("SELECT COUNT(coin_id) AS total,user_id,consignid,coin_id
				...
				$row_list['coinorder'] = '<span style="cursor:pointer;"  class="blue" onClick="window.open(\'consignments_pdf_v.php?cmd=pdf&subcmd=coinord&user_id='.$resconsign_list['user_id'].'&consignid='.$resconsign_list['consignid'].'&nocoin='.$resconsign_list['coins'].'&noposted='.$total_value.'&list='.$listed_count.'&raw='.$raw_count.'&send='.$send_count.'\',\'mywindow\',\'width=400,height=200,scrollbars=yes\')">PDF</span>';
