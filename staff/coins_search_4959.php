...
$show_certificationnumber_search=0;
	...
	SELECT p.*,c.coin_id,c.pcgs,c.Certification_No,c.consignid,c.Alternate_inventory_No,c.Title,c.Minimum_bid,c.user_id,c.relist_count,c.End_Date,c.Buy_it_now FROM " . DB_PREFIX . "coins AS c
	...
	if($ilance->db->num_rows($coins_search_Sold) > 0)
	{
		...
		//  print_r($sld);
			...
			$sldinvprojt = $ilance->db->query("SELECT * FROM " .DB_PREFIX. "invoices WHERE p2b_user_id = '".$sld['user_id']."'
				...
				$sldinvpjt = $ilance->db->query("SELECT * FROM " .DB_PREFIX. "invoices WHERE user_id = '".$sld['user_id']."'
				...
				else
				{
					...
					$listpagesold['invoiceid']='<span class="blue"><a href="buyers.php?subcmd=_detail_invoice&paidstatus='.$soldinvprojt['status'].'&amp;id='.$soldinvprojt['invoiceid'].'"">'.$soldinvprojt['invoiceid'].'</a></span>';
					$listpagesold['hammerprice']=$soldinvprojt['amount'];
				}
				}
				...
				while($sellinvpjt = $ilance->db->fetch_array($sellinvprojt))
				{
						...
						$listpagesold['invoiceid']='<span class="blue">Ins Fee:<a href="buyers.php?subcmd=_detail_invoice&paidstatus='.$sellinvpjt['status'].'&amp;id='.$sellinvpjt['invoiceid'].'"">'.$sellinvpjt['invoiceid'].'</a></span>';
			...
			{	
				...
				$sldinvpjt = $ilance->db->query("SELECT * FROM " .DB_PREFIX. "invoices WHERE user_id = '".$soldinvprojt['user_id']."'
				AND combine_project LIKE '%".$soldinvprojt['invoiceid']."%'");
