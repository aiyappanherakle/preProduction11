...
{ 
  ...
  //invoice with partial payments on that date
			...
			$result3=$ilance->db->query($query3, 0, null, __FILE__, __LINE__);
			...
			$result3=$ilance->db->query($query3, 0, null, __FILE__, __LINE__);
			$other_partial_invoices=array();
  
			if($ilance->db->num_rows($result3))
			{
				while($line3=$ilance->db->fetch_array($result3))
				{
					$other_partial_invoices[]=$line3['invoiceid'];
				}
			}
			...
			{
				$partial_part1=" and i.invoiceid not in (".implode(",",$other_partial_invoices).") ";
			...
			WHERE 
					(i.paymethod = '".$paymethod."' and  
						DATE(i.paiddate) between  '".$ilance->GPC['from_payment']."' AND '".$ilance->GPC['to_payment']."'  and i.combine_project!='' ".$partial_part1." )".$partial_part.' '.$order_by;
	
			$result1=$ilance->db->query($query1, 0, null, __FILE__, __LINE__);
			...
			if($ilance->db->num_rows($result1))
			{
				...
				while($line1=$ilance->db->fetch_array($result1))
				{
					...
					else
					{
						...
						$result2=$ilance->db->query($query2, 0, null, __FILE__, __LINE__);
						...
						if($ilance->db->num_rows($result2))
						{
							while($line2=$ilance->db->fetch_array($result2))
							{
								...
								$res['descripton'] 	= $line1['first_name'].' '.$line1['last_name'].' ['.$line1['user_id'].']';	
								$paymethod_k = ucwords($line2['paymethod']);
							if($paymethod_k=='Paypal')
							{
								$paymethod_k = 'PayPal';
							}
...
$sort_by='Method';
$sort_by_2='descripton';
$sort_order=0;//1=asc,0=des

$a=my_sort_k($data,$sort_by,$sort_order);

$c=array();
foreach($a as $key=>$b)
{
	if(!in_array($b[$sort_by],$c))
	$c[]=$b[$sort_by];
}

// Card,Check,Credit Card BofA,Others,PayPal,Trade,Wire
...
foreach($c as $key=>$d)
{
	...
	// bug id #4280 author:kumaravel start
	foreach($f as $k=>$w)
	{
		$sub_total = 0;
		foreach($w as $n=>$m)
		...
		$f[$key][$i++]=array(0=>$m['Method'].' subtotal',1=>$sub_total);
		
		//print_r($f);	
}
// bug id #4280 author:kumaravel end
// payment method order only based details 

foreach($f as $key=>$g)
{
	$f[$key]=my_sort_k($g,$sort_by_2,$sort_order);
}
// payment method and user name order based details 
...
foreach($f as $h)
{
	foreach($h as $k)
	{
		$l[$m++]=$k;
			...
			$reportoutput = $ilance->admincp->construct_csv_data_quickbooks($l, $headings);
...
function my_sort_k($data,$sort_by,$sort_order=0)
{
