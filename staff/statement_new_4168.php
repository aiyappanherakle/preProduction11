...
c.End_Date,
l.listed_first_time,
IFNULL(r.last_listed_time,'0000-00-00 00:00:00') as last_listed_time,
dd.last_dailydeal_time,
p.date_end,
c.Title,
c.Minimum_bid,
c.Buy_it_now,
...
left join 
	(
		select coin_id,user_id,count(id) as no_relist_b4_statement,max(startbydate) as startbydate,
		date_format(max(enddate),'%Y-%m-%d %H:%i:%s') as last_listed_time from ".DB_PREFIX."coin_relist 
		where 
			date(actual_end_date)<'".$stmt_date."' 
			and user_id='".$user_details['user_id']."' 
			and DATEDIFF(enddate,startbydate)>1 group by coin_id
	) r on r.coin_id=c.coin_id
left join
	(
		select coin_id,case when count(coin_id)=1 then true else false end listed_first_time from ".DB_PREFIX."coin_relist 
		where 
			date(actual_end_date)<='".$stmt_date."' 
			and user_id='".$user_details['user_id']."' 
			and DATEDIFF(enddate,startbydate)>1 group by coin_id
	) l on l.coin_id=c.coin_id
left join
	(
		select coin_id,user_id,count(id) as no_relist_b4_statement,
		date_format(max(actual_end_date),'%Y-%m-%d %H:%i:%s') as last_dailydeal_time from ilance_coin_relist 
		where 
			date(actual_end_date)<='".$stmt_date."' 
			and hour(actual_end_date)=23
			and user_id='".$user_details['user_id']."'
			and DATEDIFF(enddate,startbydate)>1 group by coin_id
	) dd on dd.coin_id=c.coin_id
left join
	(
		select sum(qty) as order_count,project_id,orderdate from ".DB_PREFIX."buynow_orders o where 
		owner_id='".$user_details['user_id']."' 
		and date(orderdate)<='".$stmt_date."' 
		and date(orderdate)>date
		(
			(
				select 
				CASE WHEN 
						count(actual_end_date)>0 
					THEN 
						date_format(max(actual_end_date),'%Y-%m-%d %H:%i:%s')   
					ELSE 
						'0000-00-00 00:00:00' 
					END as last_listed_time from ilance_coin_relist 
				where date(actual_end_date)<'".$stmt_date."' 
				and user_id='".$user_details['user_id']."' 
				and DATEDIFF(enddate,startbydate)>1 
				and coin_id=o.project_id order by actual_end_date desc limit 1
			)
		) group by project_id
	) o on c.coin_id=o.project_id 
left join
...
left join
	(
		...
		(
			i.user_id=".$user_details['user_id']." or i.p2b_user_id=".$user_details['user_id']."
		) 
		and date(i.createdate)<='".$stmt_date."' 
		and date(i.createdate)>date
		(
			(
				select 
				CASE WHEN 
				...
				and user_id='".$user_details['user_id']."' 
				and DATEDIFF(enddate,startbydate)>1 
				and coin_id=i.projectid order by actual_end_date desc limit 1
			)
...
where  
(
	c.user_id=".$user_details['user_id']." 
	and 
	(
		(
			(
				date(last_listed_time)>'".$start."'
				and date(last_listed_time)<='".$stmt_date."'
			) or
			(
				last_listed_time is null and l.listed_first_time=true
			)
		) or 
		(
			date(dd.last_dailydeal_time)>'".$start."'
			and date(dd.last_dailydeal_time)<='".$stmt_date."'
		) or 
		(
			date(c.End_Date)>='".$start."' 
			and date(c.End_Date)<='".$stmt_date."' 
			and (
					c.project_id>0 
					or c.relist_count>0
				)
		...
		(
			date(p.date_end)>='".$start."' 
			and date(p.date_end)<='".$stmt_date."'
		)
	)    
)
...
//		$html.='<td size="9">'.$sl.'</td>';
		$html.='<td size="9" bgcolor="#03FF46" >'.$coins_list_line['coin_id'].'</td>';
		...
		$html.='<td size="9">'.$disply_paid.$ilance->currency->format_real_no($coin_consignor_total).'</td>';
		$html.='</tr>';
			$sl++;
