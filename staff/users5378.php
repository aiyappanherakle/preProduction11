...
$show['no_notes'] = true;
	...
	{
		$sql2.= " group by i.invoiceid order by i.paiddate desc";
	}
...
$counter = ($ilance->GPC['series_page'] - 1) * $ilance->GPC['pp'];
	...
	$scriptpageprevnext="users.php?subcmd=_update-customer&id=".$ilance->GPC['id']."&searchby=".$ilance->GPC['searchby']."&searchkey=".$ilance->GPC['searchkey']."";
	else
	$scriptpageprevnext="users.php?subcmd=_update-customer&id=".$ilance->GPC['id'];
...
// currently consinged coins & return consignment start
		...
		*/
		...
		echo $suku_5355 = "SELECT c.coin_id,c.Title,ROUND(c.Minimum_bid,2) as minc,
...
echo '<br>';
		echo	$SQL="  SELECT c.coin_id,c.Title,ROUND(c.Minimum_bid,2) as minc,
							ROUND(c.Buy_it_now,2) as buyc,c.Create_Date,c.End_Date,
							ROUND(p.currentprice,2) as currentp,p.bids  
					FROM " . DB_PREFIX . "coins c
					left join " . DB_PREFIX . "projects p on p.project_id=c.project_id
		...
		$select_all_consign_coins_pg = $ilance->db->query($SQL.$sql_pagination_k."");
		...
		if($ilance->db->num_rows($select_all_consign_coins_pg) > 0)
		{
			...
			$select_all_consign_coins = $ilance->db->query($SQL); 
					...
					$get_filtervalue1 = $ilance->GPC['filtervalue2'];
                       ...
                       $orderlimit = ' ORDER BY ' . $filterby . ' ' . mb_strtoupper($orderby) . ' LIMIT ' . (($ilance->GPC['page']-1)*50).','.'50';
                        ...
                        $sqlsearchcustomers = $ilance->db->query("
                                SELECT user_id, username, first_name, last_name,Check_Payable, email, phone, city, state, zip_code, status, available_balance, total_balance, isadmin, permissions,ir_managed,notifyauction,access_bb
                                FROM " . DB_PREFIX . "users
                                $where
                                $orderlimit                           
                        ");
