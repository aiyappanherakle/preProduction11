...
//error_reporting(E_ALL);
		...
		$select = $ilance->db->query("
					SELECT distinct co.user_id,u.username,u.email,u.address,u.address2,u.city,u.state,u.zip_code FROM ".DB_PREFIX."coins co,".DB_PREFIX."users u  		
...
// 1779 bugs	
	...
	$table3='<table width="100%" border="0">
		<tr>
	...
	$table3.='<tr bgcolor="#CD9C9C"> 
			...
			<td width = "5%">Net to <br>Consignor</td>
		</tr>';
			...
			$p->setfont('times','',10);		
                ...
                {
	$select_user = $ilance->db->query("SELECT username,email,first_name,last_name,address,address2,city,state,zip_code FROM ".DB_PREFIX."users	
					       WHERE user_id='".$res_user['user_id']."'");
					
		$res_det=$ilance->db->fetch_array($select_user);
		
		$name = $res_det['username'];
		$email = $res_det['email'];
		$first_name = $res_det['first_name'];
		$last_name = $res_det['last_name'];
		$address = $res_det['address'];
		$address2 = $res_det['address2'];
		$city = $res_det['city'];
		$state = $res_det['state'];
		$zipcode = $res_det['zip_code'];
...
$cert_no='';
 ...
 $advanceselect = $ilance->db->query("SELECT sum(amount) as amount FROM " . DB_PREFIX . "user_advance WHERE statusnow = 'paid' AND user_id ='".$res_user['user_id']."' ");
				...
				$res_user['address'] = ($res_user['address']=='')?'':$res_user['address'].'<br>';
				$res_user['address2'] = ($res_user['address2']=='')?'':$res_user['address2'].'<br>';
				$res_user['city'] = ($res_user['city']=='')?'':$res_user['city'].'<br>';
				$res_user['state'] = ($res_user['state']=='')?'':$res_user['state'].'--';
				$res_user['zip_code'] = ($res_user['zip_code']=='')?'':$res_user['zip_code'];
				
				 $table3.='<tr><td>'.$res_user['username'].' </td><td>'.$res_user['email'].' </td><td align=left>'.$res_user['address'].$res_user['address2'].$res_user['city'].$res_user['state'].$res_user['zip_code'].'</td>
						<td>'.$totfvf.'</td>
						<td>'.$totlisting_fee.'</td><td>'.$totseller_fee.'</td>
						<td>'.$totnet_consignor.'</td></tr>';	
...
$table1.='<table width="100%"><tr><td>Thank you for consigning to GreatCollections.<br>We appreciate your business.</td></tr></table>';
	...
	//$iteration++;
		...
		$table3.='<tr><tr></tr><td></td><td></td><td><strong>Gross <br> Total</strong></td><td>'.$ilance->currency->format_no_text(array_sum($total_final),$ilconfig['globalserverlocale_defaultcurrency']).'</td>
