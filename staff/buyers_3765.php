...
if(isset($ilance->GPC['cmd']) AND $ilance->GPC['cmd']=='change_shipping' AND isset($ilance->GPC['invoice_id']) AND $ilance->GPC['invoice_id']>0 )
{
   ...
   $shipping_cost=$ilance->db->query("SELECT shipping_cost FROM " . DB_PREFIX . "invoice_projects WHERE final_invoice_id='".$ilance->GPC['invoice_id']."'");
...
$sql=$ilance->db->query("select invoiceid,user_id,combine_project,miscamount,taxamount,taxinfo,amount,paid,totalamount,status from ".DB_PREFIX."invoices where invoiceid='".$invoice_id."'");
...
if($line['paid']>0)
{
	...
	if($ilance->db->num_rows($result))
	{
		...
		while($line=$ilance->db->fetch_array($result))
		{
			...
			$line['edit']='<a href="buyers_3765.php?cmd=edit_partial_payment&payment_id='.$line['id'].'"><img src="../images/gc/icons/pencil.gif"/></a>';
...
if($invoice_list > 0)
{
$sql=$ilance->db->query("select * from ".DB_PREFIX."invoices where invoiceid='".$line['invoiceid']."'");
...
where s.track_no !='' and i.invoiceid='".$one_paid_item."'
		...
		if($ilance->db->num_rows($not_shipped_sql)==0){
			...
			$not_shipped['invoice_id']='<a href="buyers_3765.php?subcmd=_detail_invoice&paidstatus=paid&amp;id='.$not_shipped_res_1['invoiceid'].'"">'.$not_shipped_res_1['invoiceid'].'</a>';			
...
$scriptpageprevnext="buyers_3765.php?cmd=buyer&subcmd=search&searchby=".$ilance->GPC['searchby']."&searchkey=".$ilance->GPC['searchkey']."";
else
$scriptpageprevnext="buyers_3765.php?";
			...
			if($ilance->db->num_rows($sql) > 0)
		{
		   ...
		   {	
			...
			if($res['status'] != 'Paid')
			 ...
			 $res['details']='<a href="buyers_3765.php?subcmd=_detail_invoice&paidstatus='.$status_paid.'&amp;id='.$res['invoiceid'].'"">Items</a>';
...
//New Changes on 12Mar02 for Buyers Unpaid List
   if (isset($ilance->GPC['cmd']) AND $ilance->GPC['cmd'] == 'buyers_print')
   {
	...
	$invoice_type_drop_down='<select name="invoice_type" id="invoice_type" onchange="window.location=\'buyers_3765.php?invoice_type=\'+(this.value);">
