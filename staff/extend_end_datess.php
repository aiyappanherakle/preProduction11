...
if (!empty($_SESSION['ilancedata']['user']['userid']) AND $_SESSION['ilancedata']['user']['userid'] > 0 AND $_SESSION['ilancedata']['user']['isadmin'] == '1')
{
    ...
    if(isset($ilance->GPC['cmd']) AND $ilance->GPC['cmd'] == 'coins_list') 
	{
		if(!empty($ilance->GPC['filtervalue']))
		{
			...
			if($ilance->GPC['filterby'] == 'coin_id')
			{
				...
				WHERE c.coin_id = '".$ilance->GPC['filtervalue']."'
				AND p.status = 'open'
				".$chk_img_sql_cnd."
				...
				else
				{
					...
					print_action_failed('Please Enter Correct Coin id / Project id  and try again', 'extend_end_dates.php');
					exit();
			...
			{
				$searchby = $ilance->GPC['filterby']." = '".$ilance->GPC['filtervalue']."' ";
				...
				else
				{
					...
					{
						print_action_failed('Please Enter Correct User ID and try again', 'extend_end_dates.php');
						exit();
					} 
					else if ($ilance->GPC['filterby'] == 'username') 
					{
						print_action_failed('Please Enter Correct Username and try again', 'extend_end_dates.php');
						exit();
					} 
					else
					{
						print_action_failed('Please Enter Correct Email and try again', 'extend_end_dates.php');
						exit();
					}
				}
	...
	if(isset($ilance->GPC['cmd']) AND $ilance->GPC['cmd'] == 'extend_enddate') 
	{
		...
		$sellerid = (isset($ilance->GPC['userid']) AND !empty($ilance->GPC['userid']));
			
		$coin_ids = (isset($ilance->GPC['coin_ids']) AND !empty($ilance->GPC['coin_ids']));
			
		$end_date = (isset($ilance->GPC['end_date']) AND !empty($ilance->GPC['end_date']));
		...
		{
			print_action_failed('Please select aleast single coin. and try again', 'extend_end_dates.php');
			exit();
	...
	$ilance->template->fetch('main', 'extendings.html', 2);
