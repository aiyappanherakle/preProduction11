...
if (!empty($_SESSION['ilancedata']['user']['userid']) AND $_SESSION['ilancedata']['user']['userid'] > 0 AND $_SESSION['ilancedata']['user']['isadmin'] == '1')
{
	...
	$sql="SELECT *,DATE_ADD(createdate, INTERVAL 5 DAY) as payment_due   FROM " . DB_PREFIX . "invoices WHERE date(createdate) between '".$start_date."' and '".$end_date."'  and combine_project!=''";
	...
	while($test=$ilance->db->fetch_array($result))
	{
		...
		$composed['Invoice_Date']	=date("m-d-Y", strtotime($test['createdate']));
		$composed['Payment_Due']	=date("m-d-Y", strtotime($test['payment_due']));
		$composed['User_ID']		=$test['user_id'];
		$composed['Summary']		='Auction System Invoice';
		$composed['Description']	='Hammer Buyer';
		$composed['Account_Name']	='4000';
		...
		$composed['User_ID']		=$test['user_id'];
		$composed['Summary']		='Auction System Invoice';
		$composed['Description']	='Buyers Fees';
		$composed['Account_Name']	='4020';
		...
		$composed['User_ID']		=$test['user_id'];
		$composed['Summary']		='Auction System Invoice';
		$composed['Description']	='Sales Tax';
		$composed['Account_Name']	='2020';
		...
		$composed['User_ID']		=$test['user_id'];
		$composed['Summary']		='Auction System Invoice';
		$composed['Description']	='Shipping Fees';
		$composed['Account_Name']	='4070';
	...
	$ilance->template->fetch('main', 'accountant.html', 2);
