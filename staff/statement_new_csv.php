  ...
  //removed from line 91   and date(i.createdate) <= '".$stmt_date."' and date(i.createdate)>='".$start."'
	...
	$html.=get_header_pdf($user_details,$stmt_date);
	 ...
	 if($coin_final_price>0)
		...
		$html.='<td >'.$coins_list_line['Title'].'</td>';
		$html.='<td>'.$no_of_bids.'</td>';
		$html.='<td>'.$no_of_buynow.'</td>';
		$html.='<td>'.$coins_list_line['cost'].'</td>';
		$html.='<td>'.$ilance->currency->format_real_no($coins_list_line['Minimum_bid']).'</td><td>'. $ilance->currency->format(isset($coins_list_line['Buy_it_now'])?$coins_list_line['Buy_it_now']:0) .'</td>';
...
<td>Item Title</td>
<td>Listed</td>	
<td>Bids</td><td>Buy</td><td>cost</td>
