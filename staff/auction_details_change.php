 ...
 if(isset($ilance->GPC['get_user_detail']))
 {
      ...
      if(isset($ilance->GPC['get_fromuser_detail']))
      {
	   ...
	   $table='<table border="1" style="padding:10px"><tr style="height: 25px;"><td><B>ProjectID</td><td><B>User_id</td>
		                                         <td><B>Amount</td> <td><B>Auction Type</td></tr>';
...
error_reporting(E_ALL);
     ...
     echo $query6="INSERT INTO " . DB_PREFIX . "users_note
                          (note_id,user_id, note, created, modified)
                          VALUES(
                          NULL,
                          '" . $from_user . "',
                          'Did not pay, Banned User',
                          now(),
                          now())
                          ";
                          echo ';<br>';
                          $ilance->admincp->ban_user(array($from_user));
...
echo ';<br>';		 
			...
			$unpa= $ilance->db->query("SELECT status,invoiceid,user_id,amount
					  FROM " . DB_PREFIX . "invoices 
			...
			while($un_pay = $ilance->db->fetch_array($unpa))
			{
			      ...
			      echo $query105= "UPDATE " . DB_PREFIX . "invoices SET  user_id = '".$to_user."'
					...
					echo ';<br>';
                              ...
                              $amount= $de_be['amount'];
                                  $master_invoice_sql="select * from ilance_invoices where combine_project like '%".$un_pay['invoiceid']."%' and user_id='".$un_pay['user_id']."'";
                                  $master_invoice_result=$ilance->db->query($master_invoice_sql);
                                  while($master_invoice_line=$ilance->db->fetch_array($master_invoice_result))
                                  {
                                      echo $query111="UPDATE " . DB_PREFIX . "invoices SET  user_id = '".$to_user."'
                                                      WHERE invoiceid = '".$master_invoice_line['invoiceid']."'
                                                      AND user_id = '".$from_user."'";
					echo ';<br>';        
                                  }
      ...
      while($ded_bet1 = $ilance->db->fetch_array($buynow))
      {
	    ...
	    if($ilance->db->num_rows($unpaid1)>0)
	    {
		   while($de_be1 = $ilance->db->fetch_array($unpaid1))
			...
			$unpa1= $ilance->db->query("SELECT status,user_id,invoiceid,amount
					  FROM " . DB_PREFIX . "invoices 
			...
			while($un_pay1 = $ilance->db->fetch_array($unpa1))
			{
			      ...
			      echo $query109="UPDATE " . DB_PREFIX . "invoices SET  user_id = '".$to_user."'
                              ...
                              $amount= $de_be1['amount'];
                                  $master_invoice_sql="select * from ilance_invoices where combine_project like '%".$un_pay1['invoiceid']."%' and user_id='".$un_pay1['user_id']."'";
                                  $master_invoice_result=$ilance->db->query($master_invoice_sql);
                                  while($master_invoice_line=$ilance->db->fetch_array($master_invoice_result))
                                  {
                                      echo $query111="UPDATE " . DB_PREFIX . "invoices SET  user_id = '".$to_user."'
...
if (isset($ilance->GPC['cmd']) AND $ilance->GPC['cmd'] == 'auction_to_buynow')
	...
	$sqlactbuy = $ilance->db->query("
						...
						AND p.status IN ('open', 'closed', 'expired')
						AND c.coin_id IS NOT NULL 
						AND ((p.filtered_auctiontype = 'regular' AND p.winner_user_id  = '0' AND p.bids='0') 
	...
	if($ilance->db->num_rows($sqlactbuy)>0)
	{
		...
		print_action_success('This Item is Changed from Auction  to Buy Now Successfully', 'auction_details_change.php');
	...
	{
	print_action_failed('We are sorry . the given coin id  does not exists in projects.', 'auction_details_change.php');
...
if (isset($ilance->GPC['cancelcmd']) && $ilance->GPC['cancelcmd'] == 'auction_sale')
  ...
  // 		if (!preg_match('/^[0-9]*$/', $ilance->GPC['buyer_id'])) {
		// print_action_failed('Please give valid Project ID, User ID or Buyer Id  details','auction_details_change.php');
	 //    exit();
		//  }
  //       if(isset($ilance->GPC['buyer_id']) && $ilance->GPC['buyer_id']>0)
		//  {
		//   $userid = $ilance->GPC['buyer_id'];
		//  }
		//  else
		//  {
		//   $userid_user = fetch_user('user_id',0,$ilance->GPC['username']);
		
		// if (!preg_match('/^[0-9]*$/', $userid_user)) {
	 ...
	 //    exit();
		...
		if(isset($ilance->GPC['user_confirm']) AND $ilance->GPC['user_confirm'] == '1')
		{
		if (!preg_match('/^[0-9]*$/', $ilance->GPC['buyer_id'])) {
		print_action_failed('Please give valid Project ID, User ID / Buyer Id or User Name  details','auction_details_change.php');
	    exit();
		 }
			$userid = fetch_user('user_id',$ilance->GPC['buyer_id']);
		}
		else
		{
			$userid = fetch_user('user_id',0,$ilance->GPC['buyer_id']);
		}
...
$sql_projects = $ilance->db->query("
                       SELECT * FROM " . DB_PREFIX . "projects WHERE project_id = '".$ilance->GPC['proj_id']."' and haswinner != '1' and filtered_auctiontype = 'regular' AND winner_user_id  = '0' AND bids = '0'");
		...
		{		
		  ...
		  {
		    if (!preg_match('/^[0-9]+(\\.[0-9]+)?$/', $ilance->GPC['current_price'])) {
			  print_action_failed('Please give valid Selling Price ','auction_details_change.php');
			  exit();
		    }else
		    {
		  ...
		  else
		  {
              ...
              $ilance->db->query("
                        ...
                        (bid_id, user_id, project_id, project_user_id, bidamount, qty, date_added, date_awarded, bidstatus, bidstate, state,winnermarkedaspaidmethod,buyerpaymethod)
                        VALUES(
                        NULL,
                        '".$userid."',
                        '" . $res['project_id'] . "',
                ...
                $ilance->db->query("
                        ...
                        (id, bid_id, user_id, project_id, project_user_id, bidamount, qty, date_added, bidstatus, bidstate, state)
                        VALUES(
                        NULL,
                        '" . intval($this_bid_id) . "',
                        '" . intval($userid) . "',
               ...
               $ilance->db->query("
                       ...
                       SET bids = bids + 1,
					   status =  'expired', 
                       currentprice = '" . sprintf("%01.2f", $res['currentprice']) . "'
                ...
                ", 0, null, __FILE__, __LINE__);	
						...
						$transactionid = construct_transaction_id();							
						  $ilance->db->query("INSERT INTO " . DB_PREFIX . "invoices
                                              (invoiceid, projectid, user_id,p2b_user_id, description, amount, totalamount, status, invoicetype,ipaddress, createdate, duedate, custommessage, transactionid)
                                                                                VALUES(
                                                                                NULL,
                                                                                '" . $res['project_id'] . "',
                                                                                '".$userid."',
																				'".$res['user_id']."',
																				'" . $ilance->db->escape_string($phrase['_escrow_payment_forward']) . " Item ID  " . " #" . $res['project_id'] .":".$ilance->db->escape_string($res['project_title']) ."',                                                                               
                                                                                ...
                                                                                '" . $res['date_end'] . "',
                                                                                '" . DATEINVOICEDUE . "',
                                                                                ' Additional Shipping Fees: $0.00 ',
                                                                                '" . $transactionid . "')
                                                                        ", 0, null, __FILE__, __LINE__);
																		$prod_invoice = $ilance->db->insert_id();
						...
						$escrow_id = $ilance->db->insert_id();
											...
											$ilance->db->query("
												...
												startprice='" . sprintf("%01.2f", $res['currentprice']) . "'
												WHERE project_id = '" . $res['project_id'] . "'
											", 0, null, __FILE__, __LINE__);
										  ...
										  $coin = $ilance->db->query("
											UPDATE  " . DB_PREFIX . "coins
											SET  sold_qty = sold_qty + 1,End_Date='" . $res['date_end'] . "',
											 Minimum_bid =  '" . sprintf("%01.2f", $res['currentprice']) . "'											
     ...
     //list of return
      ...
      if (!isset($ilance->GPC['page']) OR isset($ilance->GPC['page']) AND $ilance->GPC['page'] <= 0)
        ...
        $con_listing = $ilance->db->query("SELECT charges,shipping_fees,user_id,coin_id,consign_id,return_date FROM " . DB_PREFIX . "coin_return
					  ORDER BY return_date DESC LIMIT " . (($ilance->GPC['page'] - 1) * $ilconfig['globalfilters_maxrowsdisplaysubscribers']) . "," . $ilconfig['globalfilters_maxrowsdisplaysubscribers']."
					  ");
	 $con_listing1 = $ilance->db->query("SELECT * FROM " . DB_PREFIX . "coin_return ORDER BY return_date DESC");
...
$pprint_array = array('return_pagnation','number','remote_addr','rid','login_include_admin','headinclude','area_title','page_title','site_name','https_server','http_server','lanceads_header','lanceads_footer','htm','ilanceversion');
        
        ($apihook = $ilance->api('admincp_dashboard_end')) ? eval($apihook) : false;
	
	$ilance->template->fetch('main', 'auction_details_change.html', 2);
	$ilance->template->parse_hash('main', array('ilpage' => $ilpage));
	$ilance->template->parse_loop('main', array('total_list','return_sale_list'));
