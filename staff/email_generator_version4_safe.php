...
{	// if logged-in as admin
	...
	if (!isset($_POST['issubmit']))	// if first level of page access, show form
	{
		if (True || isset($_GET['load']))	// ALWAYS DO UPON LOAD... if a previous mailing is being referenced
		{
			...
			$formvars = dirname($_SERVER['SCRIPT_FILENAME']).'/email_generator_version4.json';	// get file path/name
			clearstatcache(); $savedTime = @filemtime($formvars);	// get file modification time of JSON
			$formvars = @json_decode(@file_get_contents($formvars), true);
...
<br /><br />
		...
		<td valign="top" style="width: 690px;">
			<iframe name="results_frame" style="width: 100%; height: 100%; border: 0px"></iframe>
		</td>
		</tr></table>
		</form>
		</body>
	...
	if ($_POST['savedata'])	// if form data is to be saved
	{
		...
		file_put_contents(dirname($_SERVER['SCRIPT_FILENAME']).'/email_generator_version4.json', json_encode($formvars));
	}
...
';
				...
				$itemCCE .=
str_repeat('=',73).'
'.str_repeat(' ',max(0,floor((73-strlen($heading))/2))).$heading.'
'.str_repeat('=',73).'
...
';
		...
		if (!$itemHTMLFeature)	// if no feature coin defined yet
			...
			$itemHTML .= 
'<div style="width: 145px; display: inline-block; padding: 4px; vertical-align: text-top;">'.showItem($row, False, 140, 15).'</div>
...
*/
	...
	?></div>
	<?php
	echo '<br /><br /><pre>'.HTMLEntities($itemText);
...
function showItem($row, $isPlain = False, $width = 140, $corners = 15)	// output a single coin, format width and corner roundness, HTML or plain text formatted
{
	global $referal_name1;
