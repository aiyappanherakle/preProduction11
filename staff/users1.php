...
if (!empty($_SESSION['ilancedata']['user']['userid']) AND $_SESSION['ilancedata']['user']['userid'] > 0 AND $_SESSION['ilancedata']['user']['isadmin'] == '1')
{
	...
	//vijay nov 18
		   ...
		   else if (isset($ilance->GPC['subcmd']) AND $ilance->GPC['subcmd'] == 'access_bbdeactivate' AND isset($ilance->GPC['id']) AND $ilance->GPC['id'] > 0)
        {
		  ...
		  print_action_success('The selected user Access BBcode deactivated',$ilpage['users']);
			exit();
		}
	...
	print_action_success('The selected user Access BBcode activated',$ilpage['users']);
			exit();
...
$show['no_notes'] = true;
                ...
                if (isset($ilance->GPC['cmd']) AND $ilance->GPC['cmd'] == 'search')
                {
                       ...
                       $orderlimit = ' ORDER BY ' . $filterby . ' ' . mb_strtoupper($orderby) . ' LIMIT ' . (($ilance->GPC['page']-1)*50).','.'50';
                        ...
                        if ($ilance->db->num_rows($sqlsearchcustomers) > 0)
                        {
                                ...
                                while ($res = $ilance->db->fetch_array($sqlsearchcustomers, DB_ASSOC))
                                {
									...
									SELECT sess.*
									FROM " . DB_PREFIX . "sessions AS sess
									WHERE sess.userid = '$res[user_id]'
									AND sess.isuser = '1'
									AND sess.userid > 0
									GROUP BY sess.token
					...
					$res['access_bb'] = ($res['access_bb'] == '1') ? '<a href="' . $ilpage['users'] . '?subcmd=access_bbdeactivate&amp;id=' . $res['user_id'] . '"><img src="' . $ilconfig['template_relativeimagepath'] . $ilconfig['template_imagesfolder'] . 'checked.gif" alt="Click to stop Access BBcode" border="0"></a>' : '<a href="' . $ilpage['users'] . '?subcmd=access_bbactivate&amp;id=' . $res['user_id'] . '"><img src="' . $ilconfig['template_relativeimagepath'] . $ilconfig['template_imagesfolder'] . 'unchecked.gif" alt="Click to  enable Access BBcode" border="0"></a>';
					...
					if ($res['access_bb'])
                                        {
                                                ...
                                                $res['access_bb'] = '<a href="' . $ilpage['users'] . '?subcmd=access_bbdeactivate&amp;id=' . $res['user_id'] . '"><img src="' . $ilconfig['template_relativeimagepath'] . $ilconfig['template_imagesfolder'] . 'checked.gif" alt="Click to  stop Access BBcode" border="0"></a>';
                                        }
                                        else
                                        {
										
                                                $res['access_bb'] = '<a href="' . $ilpage['users'] . '?subcmd=access_bbactivate&amp;id=' . $res['user_id'] . '"><img src="' . $ilconfig['template_relativeimagepath'] . $ilconfig['template_imagesfolder'] . 'unchecked.gif" alt="Click to  enable Access BBcode" border="0"></a>';
                                        }
	...
	$ilance->template->fetch('main', 'users1.html', 2);
