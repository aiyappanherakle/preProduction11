...
//Karthik may 04
			...
			//Karthik on Dec05 for All Sales Tax CSV 
	            if (isset($ilance->GPC['cmd']) AND $ilance->GPC['cmd'] == 'all_sales_tax')
		       {
					...
					$enddate = intval($ilance->GPC['year1']) . '-' . $ilance->GPC['month1'] . '-' . $ilance->GPC['day1'];							
					 $end =  date('Y-m-d',strtotime( $enddate));
				echo 'hai';exit;
			      header("Location:all_salestax_csv_4713.php?start_date=".$start."&end_date=".$end."");
			   }
		...
		$conds = "AND p.haswinner != 1 AND p.hasbuynowwinner != 1";
		 ...
		 {
		 $sql="SELECT c.coin_id,c.site_id, c.Title, c.Certification_No,c.Buy_it_now,c.Minimum_bid,c.pcgs FROM
		...
		if(empty($ilance->GPC["excsold"])){		
		 $sql="SELECT c.coin_id,c.site_id, c.Title, c.Certification_No,c.pcgs  FROM 
...
//cs.coin_series_sort,cc.coin_detail_year asc,cd.denomination_sort,
					$listing_items = '<table border="1"><tr><td>Coin_id</td><td>Site</td><td>Sold</td><td>Title</td><td>PCGS</td><td>Certificate No</td><td>Min Bid</td><td>Buy Now</td><td>Bids</td><td>Total</td></tr>';
								...
								
							 {
							   ...
							   $no_bids = '-';
									 ...
									 if($listing['site_id']==0)
										 ...
										 $site_name='EBay';
									$listing_items.= '<tr><td>'.$projectid.'</td><td>'.$site_name.'</td><td>'.$sold.'</td><td>'.$projecttitle.'</td><td>'.$listing['pcgs'].'</td><td>'.$certnum.'</td><td>'.$current_price.'</td><td>'.$buynow_price.'</td><td>'.$no_bids.'</td><td>'.$total.'</td></tr>';
					            ...
					            while($listing = $ilance->db->fetch_array($return_list))
					                {
							         ...
							         $total =  $ilance->currency->format($cp+$bp);
									$listing_items.= '<tr><td>'.$projectid.'</td><td></td><td>'.$sold.'</td><td>'.$projecttitle.'</td><td>'.$listing['pcgs'].'</td><td>'.$certnum.'</td><td>'.$current_price.'</td><td>'.$buynow_price.'</td><td>'.$no_bids.'</td><td>'.$total.'</td></tr>';
