...
{
	if(isset($ilance->GPC['cmd']) AND $ilance->GPC['cmd'] == 'current_auctions')
	{
		...
		$sql=$ilance->db->query("SELECT p.project_id,p.project_title,p.date_end,
		...
		$fileName = "CurrentAuctions-$timeStamp";
		$action = 'csv';
	...
	else
	{
		...
		$ilance->template->fetch('main', 'current_auctions.html', 2);
