	  ...
	  else
	  {
	      ...
	      else
	      {
			...
			if($ilance->db->num_rows($insert_value) > 0)
			{
				...
				$custom['13']['QA'] = $row_value['QA'];
				$custom['14']['spl_toned'] = $row_value['spl_toned'];
				$custom['15']['hot_list'] = $row_value['hot_list'];
...
$orderby ="ORDER BY bids ASC";
		   ...
		   $bold_list=$ilance->db->query("
										     ...
										     while($values_list=$ilance->db->fetch_array($bold_list))
										       {
												...
												$project_list['hotlists'] = $values_list['hotlists'];
										         ...
										         $project_list['auctiontype'] = $values_list['filtered_auctiontype'];
										         $project_list['auctiontype']='Bid';
										         
										         if($values_list['filtered_auctiontype'] == 'fixed')
										         {
													  $project_list['auctiontype']='Buy now';
												 }
												 $project_list['noofbids'] = $values_list['bids'];
												 if($values_list['buynow']== 1)
												 {
												 $project_list['noofbids'] = '0';
										     	 }
			...
			$ilance->template->parse_hash('main', array('ilpage' => $ilpage));
			$ilance->template->parse_loop('main', array('highlitedlist'));
			$ilance->template->parse_if_blocks('main');
			$ilance->template->pprint('main', $pprint_array);
			exit();
 }		
		  ...
		  else
		  {
			  ...
			  for($k=0;$k<count($att_id);$k++)
			  {
			   ...
			   //delete that file from the server
			  	$sql1="SELECT project_id,filehash,filename FROM " . DB_PREFIX . "attachment WHERE attachid = '".$att_id[$k]."'";
			  	...
			  	if($ilance->db->num_rows($result1)>0)
			  	{
			  		while($line1= $ilance->db->fetch_array($result1))
			  		{
			  			...
			  			$filename=$line1['filename'];
			  			$filehash=$line1['filehash'];
			  			$delete_thumbs=false;
			  			$thumbs=preg_split('/\s+/', trim(shell_exec('find /home/gc/public_html/image/ -name "'.$filename.'"')));
			  			...
			  			if($delete_thumbs)
			  			{
				  			$path=DIR_AUCTION_ATTACHMENTS.floor($project_id/100).'00/'.$project_id.'/'.$filehash.'.attach';
				  			$file_removed=unlink($path);
				  			if($file_removed)
				  			{
				  				$sqlusercheck_delete = $ilance->db->query("DELETE FROM " . DB_PREFIX . "attachment WHERE attachid = '" . $att_id[$k] . "'", 0, null, __FILE__, __LINE__);
				  			}
			  			}
			  		}
			  	}
