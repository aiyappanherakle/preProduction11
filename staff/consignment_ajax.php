...
$sqlcat_siteid = $ilance->db->query("
	 ...
	 if(isset($_GET['check_pcgs']))
	 {
							...
							// murugan changes on dec 13
								...
								echo '<input type="hidden" name="coindtyear" value="'.$rescat_pcg['coin_detail_year'].'" id="coindtyear">
...
if(isset($ilance->GPC['cmd']) AND $ilance->GPC['cmd'] == '_check-user' AND isset($ilance->GPC['userid']) AND $ilance->GPC['userid'] > 0)
{
	...
	$sql = $ilance->db->query("SELECT user_id,status FROM " . DB_PREFIX . "users WHERE user_id = '".intval($ilance->GPC['userid'])."' AND status = 'active'");
