...
{ 
 	$first_sql="SELECT u.user_id,u.username,u.first_name,u.last_name,u.status,DATE_FORMAT(date(u.date_added), '%m/%d/%Y') as user_date_added,
						u.email,u.phone,u.client_representative,c.totalunpaid,i.scheduledamount,i.paratialamount,
						i.partial_invoiceid,c.pending_invoiceid,c.oldamount,c.newamount,
						CASE WHEN partial_invoiceid > 0 THEN partial_invoiceid ELSE pending_invoiceid END  as invoice_id_column,
						IFNULL(totalunpaid,0)+IFNULL(scheduledamount,0)  as sortorder, sum(miscamount) as misctotal 
		    FROM ". DB_PREFIX . "users u
		    left join (select user_id,sum(amount) as scheduledamount,sum(paid) as paratialamount,invoiceid as  partial_invoiceid 
				from ". DB_PREFIX . "invoices 
				where status='scheduled' group by user_id) i on u.user_id=i.user_id
		    left join (select t.user_id,SUM(t.amount+IFNULL(b.totalamount,0)) AS totalunpaid,t.invoiceid as pending_invoiceid,
					sum(CASE WHEN date(t.createdate) < '".FIFETEENDAYSAGO."' 
						 THEN t.amount+IFNULL(b.totalamount,0) 
						 ELSE 0 END) as oldamount,
					sum(CASE WHEN date(t.createdate) >= '".FIFETEENDAYSAGO."' 
						 THEN t.amount+IFNULL(b.totalamount,0) 
						 ELSE 0 END) as newamount 
			      from ". DB_PREFIX . "invoices t
			      left join ". DB_PREFIX . "invoices b on t.projectid=b.projectid 
					...
					and t.isfvf=0 
					and t.isif=0 
					and t.isenhancementfee=0 group by t.user_id) c on u.user_id=c.user_id
		    left join  ". DB_PREFIX . "invoices  m on u.user_id=m.user_id 
		    and m.status!='paid'
		    where i.scheduledamount is not null 
		    or c.oldamount is not null 
		    or c.newamount is not null
	...
	$table = ' <table style="font-size:15px;">
			<tr>
	<td size="20" family="helvetica" style="bold" >GC A/R ('.date('F d, Y',strtotime(DATETIME24H)).') CONFIDENTIAL</td>
			</tr>
		</table>
		<table border=1 style="font-size:15px;">
			<tr>
				...
				<td style="bold">Invoice ID</td>
				<td style="bold">Total Amount</td><td style="bold">Client rep</td>
				<td style="bold">Last InfoNote</td><td style="bold">F/U Date</td>
			</tr>';				
	...
	if($ilance->db->num_rows($sql) > 0)
    {
	    while($res = $ilance->db->fetch_array($sql))
	    {
            if(($res['client_representative'] != 'ian') && ($res['client_representative'] != 'Ian'))
           	{	 
              	...
              	$user_note =$ilance->db->query("select * 
								FROM ". DB_PREFIX . "users_note 
				...
				$invoice_id_k =$res['invoice_id_column']; 			
			...
			{
				$table .= '<td style="bold">'.strtolower($res['username']).'<br><strong>&nbsp;&nbsp;'. $res['first_name'].'&nbsp;'.$res['last_name'].'&nbsp;['.strtoupper($res['status']).']&nbsp;'.'</strong><br>'.strtolower($res['email']).'</td>';
			}	
			else
			{
				$table .= '<td style="bold">'.strtolower($res['username']).'<br><strong>&nbsp;&nbsp;'. $res['first_name'].'&nbsp;'.$res['last_name'].'</strong><br>'.strtolower($res['email']).'</td>';
			}						   				
				  				
				  	 $table .= '<td style="bold" width="5">'.$res['phone'].'</td>
				   				<td style="bold"  width="12">'.$res['user_date_added'].'</td>';
			if($res['paratialamount'] >0)
			{
				$table .= '<td style="bold"  width="12">*'.$ilance->currency->format_no_text($res['scheduledamount']+$res['misctotal']).'</td>';
			}
			else
		   	{
				$table .= '<td style="bold"  width="12">'.$ilance->currency->format_no_text($res['scheduledamount']+$res['misctotal']).'</td>';
				...
				$table .= '<td style="bold">'.$invoice_id_k.'</td>
							<td style="bold">'.$totalunpaid.'</td>				   
		...
		$sql1="select t.user_id,p.project_title,date_format(p.date_end,'%m/%d/%Y') as END_DATE,SUM(t.amount+IFNULL(b.totalamount,0)) AS totalunpaid,
						IFNULL(b.totalamount,0) as buyerfee,t.amount as itemamount,t.projectid
				from ". DB_PREFIX . "invoices t
				left join ". DB_PREFIX . "invoices b on t.projectid=b.projectid 
							and t.user_id=b.user_id 
							and t.invoiceid!=b.invoiceid 
				...
				and t.isfvf=0 
				and t.isif=0 
				and t.isenhancementfee=0  
				and t.user_id='".$res['user_id']."' 
		...
		$res1 = $ilance->db->query($sql1, 0, null, __FILE__, __LINE__);
				...
				$table.='<tr bgcolor="#f1f1f1" height="10" border=0 size=7><td  border=0 >&nbsp;</td>
										...
										<td border=0  colspan="3" >'.$line1['itemamount'].'&nbsp;&nbsp;<font color="#CC3366">'.$line1['buyerfee'].'</font>&nbsp;&nbsp;'.$line1['totalunpaid'].'</td>										
										</tr>';
		  		...
		  		$table .='<tr>
		  					<td colspan ="4" align="right">Total Scheduled </td> 
	  			...
	  			$table .='<tr>
	  						<td colspan ="4" align="right">Total Unpaid </td> 
							<td> </td>
	   			...
	   			$table .='<tr>
	   						<td colspan ="4" align="right">Total Money Owed </td> 
							<td> </td>
							<td></td>
							<td> </td>
							<td>'.$ilance->currency->format_no_text(array_sum($arraytotal)).'</td>
						</tr>';
	}	
