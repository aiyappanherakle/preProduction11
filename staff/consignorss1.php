...
if (!empty($_SESSION['ilancedata']['user']['userid']) AND $_SESSION['ilancedata']['user']['userid'] > 0 AND $_SESSION['ilancedata']['user']['isstaff'] == '1')
{
	...
	{ 
		...
		$formatted_due_date=date('m/d/Y',strtotime($stmt_date.'+8 days'));
		...
		from 
			".DB_PREFIX."coins c 
		left join
			".DB_PREFIX."projects p on c.coin_id=p.project_id and p.user_id=1605 and DATEDIFF(date_end,date_starts )>1 
		left join 
			...
			where date(actual_end_date)<='".$stmt_date."' and user_id=1605 and DATEDIFF(enddate,startbydate)>1 group by coin_id) r on r.coin_id=c.coin_id
		left join
			(select sum(qty) as order_count,project_id,orderdate from ".DB_PREFIX."buynow_orders o where owner_id=1605 and date(orderdate)<='".$stmt_date."' and date(orderdate)>date(
			(
			...
			where date(actual_end_date)<'".$stmt_date."' and user_id=1605 and DATEDIFF(enddate,startbydate)>1 and coin_id=o.project_id order by actual_end_date desc limit 1
			)
		...
		left join
			...
			from ".DB_PREFIX."invoices i where (i.user_id=1605 or i.p2b_user_id=1605) and date(i.createdate)<='".$stmt_date."' and date(i.createdate)>date((
			select CASE WHEN count(actual_end_date)>0 THEN max(actual_end_date)   ELSE '0000-00-00 00:00:00' END as last_listed_time from ilance_coin_relist 
			where date(actual_end_date)<'".$stmt_date."' and user_id=1605 and DATEDIFF(enddate,startbydate)>1 and coin_id=i.projectid order by actual_end_date desc limit 1
			))  group by i.projectid) i on c.coin_id=i.projectid 	

		where  (c.user_id=1605 and (
	...
	$headings[1]='Name';
	$headings[2]='Statement_Date';
	$headings[3]='Consignor_Payment_Due';
	//$headings[4]='User_ID';
	...
	$headings[9]='Account_name';
	$headings[10]='Line_Item_Total';
	$headings[11]='Description';
