...
$query1="SELECT * FROM " . DB_PREFIX . "ebay_listing where ebay_id=181543051062";
// echo $query1;exit;
...
if($ilance->db->num_rows($sql1))
{
	while($line1 = $ilance->db->fetch_array($sql1))
	{
			...
			foreach ($responses as $response)
			{
				...
				{	
					...
					foreach ($BuyerArray as $Buyerreg)
					{
						...
						if(transaction_not_accounted($transaction_id))
						{
							$buyer_id=getbuyer_registered($Buyerreg);
							if(isset($buyer_id) && $buyer_id > 0)
							{
							$sql="SELECT e.*,c.user_id,c.Title FROM " . DB_PREFIX . "ebay_listing e left join ".DB_PREFIX."coins c on c.coin_id=e.coin_id WHERE ebay_id = '".$ebay_coin_id."'";
								...
								", 0, null, __FILE__, __LINE__);
										...
										$invoiceid = $ilance->db->insert_id();
															...
															// vijay work for seller fee calculaton 
									
						$fvf = calculate_ebay_fvf_value($project_id,$CoinAmount,$buyer_id,$seller_id);

							
						$mode='charge';	
									 // check if we're exempt from final value fees //AND $ilance->subscription->check_access($buyer_id, 'fvfexempt') == 'yes'
                        ...
                        if ($fvf > 0)
                        {
								...
								$extrainvoicesql = '';
								if ($ilance->tax->is_taxable(intval($buyer_id), 'finalvaluefee'))
								{
								// #### fetch tax amount to charge for this invoice type
								$taxamount = $ilance->tax->fetch_amount(intval($buyer_id), $fee, 'finalvaluefee', 0);
								...
								$taxinfo = $ilance->tax->fetch_amount(intval($buyer_id), $fee, 'finalvaluefee', 1);
                                ...
                                if ($mode == 'charge')
                                {
												...
												$fvfinvoiceid = $ilance->db->insert_id();
                                                        ...
                                                        $ilance->db->query("
                                                                ...
                                                                isfvfpaid = '1',
                                                                fvfinvoiceid = '" . intval($fvfinvoiceid) . "'
                                                        ...
                                                        update_referral_action('fvf', intval($buyer_id));                                                       
						...
						{
							//change ebay_listing to partial status
		...
		{
			global $ilance;
			//echo "SELECT * FROM  ".DB_PREFIX."ebay_listing_rows WHERE  ebay_Transaction_id = '".$transaction_id."'";
...
//			$BuyerregCoun   		= $BuyerregCountry->item(0)->nodeValue;
			...
			if (empty($BuyerregCoun))
			{
				$BuyerregCon = fetch_country_id($BuyerregCoun);
			 ...
			 if (!empty($Buyerregusr) )
                {
					...
					{  
						...
						'eBay',
						'" . $ilance->db->escape_string($BuyerregCon) . "',
                        '" . DATETIME24H . "',
                        'active',
                        '" . DATETIME24H . "',
                        '0000-00-00',
