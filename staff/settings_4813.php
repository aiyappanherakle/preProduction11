...
|| # -------------------------------------------------------------------- # ||
|| # Copyright ©2000ñ2010 ILance Inc. All Rights Reserved.                # ||
  ...
  $sql = $ilance->db->query("
	   ...
	   if (isset($ilance->GPC['cmd']) AND $ilance->GPC['cmd'] == 'update_watch_count')
	   {
		...
		if ($check > $today) 
		{
				...
				order by CASE WHEN (cid = '6000120' OR cid = '6000127' OR cid = '6000128' OR cid = '6000129') THEN project_title END asc, Orderno				
				");
...
//-->
   ...
   {                                                    
    ...
    {
    $html.= '<tr class="alt1"><td align="center" style="width: 50px;">'.$count.'</td><td style="width: 500px;">'.strtr($ilance->bbcode->bbcode_to_html($row['message']),"¿¡¬√ƒ≈‡·‚„‰Â“”‘’÷ÿÚÛÙıˆ¯»… ÀËÈÍÎ«ÁÃÕŒœÏÌÓÔŸ⁄€‹˘˙˚¸ˇ—Ò","").'</td><td align="center" style="width: 100px;">'.$row['date_time'].'</td><td align="center"><a href="settings.php?id='.$row['id'].'&cmd=timeline&action=edit"><img src="'.$ilconfig['template_relativeimagepath'].$ilconfig['template_imagesfolder'].'icons/pencil.gif" alt="" border="0" /></a></td><td align="center"> <a href="settings.php?id='.$row['id'].'&cmd=timeline&action=delete">  <img src="'.$ilconfig['template_relativeimagepath'].$ilconfig['template_imagesfolder'].'icons/delete.gif" alt="" border="0" /></a> </td></tr><tr></tr>'   ;
    $count++;
    }
  ...
  if (isset($ilance->GPC['subcmd']) AND $ilance->GPC['subcmd'] == 'search')
	{
				...
				$search_text = $ilance->GPC['banner_name'];
				$level1=$ilance->db->query("SELECT * FROM " . DB_PREFIX . "banner  WHERE inner_text LIKE '%".$ilance->GPC['banner_name']."%' OR filename LIKE '%".$ilance->GPC['banner_name']."%' OR image_link LIKE '%".$ilance->GPC['banner_name']."%' OR image_text LIKE '%".$ilance->GPC['banner_name']."%' LIMIT " . (($ilance->GPC['page'] - 1) * $ilconfig['maxrowsdisplay']) . "," . $ilconfig['maxrowsdisplay']);
				
				$pagen=$ilance->db->query("SELECT * FROM " . DB_PREFIX . "banner  WHERE inner_text LIKE '%".$ilance->GPC['banner_name']."%' OR filename LIKE '%".$ilance->GPC['banner_name']."%' OR image_link LIKE '%".$ilance->GPC['banner_name']."%' OR image_text LIKE '%".$ilance->GPC['banner_name']."%'");
				...
				$banner1['order']= $row1['display_order'];
            ...
            $banner1['edit'] = '<a href="' . $ilpage['settings'] . '?subcmd=updatebanner&amp;id=' . $row1['id'] . '"><img src="' . $ilconfig['template_relativeimagepath'] . $ilconfig['template_imagesfolder'] . 'icons/pencil.gif" alt="" border="0"></a>';
            $banner1['remove'] = '<a href="' . $ilpage['settings'] . '?subcmd=deletebanner&amp;id=' . $row1['id'] . '" onClick="return confirm(\'' . $phrase['_please_take_a_moment_to_confirm_your_action'] . '\')"><img src="' . $ilconfig['template_relativeimagepath'] . $ilconfig['template_imagesfolder'] . 'icons/delete.gif" alt="" border="0"></a>';
           $banner1['status'] = ($row1['visible'] == '1') ? '<a href="' . $ilpage['settings'] . '?subcmd=suspendbanner&amp;id=' . $row1['id'] . '"><img src="' . $ilconfig['template_relativeimagepath'] . $ilconfig['template_imagesfolder'] . 'checked.gif" alt="Click to Deactivate banner" border="0"></a>' : '<a href="' . $ilpage['settings'] . '?subcmd=activatebanner&amp;id=' . $row1['id'] . '"><img src="' . $ilconfig['template_relativeimagepath'] . $ilconfig['template_imagesfolder'] . 'unchecked.gif" alt="Click to re-activate banner " border="0"></a>';
	...
	else if(isset($ilance->GPC['subcmd']) AND $ilance->GPC['subcmd'] == 'listingorder')
	{
	  ...
	  $sql_banner = $ilance->db->query("SELECT *
       ...
       while ($row= $ilance->db->fetch_array($sql_banner)) 
	   {
			...
			$banner['order']= $row['display_order'];
            ...
            $banner['edit'] = '<a href="' . $ilpage['settings'] . '?subcmd=updatebanner&amp;id=' . $row['id'] . '"><img src="' . $ilconfig['template_relativeimagepath'] . $ilconfig['template_imagesfolder'] . 'icons/pencil.gif" alt="" border="0"></a>';
            $banner['remove'] = '<a href="' . $ilpage['settings'] . '?subcmd=deletebanner&amp;id=' . $row['id'] . '" onClick="return confirm(\'' . $phrase['_please_take_a_moment_to_confirm_your_action'] . '\')"><img src="' . $ilconfig['template_relativeimagepath'] . $ilconfig['template_imagesfolder'] . 'icons/delete.gif" alt="" border="0"></a>';
           $banner['status'] = ($row['visible'] == '1') ? '<a href="' . $ilpage['settings'] . '?subcmd=suspendbanner&amp;id=' . $row['id'] . '"><img src="' . $ilconfig['template_relativeimagepath'] . $ilconfig['template_imagesfolder'] . 'checked.gif" alt="Click to Deactivate banner" border="0"></a>' : '<a href="' . $ilpage['settings'] . '?subcmd=activatebanner&amp;id=' . $row['id'] . '"><img src="' . $ilconfig['template_relativeimagepath'] . $ilconfig['template_imagesfolder'] . 'unchecked.gif" alt="Click to re-activate banner " border="0"></a>';
  ...
  if (isset($ilance->GPC['subcmd']) AND $ilance->GPC['subcmd'] == 'banner_upload')
	{
	   if($_FILES['upload'])
	   {
	    ...
	    $ilance->db->query("insert into ".DB_PREFIX."banner 
	                         values (0,'".$name."','".$type."','".$size."','".$hash."','".$ilance->GPC['link']."','".$inner_text_1."','".$inner_link_1."','0','0','".DATETIME24H."','".$inner_text_2."','".$inner_text_3."','".$inner_link_2."','".$inner_link_3."','')");
...
$firsttop=$ilance->db->query("INSERT INTO " . DB_PREFIX . "coin_show(coin_date,coin_desc,coin_end_date) values('".$ilance->GPC['show_start_date']."','".$ilance->db->escape_string($ilance->GPC['show_desc'])."','".$ilance->GPC['show_end_date']."')");
...
if(isset($ilance->GPC['cmd']) AND $ilance->GPC['cmd'] =='banner_order_change')
{
   ...
   if ($str >=1 )
   {
		...
		for($i=0;$i<count($new_listing);$i++)
		{		
		$banner_found = $ilance->db->query("SELECT *
				                        FROM " . DB_PREFIX . "banner
										WHERE filename LIKE  '%" . $ilance->db->escape_string(trim($new_listing[$i])) . "%'  			                        
									  ");
			$status = 0;	
    ...
    if($status == 0)
     { 		
		...
		echo "UPDATE " . DB_PREFIX . "banner
						SET visible = 0,
						display_order = 0
						WHERE display_order = '" . $ilance->db->escape_string($order1) . "'";
		echo '</br>';			 */	
	...
	/* echo "UPDATE " . DB_PREFIX . "banner
					SET visible = 1,
					display_order = '" . $ilance->db->escape_string($order1) . "'
					WHERE filename = '" . $ilance->db->escape_string(trim($new_listing[$i])) . "'	";				
   echo '</br>';
    echo '</br>'; */
			 $ilance->db->query("
