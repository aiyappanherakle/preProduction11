...
*/
	...
	case "update_coin_mintmarks":
		
		$cat_qury = $ilance->db->query("SELECT coin_series_denomination_no, GROUP_CONCAT(DISTINCT mintmark ORDER BY mintmark ASC SEPARATOR ',') as  mintmarks, MIN(coin_detail_year) as fromyear, MAX(coin_detail_year) as toyear FROM ".DB_PREFIX."projects WHERE mintmark!='' GROUP BY coin_series_denomination_no");

		if ($ilance->db->num_rows($cat_qury) > 0)
		{
			while($cat_reslt=$ilance->db->fetch_array($cat_qury, DB_ASSOC))
			{
				$denom_no = $cat_reslt['coin_series_denomination_no'];
				$fromyr = $cat_reslt['fromyear'];
				$toyr = $cat_reslt['toyear'];
				$mntmrks = $cat_reslt['mintmarks'];
				
				$cat_qury1 = "UPDATE ".DB_PREFIX."catalog_toplevel_4567 SET denomination_year_from = '".$fromyr."', denomination_year_to = '".$toyr."', denomination_mintmarks= '".$mntmrks."' where denomination_unique_no='".$denom_no."'";
				 $result1=$ilance->db->query($cat_qury1);
				

			}
		}
		
		$scnd_cat_qury = $ilance->db->query("SELECT coin_series_denomination_no, coin_series_unique_no, GROUP_CONCAT(DISTINCT mintmark ORDER BY mintmark ASC SEPARATOR ',') as  mintmarks, MIN(coin_detail_year) as fromyear, MAX(coin_detail_year) as toyear FROM ilance_projects WHERE mintmark!='' GROUP BY coin_series_unique_no");
		
		if ($ilance->db->num_rows($scnd_cat_qury) > 0)
		{
			while($scnd_cat_reslt=$ilance->db->fetch_array($scnd_cat_qury, DB_ASSOC))
			{
			
				// echo '<pre>';
				// print_r($scnd_cat_reslt);
				// exit;
				$denom_no1 = $scnd_cat_reslt['coin_series_denomination_no'];
				$unique_no1 = $scnd_cat_reslt['coin_series_unique_no'];
				$fromyr1 = $scnd_cat_reslt['fromyear'];
				$toyr1 = $scnd_cat_reslt['toyear'];
				$mntmrks1 = $scnd_cat_reslt['mintmarks'];
				
				$scnd_cat_qury1 = "UPDATE ".DB_PREFIX."catalog_second_level_4567 SET coin_series_date_from = '".$fromyr1."', coin_series_date_to = '".$toyr1."', coin_series_mintmarks= '".$mntmrks1."' where coin_series_unique_no='".$unique_no1."' AND coin_series_denomination_no='".$denom_no1."'";
				//echo $scnd_cat_qury1;exit;
				 $result1=$ilance->db->query($scnd_cat_qury1);
		...
		print_action_success("Coin Mintmark details and category tables are Synchronized sucessfully", "catalog.php");
		exit;
		break;
	 
		
		}
		print_action_success("Cion details and category tables are sucessfully Synchronized", "catalog_4567.php");
		exit;
	}             
			 ...
			 {           
						  ...
						  if (isset($ilance->GPC['subcmd']) AND $ilance->GPC['subcmd'] == 'deletetoplevel') 
						 {
						  $sqlusercheck_delete = $ilance->db->query("
														...
														DELETE FROM " . DB_PREFIX . "catalog_toplevel_4567
						                           ...
						                           if($_POST['id']=='')
													...
													else
													{
													  $sqlusercheck_my = $ilance->db->query("
														UPDATE  " . DB_PREFIX . "catalog_toplevel_4567
								 ...
								 if(isset($ilance->GPC['id']))
								 {
								  ...
								  //toplevel db column name fetch
									 $top_level_fieldname = $ilance->db->query("
															SELECT * FROM " . DB_PREFIX . "catalog_toplevel_4567
															");
									  $i = 0;
	...
	$ilance->template->fetch('main', 'catalog_update_4567.html', 2);
...
else
{
					...
					$top_level_search = $ilance->db->query("
										...
										$row_top_search['edit'] = '<a href="catalog_4567.php?cmd=update-catalog&subcmd=update-toplevel&id='.$row_top_search['id'].'"><img border="0" alt="" src="' . $ilconfig['template_relativeimagepath'] . $ilconfig['template_imagesfolder'] . '/icons/pencil.gif"></a>';
										$row_top_search['delete'] = '<a onclick="return confirm(\'Please take a moment to confirm your action. Continue?\')" href="catalog_4567.php?cmd=update-catalog&subcmd=deletetoplevel&amp;id='.$row_top_search['id'].'"><img border="0" alt="" src="' . $ilconfig['template_relativeimagepath'] . $ilconfig['template_imagesfolder'] . '/icons/delete.gif"></a>';
 ...
 database and number of columns in file are not the same. Please fix the problem and retry your action.", $_SERVER['PHP_SELF'].'');
								...
								if($_POST['type_insert'] == 'appendtolist')
								{
										...
										$top_type_insert = $ilance->db->query("
												SELECT * FROM " . DB_PREFIX . "catalog_toplevel_4567  WHERE denomination_unique_no ='".$data[0]."'");
												...
												{
												$ilance->db->query("
												INSERT INTO " . DB_PREFIX . "catalog_toplevel_4567
				...
				else 
				{
					...
					$top_level = $ilance->db->query("
										...
										$row_top['edit'] = '<a href="catalog_4567.php?cmd=update-catalog&subcmd=update-toplevel&id='.$row_top['id'].'"><img border="0" alt="" src="' . $ilconfig['template_relativeimagepath'] . $ilconfig['template_imagesfolder'] . '/icons/pencil.gif"></a>';
										$row_top['delete'] = '<a onclick="return confirm(\'Please take a moment to confirm your action. Continue?\')" href="catalog_4567.php?cmd=update-catalog&subcmd=deletetoplevel&amp;id='.$row_top['id'].'"><img border="0" alt="" src="' . $ilconfig['template_relativeimagepath'] . $ilconfig['template_imagesfolder'] . '/icons/delete.gif"></a>';
					...
					$top_level_search = $ilance->db->query("
										...
										$row_top_search['edit'] = '<a href="catalog_4567.php?cmd=update-catalog&subcmd=update-secondlevel&id='.$row_top_search['id'].'"><img border="0" alt="" src="' . $ilconfig['template_relativeimagepath'] . $ilconfig['template_imagesfolder'] . '/icons/pencil.gif"></a>';
										$row_top_search['delete'] = '<a onclick="return confirm(\'Please take a moment to confirm your action. Continue?\')" href="catalog_4567.php?cmd=update-catalog&subcmd=deletesecondlevel&amp;id='.$row_top_search['id'].'"><img border="0" alt="" src="' . $ilconfig['template_relativeimagepath'] . $ilconfig['template_imagesfolder'] . '/icons/delete.gif"></a>';
 ...
 database and number of columns in file are not the same. Please fix the problem and retry your action.", $_SERVER['PHP_SELF'].'');
				...
				else 
				{
					...
					$second_level = $ilance->db->query("
										...
										$row_second['edit'] = '<a href="catalog_4567.php?cmd=update-catalog&subcmd=update-secondlevel&id='.$row_second['id'].'"><img border="0" alt="" src="' . $ilconfig['template_relativeimagepath'] . $ilconfig['template_imagesfolder'] . '/icons/pencil.gif"></a>';
										$row_second['delete'] = '<a onclick="return confirm(\'Please take a moment to confirm your action. Continue?\')" href="catalog_4567.php?cmd=update-catalog&subcmd=deletesecondlevel&amp;id='.$row_second['id'].'"><img border="0" alt="" src="' . $ilconfig['template_relativeimagepath'] . $ilconfig['template_imagesfolder'] . '/icons/delete.gif"></a>';
...
//								else if(($data[0] == '') || ($data[1] == '') || ($data[2] == '') || ($data[3] == '') || ($data[4] == ''))
				...
				else 
				{
				   ...
				   //Coin list select
				       $coin_list_level = $ilance->db->query("
										...
										$row_coin_list['edit'] = '<a href="catalog_4567.php?cmd=update-catalog&subcmd=update-coinlist&id='.$row_coin_list['id'].'"><img border="0" alt="" src="' . $ilconfig['template_relativeimagepath'] . $ilconfig['template_imagesfolder'] . '/icons/pencil.gif"></a>';
										$row_coin_list['delete'] = '<a onclick="return confirm(\'Please take a moment to confirm your action. Continue?\')" href="catalog_4567.php?cmd=update-catalog&subcmd=deletecoinlist&amp;id='.$row_coin_list['id'].'"><img border="0" alt="" src="' . $ilconfig['template_relativeimagepath'] . $ilconfig['template_imagesfolder'] . '/icons/delete.gif"></a>';
				   ...
				   if (isset($ilance->GPC['cmd']) AND $ilance->GPC['cmd'] == 'search_coin')
					...
					$top_level_search = $ilance->db->query("
										...
										$row_coin_search['edit'] = '<a href="catalog_4567.php?cmd=update-catalog&subcmd=update-coinlist&id='.$row_coin_search['id'].'"><img border="0" alt="" src="' . $ilconfig['template_relativeimagepath'] . $ilconfig['template_imagesfolder'] . '/icons/pencil.gif"></a>';
										$row_coin_search['delete'] = '<a onclick="return confirm(\'Please take a moment to confirm your action. Continue?\')" href="catalog_4567.php?cmd=update-catalog&subcmd=deletecoinlist&amp;id='.$row_coin_search['id'].'"><img border="0" alt="" src="' . $ilconfig['template_relativeimagepath'] . $ilconfig['template_imagesfolder'] . '/icons/delete.gif"></a>';
	...
	$ilance->template->fetch('main', 'catalog_listing_4567.html', 2);
	$ilance->template->parse_hash('main', array('ilpage' => $ilpage));
