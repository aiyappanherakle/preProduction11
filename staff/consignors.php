...
if (!empty($_SESSION['ilancedata']['user']['userid']) AND $_SESSION['ilancedata']['user']['userid'] > 0 AND $_SESSION['ilancedata']['user']['isadmin'] == '1')
{
	...
	{ 
		...
		group by c.coin_id ORDER BY  c.coin_id ASC";
			...
			if($statement_final_total>0)
			{
				...
				$data['summary']='Auction System Consignor Statement';
				...
				$data['user_id']=$user_id;
				$data['summary']='Auction System Consignor Statement';
				$data['description']='Seller Fees';
				$data['account_name']='4030';
				...
				$data['user_id']=$user_id;
				$data['summary']='Auction System Consignor Statement';
				$data['description']='Listing Fees';
				$data['account_name']='4040';
	...
	$headings[7]='Line_Item_Total';
	$headings[8]='Tags';


	$reportoutput = $ilance->admincp->construct_csv_data($res, $headings);
	$timeStamp = date("Y-m-d-H-i-s");
	$fileName = "Consignors_summary_datails-$timeStamp";
	$action = 'csv';
		if ($action == 'csv')
		{
			header("Pragma: cache");
	...
	$ilance->template->fetch('main', 'consignors.html', 2);
