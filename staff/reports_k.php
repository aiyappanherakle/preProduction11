...
//Karthik may 04
			     ...
			     if (isset($ilance->GPC['cmd']) AND $ilance->GPC['cmd'] == 'statement_print_new')
		       {
					...
					if(isset($ilance->GPC['print']))
					  header("Location:statement_new_k.php?start_date=".$ilance->GPC['start_date']."&option=".$ilance->GPC['display'].$user_id_query);
					else if(isset($ilance->GPC['summary']))
					  header("Location:statement_new_summary_k.php?start_date=".$ilance->GPC['start_date']."&option=".$ilance->GPC['display'].$user_id_query);
		...
		$conds = "AND p.haswinner != 1 AND p.hasbuynowwinner != 1";
		 ...
		 {
		 $sql="SELECT c.coin_id, c.Title, c.Certification_No,c.Buy_it_now,c.Minimum_bid,c.pcgs FROM
											" .DB_PREFIX. "coins c left join " . DB_PREFIX . "catalog_coin cc on c.pcgs = cc.PCGS
											WHERE c.user_id = '".$userid."' 
											AND c.sold_qty =0
											GROUP BY c.coin_id
											ORDER BY  cc.Orderno ,(CASE WHEN c.pcgs = '6000120' THEN c.title END) ASC,c.grade DESC
											 ";
		...
		if(empty($ilance->GPC["excsold"])){		
		 $sql="SELECT c.coin_id, c.Title, c.Certification_No,c.pcgs  FROM 
