...
{ 
	...
	if(isset($ilance->GPC['cmd']) AND $ilance->GPC['cmd'] == 'allcheck_return_consign')
	{
	    ...
	    if($ilance->GPC['val'] == '')
		{
			print_action_failed('sorry atleast select one checkbox or dropdown val', 'users.php');
			exit();
		}
		else 
		{
			...
			{
				print_action_failed('please select without bids coins', 'users.php');
				exit();
			}
 			else
 			{
				...
				if($ilance->db->num_rows($sql_username) ==1)
				{
					...
					$ilance->template->fetch('main', 'current_consign_coins.html', 2);
	...
	{	
		...
		$country=fetch_user('country',$res_return['user_id']);
		if($country !=500)
		{
		   $country_name = '<br>'.print_user_country($res_return['user_id']);
		}
		else
		{
			$country_name ='';
		...
		$seller_name=$res_return['first_name']." ".$res_return['last_name']."<br/>
						...
						$prt_addr2.
						$res_return['city']." ".$res_return['state']." ".$res_return['zip_code'].$country_name."<br/><br/>
						Tel: ".$res_return['phone']."<br/>
		...
		$sql_multiple_return = 	$ilance->db->query("SELECT c.*,r.Title,r.consignid,r.Minimum_bid as Minimum_bid,r.Buy_it_now as Buy_it_now  FROM ".DB_PREFIX."coin_return  c
									left join " . DB_PREFIX . "coins_retruned r on c.coin_id = r.coin_id 
									WHERE c.coin_id in(".$coin_id_list.")
									GROUP BY coin_id
									ORDER BY coin_id ASC
