...
$user_where=isset($ilance->GPC['user_id'])?'  WHERE  u.user_id='.$ilance->GPC['user_id']:'';
 ...
 if(isset($ilance->GPC['user_id']) and $ilance->GPC['user_id']>0)
		...
		FROM " . DB_PREFIX . "users u
		left join (select user_id from ilance_coins where date(end_date)='".$stmt_date."' group by user_id) c on c.user_id=u.user_id
		left join (select user_id from ilance_coin_relist where date(actual_end_date)='".$stmt_date."'  group by user_id) r on r.user_id=u.user_id
		...
		where r.user_id is not null or c.user_id is not null
		group by u.user_id
...
$country=fetch_user('country',$user_id);
if($country !=500)
{
   $country_name = '<br>'.print_user_country($user_id);
}
else
{
	$country_name ='';
...
$new_header.=$city.' &nbsp; '.$state.' &nbsp; '.$zipcode.$country_name.'</td>
</tr>
<tr>
<td>&nbsp;
</td>
