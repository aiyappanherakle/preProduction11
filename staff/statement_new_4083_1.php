...
left join 
	(select coin_id,user_id,count(id) as no_relist_b4_statement,date_format(max(actual_end_date),'%Y-%m-%d %H:%i:%s') as last_listed_time from ".DB_PREFIX."coin_relist 
...
left join
	...
	select CASE WHEN count(actual_end_date)>0 THEN date_format(max(actual_end_date),'%Y-%m-%d %H:%i:%s') ELSE '0000-00-00 00:00:00' END as last_listed_time from ilance_coin_relist 
