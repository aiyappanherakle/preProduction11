...
print_action_failed("Tracking number cannot be empty");
	...
	$html.='<table>';
		...
		$ilance->db->query("
                                ...
                                (coin_id, cust_id,buyer_id, shipper_id, track_no, shipment_date, email,invoice_id,final_invoice_id,if_returned)
                                VALUES (
			...
			// #### START EMAIL ############################################
                $sql_query=  $ilance->db->query("SELECT * FROM " . DB_PREFIX . "shippers where shipperid='".$ilance->GPC['shippserviece']."'");	
					while ($result = $ilance->db->fetch_array($sql_query, DB_ASSOC))
						...
						$htmls='https://tools.usps.com/go/TrackConfirmAction?qtc_tLabels1='.$ilance->GPC['tracking_no'];
																	
						if($shipper_id==35)
						{
						$htmls='https://www.fedex.com/fedextrack/WTRK/index.html?action=track&trackingnumber='.$res['track_no'];
						}
			    ...
			    $ilance->email->get('inform_shipping_tracking_no_to_admin_new');		
                $ilance->email->set($existingnew);
                $ilance->email->send();
 ...
 database and number of columns in file are not the same. Please fix the problem and retry your action.", $_SERVER['PHP_SELF'].'?module=lanceactivity');
							exit();
								...
								$temp_data['email'] = $data[5];	
										 ...
										 $ilance->db->query("
                                					INSERT INTO " . DB_PREFIX . "shippnig_details
                               						 (coin_id, cust_id, shipper_id, track_no, shipment_date, email,if_returned))
                               						 VALUES (
