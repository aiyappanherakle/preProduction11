...
if (!empty($_SESSION['ilancedata']['user']['userid']) AND $_SESSION['ilancedata']['user']['userid'] > 0 AND $_SESSION['ilancedata']['user']['isstaff'] == '1')
{
	...
	if(isset($ilance->GPC['cmd']) and $ilance->GPC['cmd']=='coins_search')
	{
	 ...
	 $sql="
	SELECT p.*,c.coin_id ,c.pcgs,c.Certification_No,e.coin_id as ebay_coin_id,e.listedon,cr.coin_id AS coin_return,
	el.amount,el.invoice_status,el.coin_id as ebay_row_coin_id,el.buyer_id,c.consignid,
	c.Site_Id,c.Alternate_inventory_No,c.Title,c.Minimum_bid,c.user_id,c.relist_count,
	c.End_Date,c.Buy_it_now,cr.return_id,cr.return_date FROM " . DB_PREFIX . "coins AS c
	LEFT JOIN  " . DB_PREFIX . "projects AS p ON p.project_id = c.coin_id
	LEFT JOIN  " . DB_PREFIX . "coin_return AS cr ON cr.coin_id = c.coin_id
				...
				");
							...
							{	
								while($sellinvpjt = $ilance->db->fetch_array($sellinvprojt))
								{
									...
									$listpagesold['Shipped']='<strong>' .$shipped .'</strong>';
									$listpagesold['Shipped_Date']='<strong>' . date("F d,Y",strtotime($shippeddate)) .'</strong>';
									$listpagesold['track_no']='<strong>' .$track_no.'</strong>';$listpagesold['Shipped']='<strong>' .$shipped .'</strong>';
									$listpagesold['Shipped_Date']='<strong>' . date("F d,Y",strtotime($shippeddate)) .'</strong>';
									if($shipped == '--')
									{
										$listpagesold['Shipped_Date']='--';
	...
	$ilance->template->fetch('main', 'coins_search_1.html', 2);
