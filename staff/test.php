...
echo $sql="SELECT p.project_id,a.attachid,a.filehash,p.date_end FROM ilance_attachment a left join ilance_projects p on p.project_id=a.project_id WHERE p.`date_end` < '2012-08-01 00:00:00' AND p.`haswinner` = 1 and a.filehash is not null 
    ...
    while($line= $ilance->db->fetch_array($result))
    {
        ...
        $coin_id=$line['project_id'];
                ...
                if(is_writable($m))
                {
                    ...
                    {
                        echo "<br> source note redable ".$source ;
                    }
                }else
                {
