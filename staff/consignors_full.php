...
if (!empty($_SESSION['ilancedata']['user']['userid']) AND $_SESSION['ilancedata']['user']['userid'] > 0 AND $_SESSION['ilancedata']['user']['isstaff'] == '1')
{
	...
	{ 
		...
		left join 
			(select coin_id,user_id,count(id) as no_relist_b4_statement,max(actual_end_date) as last_listed_time from ".DB_PREFIX."coin_relist_v1
		...
		left join
			(select sum(qty) as order_count,project_id,orderdate from ".DB_PREFIX."buynow_orders o where owner_id='".$user_details['user_id']."' 
			
			and orderdate>(select CASE WHEN count(actual_end_date)>0 THEN max(actual_end_date)   ELSE '0000-00-00 00:00:00' END as last_listed_time from ilance_coin_relist
			where date(actual_end_date)<='".$start."' and user_id='".$user_details['user_id']."' and DATEDIFF(enddate,startbydate)>1 and coin_id=o.project_id order by actual_end_date desc limit 1)
			
			and orderdate<=(select CASE WHEN count(enddate)>0 THEN max(enddate)   ELSE concat('".$stmt_date."', ' 23:59:59') END as statement_date from ilance_coin_relist
			where date(startbydate)<'".$stmt_date."' and user_id='".$user_details['user_id']."' and DATEDIFF(enddate,startbydate)>1 and coin_id=o.project_id order by actual_end_date desc limit 1) 
		...
		left join
			...
			from ".DB_PREFIX."invoices i where (i.user_id=".$user_details['user_id']." or i.p2b_user_id=".$user_details['user_id'].") 
			
			and i.createdate>(select CASE WHEN count(actual_end_date)>0 THEN max(actual_end_date)   ELSE '0000-00-00 00:00:00' END as last_listed_time from ilance_coin_relist 
			where date(actual_end_date)<='".$start."' and user_id='".$user_details['user_id']."' and DATEDIFF(enddate,startbydate)>1 and coin_id=i.projectid order by actual_end_date desc limit 1) 
			
			and i.createdate<=(select CASE WHEN count(enddate)>0 THEN max(enddate)   ELSE concat('".$stmt_date."', ' 23:59:59') END as statement_date from ilance_coin_relist
			where date(startbydate)<'".$stmt_date."' and user_id='".$user_details['user_id']."' and DATEDIFF(enddate,startbydate)>1 and coin_id=i.projectid order by actual_end_date desc limit 1) 
		...
		group by c.coin_id ORDER BY  c.coin_id ASC";
			...
			{ 
				...
				if($coin_final_price>0)
				{
					...
					$data['line_total']='';
					$data['tags']='';
					$data['coin']=$coins_list_line['coin_id'];
					$data['enddate']=$coins_list_line['last_listed_time'];
					$data['amount']=$coin_final_price;
					$data['buynow_qty']=$no_of_buynow;
