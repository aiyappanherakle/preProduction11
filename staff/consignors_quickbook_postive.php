...
if (!empty($_SESSION['ilancedata']['user']['userid']) AND $_SESSION['ilancedata']['user']['userid'] > 0 AND $_SESSION['ilancedata']['user']['isstaff'] == '1')
{
	...
	{ 
		...
		group by c.coin_id ORDER BY  c.coin_id ASC";
			...
			{ 
				...
				$data['name'] = $user_name;
				$data1['statement_number']=$statement_number;
				...
				{
					$data1['statement_date']=$formatted_statement_date;
					$data1['payment_due']=$formatted_due_date;
					//$data['user_id']=$user_id;
					$data1['address']=$user_address;
					$data1['address2']=$user_address1;
					$data1['city']=$user_city;
					$data1['state']=$user_state;
					$data1['zip_code']=$user_code;
					$data1['account_name']='5000 - Hammer - Consignor';
					$data1['line_total']=$statement_final_total;
					$data1['description']='Auction System Consignor Statement';
					$data1['msg']='Consignor Statement';
					$result[]=$data1;
				...
				else
				{
					...
					$data1['state']=$user_state;
					$data1['zip_code']=$user_code;
					$data1['account_name']='4030 - Seller Fees';
					$data1['line_total']='-'.$statement_seller_fee_total;
					$data1['description']='Auction System Consignor Statement';
					$data1['msg']='Consignor Statement';
					$result[]=$data1;
				}
				...
				else
				{
					...
					$data1['state']=$user_state;
					$data1['zip_code']=$user_code;
					$data1['account_name']='4040 - Listing Fees';
					$data1['line_total']='-'.$statement_listing_fee_total;
					$data1['description']='Auction System Consignor Statement';
					$data1['msg']='Consignor Statement';
					$result[]=$data1;
	...
	$fileName = "Consignors_positive-$timeStamp";
	$action = 'csv';
		if ($action == 'csv')
		{
			header("Pragma: cache");
	...
	$heading1[0]='RefNumber';
	$heading1[1]='Vendor';
	$heading1[2]='TxnDate';
	$heading1[3]='DueDate';
	//$headings[4]='User_ID';
	$heading1[4]='ADDRESSLINE1';
	$heading1[5]='ADDRESSLINE2';
	$heading1[6]='ADDRESSCITY';
	$heading1[7]='ADDRESSSTATE';
	$heading1[8]='ADDRESSPOSTALCODE';
	$heading1[9]='ExpenseAccount';
	$heading1[10]='ExpenseAmount';
	$heading1[11]='ExpenseDescription';
	$heading1[12]='Msg';
	$reportoutputs = $ilance->admincp->construct_csv_data_quickbooks($result, $heading1);
	$timeStamps = date("Y-m-d-H-i-s");
	$fileNames = "Consignors_negative-$timeStamp";
	$actions = 'csv';
		if ($action == 'csv')
		{
			header("Pragma: cache");
	...
	$ilance->template->fetch('main', 'consignors_quickbook_postive.html', 2);
