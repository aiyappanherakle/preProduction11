...
else if (isset($ilance->GPC['cmd']) AND $ilance->GPC['cmd'] == '_do-invoice-payment' AND isset($ilance->GPC['id']) AND $ilance->GPC['id'] > 0 AND isset($ilance->GPC['amount']) AND isset($ilance->GPC['account_id']) AND isset($ilance->GPC['invoicetype']))
{
	...
	if ($success == false)
	{
		...
		print_notice($phrase['_invoice_error'], $phrase['_were_sorry_your_transaction_has_encountered_an_error_and_can_not_be_processed_at_the_moment'], HTTPS_SERVER . $ilpage['accounting'], $phrase['_my_account']);
		exit();
	}
...
else
{
	...
	else
	{
		...
		print_notice($phrase['_invoice_error'], $phrase['_were_sorry_this_invoice_does_not_exist'], HTTPS_SERVER . $ilpage['accounting'], $phrase['_my_account']);
		exit();
	}
	...
	if (!empty($txn))
	{
		$sql_invoice = $ilance->db->query("
			...
			WHERE transactionid = '" . $ilance->db->escape_string($txn) . "'
				AND (user_id = '" . $user_id . "' OR p2b_user_id = '" . $user_id . "')
		");
	...
	if ($ilance->db->num_rows($sql_invoice) > 0)
	{
		...
		{
			if ($res_invoice['p2b_user_id'] == $user_id)
			{
				$show['paymentpulldown'] = 0;
				$cmd = '_do-invoice-action';
			}
			else if ($res_invoice['user_id'] == $user_id)
			{
		...
		$payment_method_pulldown = print_paymethod_pulldown('invoicepayment', 'account_id', $user_id);
