...
$sql=$ilance->db->query("insert into ". DB_PREFIX ."partial_payment (invoiceid,paid,amount,totalamount,paymentdate,paymethod,partial_amount) values (
	...
	{
		$ilance->GPC['picked_up'] = 0;
		combine_invoice_payment($ilance->GPC['invoice_id'],$ilance->GPC['account_id'],$ilance->GPC['picked_up'],$ilance->GPC['picked_up_date']);
...
$sql=$ilance->db->query("select user_id,combine_project,miscamount,taxamount,taxinfo,amount,paid,totalamount,status,DATE(paiddate) as paid_date,paymethod from ".DB_PREFIX."invoices where invoiceid='".$invoice_id."' and user_id ='".$invoiceuser_id."'");
while($line=$ilance->db->fetch_array($sql))
{
	if($line['paid_date'] == '0000-00-00')
	{
		$paiddate = '';
		$paid_method = '';
		$show['paid_payment_rows'] = false;
	}
	else
	{
		$paiddate = $line['paid_date'];
		$paid_method = ucfirst(strtolower($line['paymethod']));
		$show['paid_payment_rows'] = true;
	}
...
if($line['paid']>0)
{
	...
	if($ilance->db->num_rows($result))
	{
		...
		$show['paid_payment_rows']=false;
		while($line=$ilance->db->fetch_assoc($result))
		{
...
$new_ship_project = implode(",",$project_id_ship);
	if ($new_ship_project != '')
	{
		echo "SELECT track_no,shipment_date,shipper_id FROM ".DB_PREFIX."shippnig_details WHERE coin_id IN (".$new_ship_project.") AND buyer_id = '".$buyer_id."'GROUP BY track_no ORDER BY track_no DESC";

		$ship_sel = $ilance->db->query("SELECT track_no,shipment_date,shipper_id FROM ".DB_PREFIX."shippnig_details WHERE coin_id IN (".$new_ship_project.") AND buyer_id = '".$buyer_id."'GROUP BY track_no ORDER BY track_no DESC");
		if($ilance->db->num_rows($ship_sel) > 0 && $ilance->db->num_rows($ship_sel) == 1)
		{
			...
			$shipper_id = $shipingarr['shipper_id'];
		...
		else if ($ilance->db->num_rows($ship_sel) > 1)
		{
			while($shipingarr = $ilance->db->fetch_array($ship_sel))
			{
				...
				//$shipper_id1[] = $shipingarr['shipper_id'];
			}
 ...
 if(isset($ilance->GPC['cmd']) AND $ilance->GPC['cmd'] == 'buyer' AND isset($ilance->GPC['subcmd']) AND $ilance->GPC['subcmd'] == 'search')
	{
		...
		if($ilance->GPC['searchby']=='totalamount' ){
			...
			//$sql2="AND i.".$ilance->GPC['searchby']." = '".$ilance->db->escape_string($ilance->GPC['searchkey'])."'";

			if (is_int($ilance->GPC['searchkey'])) 
			{
				$sql2="AND i.".$ilance->GPC['searchby']." = '".$ilance->db->escape_string($ilance->GPC['searchkey'])."'";
			}
			else
			{
				$totl_amnt = round($ilance->GPC['searchkey'], 2);
				$sql2="AND FORMAT(i.totalamount,2) = FORMAT(".$totl_amnt.",2)";
			}
				 
			 $show['all_invoices']=true;
		}elseif($ilance->GPC['searchby']=='user_id' ){
			$sql2="AND u.".$ilance->GPC['searchby']." = '".$ilance->db->escape_string($ilance->GPC['searchkey'])."'";
			 $show['all_invoices']=true;
		}
		else{
			if(!empty($ilance->GPC['searchkey']))
			$sql2="AND ".$ilance->GPC['searchby']." = '".$ilance->db->escape_string($ilance->GPC['searchkey'])."'";
			$show['all_invoices']=true;
		}
		...
		if($ilance->db->num_rows($sql) > 0)
		{
		   ...
		   {	
			...
			if($res['status'] != 'Paid')
			 ...
			 $res['user_name'] = '<a href="'.$ilpage['users'].'?subcmd=_update-customer&amp;id='.$res['user_id'].'"">'.$res['username'].'</a>';
			 ...
			 $res['paymethod']=ucfirst(strtolower($res['paymethod']));
				...
				/*if(!isset($res['ship_status']))
				{
				var_dump($res['invoice_item_count']);
				var_dump($res['shipped_items']);
				exit;
				
				}
				$res['ship_status'].='|'.$res['invoice_item_count'].'|'.$res['shipped_items'];*/
			 $invoicelist[] = $res;
...
$second_level_number1=(int)$ilance->db->num_rows($second_level2);
		...
		$sqlquery=$ilance->db->query("select i.invoiceid,i.totalamount,i.user_id,u.username,u.first_name,u.last_name,i.paymethod from ".DB_PREFIX."invoices i
		left join ".DB_PREFIX."users u on u.user_id=i.user_id
		where i.status = 'scheduled' GROUP BY invoiceid order by i.invoiceid desc LIMIT ". (($ilance->GPC['series_page'] - 1) * 10) . "," . 10 ) ;
			if($ilance->db->num_rows($sqlquery) > 0)
		{		   
		   while($res1 = $ilance->db->fetch_array($sqlquery))
		   {	 
			 $res1['user_name'] = '<a href="'.$ilpage['users'].'?subcmd=_update-customer&amp;id='.$res1['user_id'].'"">'.$res1['username'].'</a>';
