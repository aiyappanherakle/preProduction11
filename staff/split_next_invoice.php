...
if (((!empty($ilance->GPC['id']) AND $ilance->GPC['id'] > 0) OR (!empty($ilance->GPC['userid']) AND $ilance->GPC['userid'] > 0) ) AND $_SESSION['ilancedata']['user']['isadmin'] == '1')
{
	...
	if(isset($ilance->GPC['cmd']) AND $ilance->GPC['cmd'] == 'did_invoice')
	{
		...
		{
					print_action_success('<b>Invoice Payment:</b> &nbsp;<p>Thank you for submitting your payment. Once your payment has been verified, we will mark your invoice as paid and advise you of shipping information.  Thank you for your business.',"split_invoices.php");
					exit();
		}
		else if(isset($ilance->GPC['subcmd']) AND $ilance->GPC['subcmd'] == 'cancel')	
		{
					print_action_failed('<b>Invoice Payment:</b> &nbsp;Your invoice remains unpaid.  To pay for your pending invoice at a later time, please return to My GC and click on Buyer Invoices.  Thank you for using GreatCollections',"split_invoices.php");
					exit();
		}
	}
	...
	else if ($ilance->GPC['account_id'] == 'paypal')
	{
		...
		$directpaymentform = $ilance->paypal->print_payment_form_staff_split_invoice($ilance->GPC['uid'], '', $totalamount, intval($ilance->GPC['id']), 0, $description, $ilconfig['paypal_business_email'], $ilconfig['paypal_master_currency'], '', $customencrypted, 0);
	...
	else if ($ilance->GPC['account_id'] == 'stormpay')
	{
		...
		$directpaymentform = $ilance->stormpay->print_payment_form_staff_split_invoice($ilance->GPC['uid'], '', $totalamount, intval($ilance->GPC['id']), 0, $description, $ilconfig['stormpay_business_email'], $ilconfig['stormpay_master_currency'], $ilconfig['stormpay_secret_code'], $customencrypted, 0);
	...
	else if ($ilance->GPC['account_id'] == 'cashu')
	{
		...
		$directpaymentform = $ilance->cashu->print_payment_form_staff_split_invoice($ilance->GPC['uid'], $_SESSION['ilancedata']['user']['email'], $totalamount, intval($ilance->GPC['id']), 0, $description, $ilconfig['cashu_business_email'], $ilconfig['cashu_master_currency'], $ilconfig['cashu_secret_code'], $customencrypted, $ilconfig['cashu_testmode']);
...
if($ilance->db->num_rows($user_invoice_check_result)==0)
{
  ...
  // [cmd] => _do-invoice-payment-checkorder
    ...
    [agreecheck] => on*/
		...
		print_action_success('<b>Thank you for your payment notification</b><br/><br/>Thank you for paying by check.<br><br>The total of your invoice is: $'.$res_totamount['totalamount'].' (including insured shipping).<br><br>Please make the check payable to GreatCollections and mail to us at:<br><br>GreatCollections,<br>17500 Red Hill Avenue, Suite 160,<br>Irvine CA 92614.<br><br>We appreciate your business.', "split_invoices.php");
...
if($ilance->db->num_rows($user_invoice_check_result)==0)
{
		print_action_failed('User dont have one such invoice<br/>Please login again', "split_invoices.php");
  ...
  // [cmd] => _do-invoice-payment-checkorder
    ...
    [agreecheck] => on*/
		    ...
		    ");
				...
				print_action_success('<b>Invoice Marked as Payment Pending:</b> You have successfully confirmed that your invoice will be paid by bank wire.  If you need our wire instructions, please contact us.  Once we receive your wire, we will mark your invoice as paid and ship the items to you via your preferred method.  Thank you for your business.', "split_invoices.php");
		        exit(); 

}	

if (isset($ilance->GPC['cmd']) AND $ilance->GPC['cmd'] == '_do-invoice-payment-trade')
{
...
if($ilance->db->num_rows($user_invoice_check_result)==0)
{
		print_action_failed('User dont have one such invoice<br/>Please try again', "split_invoices.php");

}

  // [cmd] => _do-invoice-payment-trade
    /*[userid] => 34
    [display_text] => Escrow Payment Forward
    [currency] => 
    [amount] => 66.00
    ...
    [agreecheck] => on*/
		...
		print_action_success('<b>Invoice Marked as Payment Pending:</b> You have confirmed that invoice will be paid by a Trade Against Consignor Proceeds.<br><br>The total of your invoice is: $'.$res_totamount['totalamount'], "split_invoices.php");
		exit(); 
...
document.getElementById("totalamt_area").innerHTML="US$"+totalhidden_text;
		...
		{
			print_action_failed('There are no unpaid invoices in this user account.', "split_invoices.php");
		}
	}
