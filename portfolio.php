...
$headinclude .= "\t<script language=\"javascript\" type=\"text/javascript\" src=\"" . $ilconfig['template_relativeimagepath'] . "functions/javascript/functions_trail.js\"></script>\n";
/*$headinclude .= "
...
{
	if ($ilance->subscription->check_access($_SESSION['ilancedata']['user']['userid'], 'addportfolio') == 'no')
	{
                $area_title = $phrase['_access_denied_to_portfolio_resources'];
		$page_title = SITE_NAME . ' - ' . $phrase['_access_denied_to_portfolio_resources'];
		
		print_notice($phrase['_access_denied'], $phrase['_your_current_subscription_level_does_not_permit_you_to_use_this_marketplace_resource']." <a href='".$ilpage['subscription']."'><strong>".$phrase['_click_here']."</strong></a>", $ilpage['subscription'], ucwords($phrase['_click_here']), fetch_permission_name('addportfolio'));
		exit();
        }
...
else if (isset($ilance->GPC['cmd']) AND $ilance->GPC['cmd'] == '_feature-portfolio' AND isset($ilance->GPC['id']) AND $ilance->GPC['id'] > 0)
{
	...
	if ($ilconfig['portfolioupsell_featuredactive'] == false)
        {
		$area_title = $phrase['_invalid_portfolio_menu'];
		$page_title = SITE_NAME . ' - ' . $phrase['_invalid_portfolio_menu'];
		
		print_notice($phrase['_access_denied'], $phrase['_were_sorry_this_feature_is_currently_disabled'], 'javascript:history.back(1);', $phrase['_back']);
		exit();
	}
        
        $area_title = $phrase['_featured_portfolio_order_menu'];
        $page_title = SITE_NAME . ' - ' . $phrase['_featured_portfolio_order_menu'];
        ...
        { 
                ...
                $payment_method_pulldown = print_paymethod_pulldown('portfolio', 'account_id', $_SESSION['ilancedata']['user']['userid'], $javascript = '');
			...
			{
				$amount_formatted = $phrase['_free'];
				$total_formatted = $phrase['_free'];
				$amount = 0;
			}
                }
                
		$pprint_array = array('total_formatted','tax_formatted','cid','category','duration','amount_formatted','payment_method_pulldown','item_name','id','portfolio_id','amount','qty','topay','input_style','remote_addr','rid','login_include','headinclude','onload','area_title','page_title','site_name','https_server','http_server','lanceads_header','lanceads_footer');
        ...
        else
        {
                ...
                print_notice($phrase['_invalid_portfolio_catalog'], $phrase['_were_sorry_the_portfolio_catalog_for_this_customer_has_been_removed_or_is_currently_not_prepared_for_viewing']." ".$phrase['_please']." <span class=\"blue\"><a href='".$ilpage['portfolio']."'>".$phrase['_view_other_portfolios']."</a></span> ".$phrase['_here_a_few_reasons_you_might_be_seeing_this_page'].":<br /><br /><li>".$phrase['_portfolio_has_not_been_setup_or_created']."</li><li>".$phrase['_this_vendors_subscription_may_be_inactive']."</li><li>".$phrase['_this_vendor_may_be_suspended_or_removed_from_the_marketplace']."</li>", 'javascript:history.back(1);', $phrase['_back']);
                exit();
        }
}
...
else if (isset($ilance->GPC['cmd']) AND $ilance->GPC['cmd'] == '_portfolio-purchase-preview' AND isset($ilance->GPC['portfolio_id']) AND $ilance->GPC['portfolio_id'] > 0 AND isset($ilance->GPC['qty']) AND $ilance->GPC['qty'] > 0)
{
	...
	$area_title = $phrase['_featured_portfolio_preview_confirmation_menu'];
	$page_title = SITE_NAME . ' - ' . $phrase['_featured_portfolio_preview_confirmation_menu'];
	...
	if ($ilance->db->num_rows($sql) > 0)
	{
		...
		$pprint_array = array('total','total_formatted','tax_formatted','duration','category','amount_formatted','payment_method_pulldown','item_name','id','portfolio_id','amount','qty','topay','input_style','remote_addr','rid','login_include','headinclude','onload','area_title','page_title','site_name','https_server','http_server','lanceads_header','lanceads_footer');
...
else if (isset($ilance->GPC['cmd']) AND $ilance->GPC['cmd'] == '_portfolio-purchase-process' AND isset($ilance->GPC['portfolio_id']) AND $ilance->GPC['portfolio_id'] > 0  AND isset($ilance->GPC['qty']) AND $ilance->GPC['qty'] > 0  AND isset($ilance->GPC['amount']) AND isset($ilance->GPC['total']))
{
	...
	if ($ilance->db->num_rows($sql) > 0)
	{
		$area_title = $phrase['_featured_portfolio_payment_process'];
		$page_title = SITE_NAME . ' - ' . $phrase['_featured_portfolio_payment_process'];
		...
		if ($successful)
		{
			...
			print_notice($phrase['_invoice_payment_complete'], $phrase['_your_invoice_has_been_paid_in_full'], $ilpage['portfolio'] . '?cmd=management', $phrase['_portfolio_manager']);
			exit();
		}
		else
		{
			...
			print_notice($phrase['_invoice_payment_warning_insufficient_funds'], $phrase['_were_sorry_this_invoice_can_not_be_paid_due_to_insufficient_funds'], $ilpage['accounting'] . '?cmd=deposit', $phrase['_deposit_funds_menu']);
			exit();
		}
	}
	else
...
else if (isset($ilance->GPC['id']) AND !empty($ilance->GPC['id']))
{
	...
	$area_title = $phrase['_viewing_portfolio_for'] . ' ' . fetch_user('username', intval($id));
	$page_title = SITE_NAME . ' - ' . $phrase['_viewing_portfolio_for'] . ' ' . fetch_user('username', intval($id));
	...
	// displayable image types that will appear as images in the portfolio
	...
	if ($ilance->db->num_rows($result) > 0)
	{
		...
		while ($row = $ilance->db->fetch_array($result))
		{
			...
			$countryname = print_user_country($row['user_id']);
			...
			if ($displayable == 1)
			{
				if (isset($not_zoomed) AND $not_zoomed)
				{
					...
					$thumbnail_photo = '<a href="' . $ilpage['portfolio'] . '?cmd=zoom&amp;item=' . $row['attachid'] . '&amp;id=' . $row['user_id'] . '" title="' . $row['filename'] . ': ' . $row['filesize'] . ' ' . $phrase['_bytes'] . '"><img src="' . $ilpage['attachment'] . '?cmd=thumb&id=' . $row['filehash'] . '&subcmd=portfolio' . '" alt="' . $row['filename'] . ': ' . $row['filesize'] . ' ' . $phrase['_bytes'] . '" border="0" /></a>';
				}
				else if (isset($zoomed) AND $zoomed)
				{
					$thumbnail_photo = '<a href="' . $ilpage['portfolio'] . '?id=' . $id . '" title="' . $row['filename'] . ': ' . $row['filesize'] . ' ' . $phrase['_bytes'] . '"><img src="' . $ilpage['attachment'] . '?cmd=portfolio&id=' . $row['filehash'] . '" alt="' . $row['filename'] . ': ' . $row['filesize'] . ' ' . $phrase['_bytes'] . '" border="0" /></a>';
				}
			}
			else
			{
				...
				if (isset($not_zoomed) AND $not_zoomed)
				{
					$thumbnail_photo = '<img src="' . $ilconfig['template_relativeimagepath'] . $ilconfig['template_imagesfolder'] . 'icons/'.$attachextension.'" alt="'.$row['filename'].': '.$row['filesize'].' '.$phrase['_bytes'].'" /> <a href="' . $ilpage['portfolio'] . '?cmd=zoom&item=' . $row['attachid'] . '&id=' . $row['user_id'].'" title="'.$row['filename'].': '.$row['filesize'].' '.$phrase['_bytes'].'" border="0">'.$row['filename'].'</a>';
				}
				else if (isset($zoomed) AND $zoomed)
				{
					$thumbnail_photo = '<a href="' . $ilpage['portfolio'] . '?id=' . $id . '" title="'.$row['filename'].': '.$row['filesize'].' '.$phrase['_bytes'].'"><img src="' . $ilconfig['template_relativeimagepath'] . $ilconfig['template_imagesfolder'] . 'icons/'.$attachextension.'" alt="'.$row['filename'].': '.$row['filesize'].' '.$phrase['_bytes'].'" border="0" /></a> '.$row['filename'].'<div><strong><a href="'.$ilpage['attachment'].'?id='.$row['filehash'].'">'.$phrase['_download'].'</a></strong></div>';
				}
			}
			...
			{
				$area_title = $phrase['_detailed_portfolio_for'] . ' ' . fetch_user('username', intval($id)) . ' ' . $phrase['_in'] . ' ' . $ilance->categories->title($_SESSION['ilancedata']['user']['slng'], 'service', $row['category_id']) . ' | ' . $thumbnail_caption . ' (' . $row['filename'] . ')';
				$page_title = SITE_NAME . ' - ' . $phrase['_detailed_portfolio_for'] . ' ' . fetch_user('username', intval($id)) . ' ' . $phrase['_in'] . ' ' . $ilance->categories->title($_SESSION['ilancedata']['user']['slng'], 'service', $row['category_id']) . ' | ' . $thumbnail_caption . ' (' . $row['filename'] . ')';
				
				$metakeywords = fetch_user('username', intval($id)) . ', ' . $thumbnail_caption . ', ' . $ilance->categories->title($_SESSION['ilancedata']['user']['slng'], 'service', $row['category_id']) . ', ' . $phrase['_portfolio'];
				$metadescription = stripslashes($row['description']);
				
				$navcrumb = array();
				if ($ilconfig['globalauctionsettings_seourls'])
				{
					$navcrumb[HTTP_SERVER . "portfolios"] = $phrase['_portfolios'];
				}
				else
				{
					$navcrumb["$ilpage[portfolio]"] = $phrase['_portfolios'];
				}
				$navcrumb[construct_seo_url('portfoliocatplain', $row['category_id'], 0, $ilance->categories->title($_SESSION['ilancedata']['user']['slng'], 'service', $row['category_id']), '', 0, '', 0, 0)] = $ilance->categories->title($_SESSION['ilancedata']['user']['slng'], 'service', $row['category_id']);
			...
			// check for selling profile logo
			...
			$thumbnail_category = $ilance->categories->title($_SESSION['ilancedata']['user']['slng'], 'service', $row['category_id']);
			$thumbnail_description = (!empty($row['description'])) ? shorten(stripslashes($row['description']), 35) : $phrase['_none'];
			...
			if (isset($not_zoomed) AND $not_zoomed)
			{
				...
				else
				{
					...
					<td valign="bottom" width="33%">
                                                ...
                                                <tr class="alt3">
                                                    <td colspan="2"><strong>' . $thumbnail_caption . '</strong></td>
                                                </tr>
                                                <tr class="alt1">
                                                    <td><div class="gray">'.$phrase['_views'].'</div></td>
                                                    <td width="75%"><div class="blue">' . $thumbnail_views . '</div></td>
                                                </tr>
                                                <tr class="alt1">
                                                    <td><div class="gray">'.$phrase['_size'].'</div></td>
                                                    <td width="75%"><div class="blue">' . $thumbnail_size . '</div></td>
                                                </tr>
                                                <tr class="alt1">
                                                    <td><div class="gray">'.$phrase['_category'].'</div></td>
                                                    <td width="75%"><div class="blue">' . $thumbnail_category . '</div></td>
                                                </tr>
                                                </table>
					</td>';
				}
				...
				$portfolio_html .= '
                                <div class="block-wrapper">
                                        <div class="block3">
                                                        ...
                                                        <div class="block3-header">' . $thumbnail_caption . '</div>
                                                        <div class="block3-content-gray" style="padding:9px"><div class="smaller">' . $thumbnail_description . '</div></div>
                                                        ...
                                                        <div>
                                                                ...
                                                                <tr class="alt1">
                                                                    <td colspan="2"><div align="center">' . $thumbnail_photo . '</div></td>
                                                                </tr>
								<tr class="alt1">
                                                                    <td><div class="gray">' . $phrase['_filename'] . '</div></td>
                                                                    <td width="75%"><div class="blue">' . $row['filename'] . '</div></td>
                                                                </tr>
                                                                <tr class="alt1">
                                                                    <td><div class="gray">' . $phrase['_size'] . '</div></td>
                                                                    <td width="75%"><div class="blue">' . $thumbnail_size . '</div></td>
								...
								<tr class="alt1">
                                                                    <td><div class="gray">' . $phrase['_traffic'] . '</div></td>
                                                                    <td width="75%"><div class="blue">' . $thumbnail_views . ' ' . $phrase['_views_lower'] . '</div></td>
                                                                </tr>
                                                                <tr>
                                                                    <td><div class="gray">' . $phrase['_category'] . '</div></td>
                                                                    <td width="75%"><div class="blue">' . $thumbnail_category . '</div></td>
	...
	$prevnext = print_pagnation($number, $ilconfig['portfoliodisplay_thumbsperpage'], intval($ilance->GPC['page']), $counter, $ilpage['portfolio'] . '?id=' . $id . $catbit);
        ...
        $pprint_array = array('portfolio_html2','profile_logo','js','feed1','feed6','feed12','vendorstars','memberstart','countryname','profile_logo','id','provider','prevnext','portfolio_html','input_style','remote_addr','rid','login_include','headinclude','onload','area_title','page_title','site_name','https_server','http_server','lanceads_header','lanceads_footer');
...
else if (isset($ilance->GPC['cmd']) AND $ilance->GPC['cmd'] == 'management' OR isset($ilance->GPC['cmd']) AND $ilance->GPC['cmd'] == 'create-portfolio')
{
	...
	if (!empty($_SESSION['ilancedata']['user']['userid']) AND $_SESSION['ilancedata']['user']['userid'] > 0)
	{
		...
		if ($ilance->subscription->check_access($_SESSION['ilancedata']['user']['userid'], 'addportfolio') == 'yes')
		{
...
<!--
var urlBase = 'ajax.php?do=inlineedit&action=portfolio&id=';
//-->
</script>";
			...
			if ($ilance->db->num_rows($res) > 0)
			{
				while ($row = $ilance->db->fetch_array($res, DB_ASSOC))
				{
					...
					$row['attach_caption'] = '<div id="portfolio_attachid_' . $row['portfolio_id'] . '_title" onmouseover="return toggle_show(\'' . $row['portfolio_id'] . '_title_edit\')" onmouseout="return toggle_hide(\'' . $row['portfolio_id'] . '_title_edit\')"><strong><span id="phrase' . $row['portfolio_id'] . '_titleinline"><span ondblclick="do_inline_edit(\'' . $row['portfolio_id'] . '_title\', this);">' . $caption . '</span></span></strong><span style="display:none" id="' . $row['portfolio_id'] . '_title_edit" class="smaller litegray">&nbsp;&nbsp;&nbsp;(' . $phrase['_doubleclick_to_edit'] . ')</span></div>';
					$description = stripslashes($ilance->db->fetch_field(DB_PREFIX . "portfolio", "portfolio_id = '" . $row['portfolio_id'] . "'", "description"));
					if (empty($description))
					{
						$description = $phrase['_no_description_available'];
					}
					$row['attach_description'] = '<div id="portfolio_attachid_' . $row['portfolio_id'] . '_description" class="gray" onmouseover="return toggle_show(\'' . $row['portfolio_id'] . '_description_edit\')" onmouseout="return toggle_hide(\'' . $row['portfolio_id'] . '_description_edit\')"><span id="phrase' . $row['portfolio_id'] . '_descriptioninline"><span ondblclick="do_inline_edit(\'' . $row['portfolio_id'] . '_description\', this);">' . $description . '</span></span><span style="display:none" id="' . $row['portfolio_id'] . '_description_edit" class="smaller litegray">&nbsp;&nbsp;&nbsp;(' . $phrase['_doubleclick_to_edit'] . ')</span></div>';
					...
					if ($ilance->db->fetch_field(DB_PREFIX."portfolio", "portfolio_id = '" . $row['portfolio_id'] . "'", "featured"))
					{
						...
						if ($days < 0)
						{
							...
							$ilance->db->query("
								...
								LIMIT 1
							");
							$row['isfeatured'] = '<input type="button" value=" ' . $phrase['_feature'] . ' " onclick="location.href=\'' . $ilpage['portfolio'] . '?cmd=_feature-portfolio&amp;id=' . $row['portfolio_id'] . '\'" class="buttons_smaller" />';
						}
						$row['isfeatured'] = '<input type="button" value=" ' . $days . ' ' . $phrase['_days_left'] . ' " onclick="location.href=\'\'" class="buttons_smaller" disabled="disabled" />';
						$row['class'] = 'featured_highlight';
					}
					elseif ($ilconfig['portfolioupsell_featuredactive'])
					{
						$row['isfeatured'] = '<input type="button" value=" ' . $phrase['_feature'] . ' " onclick="location.href=\'' . $ilpage['portfolio'] . '?cmd=_feature-portfolio&amp;id=' . $row['portfolio_id'] . '\'" class="buttons_smaller" />';
						$row['class'] = ($row_count % 2) ? 'alt2' : 'alt1';
					}
					...
					else
					{
						...
						$row['thumbnail'] = '<span class="smaller gray">' . $phrase['_nonimage_format'] . '</span>';
					}
			...
			if ($ilance->db->num_rows($result2) > 0)
			{
				...
				while ($row_ = $ilance->db->fetch_array($result2))
				{
					...
					$row_['status'] = $phrase['_review_in_progress'];
			...
			$uploadbutton = '<a href="javascript:void(0)" onclick=Attach("' . $ilpage['upload'] . '?crypted=' . encrypt_url($hiddeninput) . '&amp;refresh=1")>' . $phrase['_upload_media'] . '</a>';
			$hiddeninput = encrypt_url($hiddeninput);
			
			$pprint_array = array('hiddeninput','numcolumns','uploadbutton','project_id','category_id','filehash','user_id','attachtype','max_filesize','attach_user_max','attach_usage_total','prevnext','prevnext2','input_style','remote_addr','rid','login_include','headinclude','onload','area_title','page_title','site_name','https_server','http_server','lanceads_header','lanceads_footer');
...
else
{
	...
	$metatitle = $phrase['_portfolios'];
	
	// #### setup some page titles and meta tags ###########################
	if ($cid > 0)
	{
		$metatitle = $ilance->categories->title($_SESSION['ilancedata']['user']['slng'], 'service', $cid) . ' | ' . $phrase['_portfolios'];
		$metadescription = $ilance->categories->description($_SESSION['ilancedata']['user']['slng'], 'service', $cid) . ' ' . $phrase['_portfolios'];
	...
	if ($ilance->db->num_rows($resultfeatured) > 0)
	{
		...
		while ($rowfeatured = $ilance->db->fetch_array($resultfeatured, DB_ASSOC))
		{
			...
			if (empty($rowfeatured['caption']))
			{
				...
				{
					// $thumbnail_caption .= '<img src="' . $ilconfig['template_relativeimagepath'] . $ilconfig['template_imagesfolder'] . 'showcase.gif" alt="'.$phrase['_featured'].'" border="0" />';
				}
			}
			else
			{
			...
			$thumbnail_provider = print_username($rowfeatured['user_id'], 'href', 1);
                        $thumbnail_invite = '<a href="' . HTTP_SERVER . $ilpage['rfp'] . '?cmd=rfp-invitation&amp;id=' . $rowfeatured['user_id'] . '&amp;trk=f_gallery" rel="nofollow">' . $phrase['_invite_to_bid'] . '</a>';
			$thumbnail_views = $rowfeatured['counter'];
			$thumbnail_size = print_filesize($rowfeatured['filesize']);
			$thumbnail_category = $ilance->categories->title($_SESSION['ilancedata']['user']['slng'], 'service', $rowfeatured['category_id']);
			$thumbnail_description = $phrase['_no_description'];
			...
			else
			{
				...
				$portfolio_featured_html .= '
				<td valign="bottom" width="33%" class="alt' . $tdclass . '">
                                        ...
                                        <tr class="alt1">
                                                <td><span class="gray">' . $phrase['_provider'] . '</span></td>
                                ...
                                if ($ilconfig['enableskills'])
                                {
                                        $portfolio_featured_html .= '
                                        <tr class="alt1">
                                                <td valign="top"><span class="gray">' . $phrase['_skills'] . '</span></td>
                                                <td width="95%" valign="top"><div class="blue">' . print_skills($rowfeatured['user_id'], $showmaxskills = 3, $nourls = true) . '</div></td>
                                        </tr>';
                                }
                                ...
                                $portfolio_featured_html .= '
                                        <tr>
                                                ...
                                                <td width="75%"><span class="smaller" style="float:right"><span class="gray"><a href="' . $ilpage['abuse'] . '?cmd=portfolio&amp;id=' . $rowfeatured['attachid'] . '" rel="nofollow">' . $phrase['_report_abuse'] . '</a></span></span><span class="blue">' . $thumbnail_category . '</span></td>
                                        </tr>
                                        </table>
				</td>';
			}
	...
	if ($ilance->db->num_rows($result) > 0)
	{
		...
		while ($row = $ilance->db->fetch_array($result, DB_ASSOC))
		{
			...
			$thumbnail_provider = print_username($row['user_id'], 'href', 1);
			$thumbnail_invite = '<a href="' . HTTP_SERVER . $ilpage['rfp'] . '?cmd=rfp-invitation&amp;id=' . $row['user_id'] . '">' . $phrase['_invite_to_bid'] . '</a>';
			...
			if ($num == $numcolumns)
			{
				$portfolio_html .= '<tr class="alt1"><td colspan="' . $numcolumns . '"></td></tr>';
			}
			else
			{
				if ($num == $numcolumns)
				...
				<td valign="bottom" width="33%" class="alt' . $tdclass . '">
                                        ...
                                        <div align="center" style="margin-top:5px;margin-bottom:14px">' . $thumbnail_photo . '</div>
                                        
                                        <table style="float:right" border="0" cellpadding="' . $ilconfig['table_cellpadding'] . '" cellspacing="' . $ilconfig['table_cellspacing'] . '" width="100%" dir="' . $ilconfig['template_textdirection'] . '">
                                        <tr>
						<td class="alt2" colspan="2"><strong>' . $thumbnail_caption . '</strong> <span class="smaller gray">(' . $thumbnail_size . ')</span></td>
                                        </tr>
                                        <tr class="alt1">
						<td><span class="gray">' . $phrase['_provider'] . '</span></td>
						<td width="75%"><span class="blue">' . $thumbnail_provider . '</span></td>
                                        </tr>
                                        <tr>
						<td><span class="gray">' . $phrase['_category'] . '</span></td>
						<td width="75%"><span class="smaller" style="float:right"><span class="gray"><a href="' . $ilpage['abuse'] . '?cmd=portfolio&amp;id=' . $row['attachid'] . '" rel="nofollow">' . $phrase['_report_abuse'] . '</a></span></span><span class="blue">' . $thumbnail_category . '</span></td>
                                        </tr>
                                        </table>
			...
			if ($num == $numcolumns)
			{
				$portfolio_html .= '</tr><tr class="alt1">'; 
				$num = 0;
			}
...
if (!empty($ilance->GPC['cid']) AND $ilance->GPC['cid'] > 0)
{
	...
	if ($ilconfig['globalauctionsettings_seourls'])
	{
		...
		$navcrumb[HTTP_SERVER . "$url"] = $phrase['_portfolios'];
		unset($url);
	}
	else
	{
		$navcrumb[HTTP_SERVER . "$ilpage[portfolio]"] = $phrase['_portfolios'];
	}	
...
$portfoliocategory = $ilance->template->print_left_nav('portfolio', $cid, 1, 0, $ilconfig['globalfilters_enablecategorycount']);

// #### recently featured portfolio entries ####################################
$recentlyfeatured = $ilance->portfolio->print_recently_featured_users($cid, 30);
...
$pprint_array = array('portfoliocalendar','recentlyfeatured','portfoliocats','portfolioitems','portfolioviews','portfoliodiskspace','portfoliocategory','js','prevnext','prevnext2','portfolio_featured_html','portfolio_html','input_style','remote_addr','rid','login_include','headinclude','onload','area_title','page_title','site_name','https_server','http_server','lanceads_header','lanceads_footer');
