...
if (!empty($_SESSION['ilancedata']['user']['userid']) AND $_SESSION['ilancedata']['user']['userid'] > 0 AND $_SESSION['ilancedata']['user']['isstaff'] == '2' or $_SESSION['ilancedata']['user']['isadmin'] == '1' )
{
	...
	if (isset($ilance->GPC['cmd']) AND $ilance->GPC['cmd'] == 'coin_list')
    {
			...
			$ilance->template->fetch('main', 'consignments_coin_listing.html', 4);
    ...
    {        
				...
				$ilance->template->fetch('main', 'consignments_create.html', 4);
				$ilance->template->parse_hash('main', array('ilpage' => $ilpage));
...
unset($hiddeninput);
	...
	$ilance->template->fetch('main', 'consignments_add_single_coin.html', 4);
	$ilance->template->parse_hash('main', array('ilpage' => $ilpage));
	$ilance->template->parse_if_blocks('main');
	$ilance->template->pprint('main', $pprint_array);
	exit();
	...
	$ilance->template->fetch('main', 'consignments_raw_coin.html', 4);
	$ilance->template->parse_hash('main', array('ilpage' => $ilpage));
	$ilance->template->parse_if_blocks('main');
	$ilance->template->pprint('main', $pprint_array);
	exit();
	...
	$ilance->template->fetch('main', 'consignments_send_certification.html', 4);
	$ilance->template->parse_hash('main', array('ilpage' => $ilpage));
	$ilance->template->parse_if_blocks('main');
	$ilance->template->pprint('main', $pprint_array);
	exit();
	...
	$ilance->template->fetch('main', 'consignments_listing_raw.html', 4);
	$ilance->template->parse_hash('main', array('ilpage' => $ilpage));
...
$search_result['desci'] = '<span class="blue"><a href="consignments.php?cmd=add_single_coin&subcmd=update_edit&user_id='.$search_result['user_id'].'&consignid='.$search_result['consignid'].'&pro='.fetch_user_attach('project_id',$search_result['coin_id']).'&coin_id='.$search_result['coin_id'].'">'.$search_result['Title'].'</a></span>';	
	...
	$search_result['pc'] = '<span class="blue"><a href="catalog.php?cmd=update-catalog&subcmd=update-coinlist&id='.fetch_cat('id',$search_result['pcgs']).'">'.$search_result['pcgs'].'</a></span>';
		...
		$ilance->template->fetch('main', 'consignments_search.html', 4);
		$ilance->template->parse_hash('main', array('ilpage' => $ilpage));
    ...
    if (isset($ilance->GPC['cmd']) AND $ilance->GPC['cmd'] == 'consign_edit')
	{
		...
		/* 	$year = $dataex[0];
			...
			$ilance->template->fetch('main', 'consignments_edit.html', 4);
...
/*				
	...
	$ilance->template->fetch('main', 'consignments_pending_listing.html', 4);
	$ilance->template->parse_hash('main', array('ilpage' => $ilpage));
	$ilance->template->parse_loop('main', array('consignment_listing','consignment_listing_search'));
	$ilance->template->parse_if_blocks('main');
	$ilance->template->pprint('main', $pprint_array);
	...
	$ilance->template->fetch('main', 'consignments_quick_page.html', 4);
	$ilance->template->parse_hash('main', array('ilpage' => $ilpage));
	$ilance->template->parse_loop('main', array('consignment_listing','consignment_listing_search'));
	$ilance->template->parse_if_blocks('main');
	$ilance->template->pprint('main', $pprint_array);
	...
	$ilance->template->fetch('main', 'consignments_listing_seller.html', 4);
	$ilance->template->parse_hash('main', array('ilpage' => $ilpage));
	$ilance->template->parse_loop('main', array('consignment_listing','consignment_listing_search'));
	$ilance->template->parse_if_blocks('main');
	$ilance->template->pprint('main', $pprint_array);
...
{	
   ...
   break;
    ...
    $SQL="SELECT cn.consignid,cn.coins,c.coincount,u.user_id,u.first_name,u.last_name,u.username,u.email,
	 DATE_FORMAT(cn.receive_date,'%b %d %Y') as receive_date,c.consignment_cost
			FROM " . DB_PREFIX . "consignments cn 
			left join " . DB_PREFIX . "users u on u.user_id=cn.user_id
			left join (select count(coin_id) as coincount,sum(cost) as consignment_cost,consignid 
	...
	$ilpage['consignments']=HTTP_SERVER.'/staff/'.__FILE__;		
	...
	$scriptpageprevnext = $ilpage['consignments']. '?cmd=listing'.$query_string;
		...
		if($ilance->db->num_rows($result))
		{
			while($line=$ilance->db->fetch_array($result))
			{
				...
				$row_list['View'] 		= '<a href="consignments.php?cmd=coin_list&user_id='.$line['user_id'].'&consignid='.$line['consignid'].'">View</a>'; 
				$row_list['edit'] 		= '<a href="consignments.php?cmd=consign_edit&user_id='.$line['user_id'].'&consignid='.$line['consignid'].'">Edit</a>'; 
	...
	$ilance->template->fetch('main', 'consignments_listing_4975.html', 4);
	$ilance->template->parse_hash('main', array('ilpage' => $ilpage));
