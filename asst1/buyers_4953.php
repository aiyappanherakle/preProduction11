...
$pprint_array = array('payment_id','paymentdate','totalamount','paymethod','login_include','headinclude','area_title','page_title','site_name','https_server','http_server','lanceads_header','lanceads_footer','series_prevnext1');
	...
	$ilance->template->fetch('main', 'buyers_invoice_payment.html', 4);
...
$pprint_array = array('payment_id','paymentdate','partial_amount','paymethod','login_include','headinclude','area_title','page_title','site_name','https_server','http_server','lanceads_header','lanceads_footer','series_prevnext1');
	...
	$ilance->template->fetch('main', 'buyers_edit_partial_payment.html', 4);
	$ilance->template->parse_hash('main', array('ilpage' => $ilpage));
	$ilance->template->parse_if_blocks('main');
	$ilance->template->pprint('main', $pprint_array);
	exit();
...
$payment_methods=print_paymethod_pulldown('invoicepayment', 'account_id', 1);
$buyerfee=$ilance->currency->format(array_sum($buyerfees));
//echo print_active_ipn_options('invoicepayment');
...
$link2='print_invoice2.php?id='.$ilance->GPC['id'];
	...
	$ilance->template->fetch('main', 'buyers_invoice.html', 4);
	$ilance->template->parse_hash('main', array('ilpage' => $ilpage));
	$ilance->template->parse_if_blocks('main');
    ...
    if(isset($ilance->GPC['cmd']) AND $ilance->GPC['cmd'] == 'buyer' AND isset($ilance->GPC['subcmd']) AND $ilance->GPC['subcmd'] == 'search')
	{
		...
		if($ilance->GPC['searchby']=='username' or $ilance->GPC['searchby']=='email')
		{
			 ...
			 {
				 $searchkey_value = $ilance->GPC['searchkey'];
			 }
			 $sql=$ilance->db->query("select user_id from ".DB_PREFIX."users where ".$ilance->GPC['searchby']." = '".$ilance->GPC['searchkey']."' ");
			  if($ilance->db->num_rows($sql) > 0)
				{	
					 $res = $ilance->db->fetch_array($sql);
	...
	$sql2.= "order by invoiceid DESC ".$_SESSION['sort_order'];
...
$second_level_number=(int)$ilance->db->num_rows($second_level1);
	...
	$final_query1="select * from ".DB_PREFIX."invoices where combine_project ".$sql2."  LIMIT ". (($ilance->GPC['series_page'] - 1) * 10) . "," . 10;
	echo $final_query1;exit;
		$sql=$ilance->db->query($final_query1) ;
			if($ilance->db->num_rows($sql) > 0)
		{
		   ...
		   {	
			...
			if($res['status'] != 'paid')
			 ...
			 //karthik may12
			 $res['first_name'] = fetch_user('first_name', $res['user_id']);
			  $res['last_name'] = fetch_user('last_name', $res['user_id']);
			 $res['user_name'] = '<a href="'.$ilpage['users'].'?subcmd=_update-customer&amp;id='.$res['user_id'].'"">'.fetch_user('username', $res['user_id']).'</a>';
		...
		$series_prevnext = print_pagnation($second_level_number, 10, $ilance->GPC['series_page'], $counter, $scriptpageprevnext, 'series_page');
