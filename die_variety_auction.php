...
global $project_like_keyword,$key_cmd,$navval,$featuredsql,$block_header, $block_content, $block_content_yellow,$project_user_id;
if (isset($ilance->GPC['key_cmd']) AND $ilance->GPC['key_cmd'] == 'die_variety')
{
    $project_like_keyword=array('FS');
    $ilance->GPC['frombid']=1;
    //frombid=1&tobid=500
    $project_user_id=12513;
    $key_cmd = '<input type="hidden" name="key_cmd" value="die_variety" />';
    $page_title = $phrase['_cwelchuscoins_page_title'];
    $navval = 'die_variety';
    $show['page_for'] = 'young_collection';
}
else if (isset($ilance->GPC['key_cmd']) AND $ilance->GPC['key_cmd'] == 'die_variety_all')
{
    $project_user_id=12513;
    $key_cmd = '<input type="hidden" name="key_cmd" value="die_variety_all" />';
    $page_title = $phrase['_cwelchuscoins_all_page_title'];
    $navval = 'die_variety_all';
    $show['page_for'] = 'young_collection';
}
else if (isset($ilance->GPC['key_cmd']) AND $ilance->GPC['key_cmd'] == 'young_collection')
{
...
$product_category_selection = $search_category_pulldown = '';
...
if (!empty($ilance->GPC['mode']) AND $show['searcherror'] == 0)
{
        ...
        $scriptpage = HTTP_SERVER . 'die_variety_auction.php' . print_hidden_fields(true, array('do','cmd','page','budget','searchid','list'), true, '', '', $htmlentities = true, $urldecode = false);
...
//        $show['widescreen'] = true;
        ...
        switch ($ilance->GPC['mode'])
        {
            ...
            // #### keywords entered by user #######################
                       ...
                       $ilconfig['globalfilters_maxrowsdisplaysubscribers']=50;
            ...
            // #### finalize country verbose text so it's placed after the region
                                ...
                                if(!empty($ilance->GPC['grading_service']) || !empty($ilance->GPC['fromyear']) || !empty($ilance->GPC['toyear']) || !empty($ilance->GPC['grade_range_1']) || !empty($ilance->GPC['grade_range_2']) || !empty($ilance->GPC['frombid']) || !empty($ilance->GPC['tobid']) || !empty($ilance->GPC['listing_type']) || !empty($ilance->GPC['denom_all']) || !empty($ilance->GPC['denomination']) ){
                                    ...
                                    //$ilance->GPC['mintage']=$temp[1];
                                        ...
                                        if(!empty($ilance->GPC['fromyear']) && !empty($ilance->GPC['toyear'])){
                                            ...
                                            $sqlquery['year_range'] = " AND (p.coin_detail_year BETWEEN ".$ilance->GPC['fromyear1']." AND ".$ilance->GPC['toyear1']." ) ";
                                        }
                                        if(!empty($ilance->GPC['fromyear']) && empty($ilance->GPC['toyear'])){
                                        
                                            $sqlquery['year_range'] = " AND (p.coin_detail_year BETWEEN ".$ilance->GPC['fromyear1']." AND ".date("Y")." ) ";
                                        }
                                        if(empty($ilance->GPC['fromyear']) && !empty($ilance->GPC['toyear'])){
                                            
                                            $sqlquery['year_range'] = " AND (p.coin_detail_year BETWEEN ''  AND ".$ilance->GPC['toyear1'].") ";
                                        }
                ...
                else
                {
                    $sqlquery['genrequery'] = '';
                        ...
                        if(isset($project_user_id) and $project_user_id>0)
                        {
                            $sqlquery['projecttitle_like'].= " AND p.user_id=".$project_user_id." ";
                        }
                      ...
                      //  $page_title = ((isset($keyword_text) AND !empty($keyword_text)) ? $keyword_text . ', ' : '') . '' . $phrase['_find'] . ' ' . (($project_state == 'service') ? $phrase['_services'] : $phrase['_items']) . ' ' . ((!empty($metadescription)) ? $phrase['_in'] . ' ' . $metatitle . ', ' . $metadescription : '') . ' | ' . SITE_NAME;
                        ...
                        if ($ilance->db->num_rows($result) > 0)
                        {
                            ...
                            if(!empty($row['filehash']))
                            {
                                ...
                                <a href="' . $url . '"><img src="' .HTTP_SERVER. 'image.php?cmd=thumb&amp;subcmd=results&amp;id=' . $row['filehash'] . '" border="' . $borderwidth . '" alt="" style="border-color:' . $bordercolor . '" class="gallery-thumbs-image-cluster" /></a>
    ...
    : '<div class="smaller blue"> Bid </div>';
                            ...
                            if(!empty($row['filehash']))
                            {
                                $td['sample'] ='<a href="' . $url . '"><img src="' .HTTPS_SERVER. $ilpage['attachment'] . '?cmd=thumb&amp;subcmd=resultsgallery&amp;id=' . $row['filehash'] . '" border="' . $borderwidth . '" alt="" style="border-color:' . $bordercolor . '" class="gallery-thumbs-image-cluster" /></a> ';
                            }else
                            {
                            $td['sample'] = '<img src="' . $ilconfig['template_relativeimagepath'] . $ilconfig['template_imagesfolder'] . 'nophoto.gif" alt="" border="0" />';
                            }
                    ...
                    $pbit = $row['maxamount'];
                     ...
                     $featuredproductauctions = ($show['mode_product']) ?fetch_featured_auctions_new('product', 4, 1, $cid, $keyword_text, false, $excludelist,$series,$featuredsql) : '';
                        ...
                        unset($budgettemp);
                                ...
                                $v3left_nav = $ilance->template->print_left_nav('product', $cid, $dosubcats = 1, $displayboth = 0, $ilconfig['globalfilters_enablecategorycount'], true,0,'die_variety');
            ...
            $hiddenfields = print_hidden_fields(false, array('searchid','sef','cid','buynow','sort','images','freeshipping','listedaslots','budget','publicboard','escrow','underage','endstart','endstart_filter','q','page'), $questionmarkfirst = false, $prepend_text = '', $append_text = '', $htmlentities = true, $urldecode = true);
                ...
                $metadescription="Searching 1000s of ".$keywords." certified coins at GreatCollections Coin Auctions & Rare Coin Sales";
                            ...
                            $toyear= !empty($ilance->GPC['toyear']) ? $ilance->GPC['toyear'] : '2015';
                            
                            $frombid= !empty($ilance->GPC['frombid'] ) ? $ilance->GPC['frombid'] :'0';
                            $tobid= !empty($ilance->GPC['tobid']) ? $ilance->GPC['tobid'] :'';
                            ...
                            //nav crumb
                            if(!empty($navval) && $navval == 'die_variety')
                            {
                                $navcrumb = array("young-collection"=>$phrase['_cwelchus_coins']);
                                $block_header = $phrase['_cwelchuscoins_block_header'];
                                $block_content = $phrase['_cwelchuscoins_block_content'];
                                $block_content_yellow =  $phrase['_cwelchuscoins_block_content_yellow'];
                            }
                            else if(!empty($navval) && $navval == 'die_variety_all')
                            {
                                $navcrumb = array("young-collection"=>$phrase['_cwelchuscoinsall']);
                                $block_header = $phrase['_cwelchuscoinsall_block_header'];
                                $block_content = $phrase['_cwelchuscoinsall_block_content'];
                                $block_content_yellow =  $phrase['_cwelchuscoinsall_block_content_yellow'];
                            }                           
                            else if(!empty($navval) && $navval == 'young_collection')
                            {
                            ...
                            //Bug ID 4070 end
                                ...
                                $ilance->template->fetch('main', 'die_variety_auction.html');
                                $ilance->template->parse_hash('main', array('ilpage' => $ilpage));
...
function fetch_featured_auctions_new($auctiontype = '', $columns = 4, $rows = 1, $cid = 0, $keywords = '', $forcenoflash = false, $excludelist = array(),$series, $featuredsql='')
        {
                global $ilance, $ilconfig, $show, $phrase, $ilpage, $project_like_keyword,$project_user_id;           
                ...
                {
                        $show['featuredproductauctions'] = 0;
                        return;
                }
